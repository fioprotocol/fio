...
fc::variant call(const std::string &url,
    ...
    try {
        ...
        if (url == ::url)
            std::cerr << localized("Failed to connect to ${n} at ${u}; is ${n} running?",
                                   ("n", node_executable_name)("u", url)) << std::endl;
        else if (url == ::wallet_url)
            std::cerr << localized("Failed to connect to ${k} at ${u}; is ${k} running?",
                                   ("k", key_store_executable_name)("u", url)) << std::endl;
...
chain::action create_newaccount(const name &creator, const name &newaccount, auth_type owner, auth_type active) {
    return action{
            ...
            eosio::chain::newaccount{
                    .creator      = creator,
                    .name         = newaccount,
                    .owner        = owner.contains<public_key_type>() ? authority(owner.get<public_key_type>())
                                                                      : authority(owner.get<permission_level>()),
                    .active       = active.contains<public_key_type>() ? authority(active.get<public_key_type>())
                                                                       : authority(active.get<permission_level>())
            }
    };
}
...
struct set_account_permission_subcommand {
    ...
    set_account_permission_subcommand(CLI::App *accountCmd) {
        ...
        permissions->add_flag("--add-code", add_code,
                              localized("[code] add '${code}' permission to specified permission authority",
                                        ("code", name(config::eosio_code_name))));
        permissions->add_flag("--remove-code", remove_code,
                              localized("[code] remove '${code}' permission from specified permission authority",
                                        ("code", name(config::eosio_code_name))));
        ...
        permissions->set_callback([this] {
            ...
            EOSC_ASSERT((add_code ^ remove_code) || !authority_json_or_file.empty(),
            ...
            if (need_parent || need_auth) {
                fc::variant json = call(get_account_func,
                                        fc::mutable_variant_object("account_name", account.to_string()));
                ...
                if (need_auth) {
                    ...
                    if (itr != res.permissions.end()) {
                            ...
                            if (itr2 != auth.accounts.end() && itr2->permission == code_perm) {
                                ...
                                if ((*itr2).weight < auth.threshold) {
                                    if (auth.threshold > std::numeric_limits<weight_type>::max()) {
                                        std::cerr
                                                << "ERROR: Threshold is too high to be satisfied by sole code permission"
                                                << std::endl;
                                        return;
                                    }
                                    std::cerr << localized(
                                ...
                                if (auth.threshold > std::numeric_limits<weight_type>::max()) {
                                    std::cerr << "ERROR: Threshold is too high to be satisfied by sole code permission"
                                              << std::endl;
                                    return;
                                }
                        ...
                        if (add_code) {
                            ...
                            std::cerr << localized("ERROR: The permission '${permission}' does not exist",
    ...
    set_action_permission_subcommand(CLI::App *actionRoot) {
        ...
        permissions->set_callback([this] {
            ...
            if (is_delete) {
                ...
                send_actions({create_linkauth(account, code, type, requirement, max_fee)});
            }
        });
    }
};
...
void try_local_port(uint32_t duration) {
    ...
    while (!local_port_used()) {
        if (duration_cast<std::chrono::milliseconds>(system_clock::now().time_since_epoch()).count() - start_time >
            duration) {
            std::cerr << "Unable to connect to " << key_store_executable_name << ", if " << key_store_executable_name
...
void ensure_keosd_running(CLI::App *app) {
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        if (keos.running()) {
            ...
            std::cerr << "No wallet service listening on " << wallet_url << ". Failed to launch " << binPath
                      << std::endl;
        }
    } else {
...
struct create_account_subcommand {
    ...
    create_account_subcommand(CLI::App *actionRoot, bool s) : simple(s) {
        ...
        createAccount->add_option("OwnerKey", owner_key_str, localized(
      ...
      //                              (localized("Transfer voting power and right to unstake tokens to receiver")));
        ...
        createAccount->set_callback([this] {
            ...
            if (!simple) {
                ...
                EOSC_ASSERT(!buy_ram_bytes_in_kbytes || !buy_ram_bytes,
                            ...
                            for (auto &action_v : action_variants) {
                                ...
                                if (action_vo.contains("data") && action_vo.contains("hex_data")) {
                                    ...
                                    if (!action_vo["data"].is_string()) {
                                        std::cerr << "get transaction_id only supports un-exploded 'data' (hex form)"
                                                  << std::endl;
                                        return;
                                    }
                                }
...
struct delegate_bandwidth_subcommand {
    ...
    delegate_bandwidth_subcommand(CLI::App *actionRoot) {
        ...
        delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount,
                                       localized("The amount of tokens to stake for network bandwidth"))->required();
        delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount,
                                       localized("The amount of tokens to stake for CPU bandwidth"))->required();
        delegate_bandwidth->add_option("--buyram", buy_ram_amount, localized("The amount of tokens to buyram"));
        delegate_bandwidth->add_option("--buy-ram-bytes", buy_ram_bytes,
                                       localized("The amount of RAM to buy in number of bytes"));
        ...
        delegate_bandwidth->set_callback([this] {
            ...
            std::vector<chain::action> acts{
                    create_action(accountPermissions, config::system_account_name, N(delegatebw), act_payload)};
            EOSC_ASSERT(!(buy_ram_amount.size()) || !buy_ram_bytes,
...
struct undelegate_bandwidth_subcommand {
    ...
    undelegate_bandwidth_subcommand(CLI::App *actionRoot) {
        ...
        undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount,
                                         localized("The amount of tokens to undelegate for CPU bandwidth"))->required();
        ...
        undelegate_bandwidth->set_callback([this] {
            ...
            auto accountPermissions = get_account_permissions(tx_permission, {from_str, config::active_name});
            send_actions(
                    {create_action(accountPermissions, config::system_account_name, N(undelegatebw), act_payload)});
        });
    }
};
    ...
    bidname_info_subcommand(CLI::App *actionRoot) {
        ...
        list_producers->set_callback([this] {
            auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                    ...
                    // Change to newname.value when clio no longer needs to support nodeos versions older than 1.5.0
                    ("limit", 1));
            if (print_json) {
                std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
                return;
...
struct buyram_subcommand {
    ...
    buyram_subcommand(CLI::App *actionRoot) {
        ...
        buyram->set_callback([this] {
            ...
            if (kbytes || bytes) {
                send_actions({create_buyrambytes(from_str, receiver_str,
...
struct sellram_subcommand {
    ...
    sellram_subcommand(CLI::App *actionRoot) {
        ...
        sellram->add_option("account", receiver_str,
                            localized("The account to receive tokens for sold RAM"))->required();
...
struct deposit_subcommand {
    ...
    deposit_subcommand(CLI::App *actionRoot) {
        auto deposit = actionRoot->add_subcommand("deposit", localized(
...
struct withdraw_subcommand {
    ...
    withdraw_subcommand(CLI::App *actionRoot) {
        auto withdraw = actionRoot->add_subcommand("withdraw", localized(
...
struct buyrex_subcommand {
    ...
    buyrex_subcommand(CLI::App *actionRoot) {
        ...
        buyrex->add_option("amount", amount_str,
                           localized("Amount to be taken from REX fund and used in buying REX"))->required();
...
struct lendrex_subcommand {
    ...
    lendrex_subcommand(CLI::App *actionRoot) {
        auto lendrex = actionRoot->add_subcommand("lendrex", localized(
        ...
        lendrex->add_option("amount", amount_str,
                            localized("Amount of liquid tokens to be used in buying REX"))->required();
...
struct unstaketorex_subcommand {
    ...
    unstaketorex_subcommand(CLI::App *actionRoot) {
        ...
        unstaketorex->add_option("receiver", receiver_str,
                                 localized("Account that tokens have been staked to"))->required();
        unstaketorex->add_option("from_net", from_net_str, localized(
                "Amount to be unstaked from Net resources and used in REX purchase"))->required();
        unstaketorex->add_option("from_cpu", from_cpu_str, localized(
...
struct cancelrexorder_subcommand {
    ...
    cancelrexorder_subcommand(CLI::App *actionRoot) {
        auto cancelrexorder = actionRoot->add_subcommand("cancelrexorder",
                                                         localized("Cancel queued REX sell order if one exists"));
...
struct rentcpu_subcommand {
    ...
    rentcpu_subcommand(CLI::App *actionRoot) {
        ...
        rentcpu->add_option("receiver", receiver_str,
                            localized("Account to whom rented CPU bandwidth is staked"))->required();
        rentcpu->add_option("loan_payment", loan_payment_str,
                            localized("Loan fee to be paid, used to calculate amount of rented bandwidth"))->required();
        rentcpu->add_option("loan_fund", loan_fund_str,
                            localized("Loan fund to be used in automatic renewal, can be 0 tokens"))->required();
...
struct rentnet_subcommand {
    ...
    rentnet_subcommand(CLI::App *actionRoot) {
        ...
        rentnet->add_option("receiver", receiver_str,
                            localized("Account to whom rented Network bandwidth is staked"))->required();
...
struct mvfrsavings_subcommand {
    ...
    mvfrsavings_subcommand(CLI::App *actionRoot) {
        auto mvfrsavings = actionRoot->add_subcommand("mvfromsavings",
                                                      localized("Move REX tokens out of savings bucket"));
        mvfrsavings->add_option("owner", owner_str, localized("REX owner"))->required();
        mvfrsavings->add_option("rex", rex_str,
                                localized("Amount of REX to be moved out of savings bucket"))->required();
...
struct updaterex_subcommand {
    ...
    updaterex_subcommand(CLI::App *actionRoot) {
        auto updaterex = actionRoot->add_subcommand("updaterex",
                                                    localized("Update REX owner vote stake and vote weight"));
...
struct consolidate_subcommand {
    ...
    consolidate_subcommand(CLI::App *actionRoot) {
        auto consolidate = actionRoot->add_subcommand("consolidate", localized(
...
struct rexexec_subcommand {
    ...
    rexexec_subcommand(CLI::App *actionRoot) {
        auto rexexec = actionRoot->add_subcommand("rexexec", localized(
        ...
        rexexec->add_option("max", max_str, localized(
...
struct closerex_subcommand {
    ...
    closerex_subcommand(CLI::App *actionRoot) {
        auto closerex = actionRoot->add_subcommand("closerex",
                                                   localized("Delete unused REX-related user table entries"));
...
void get_account(const string &accountName, const string &coresym, bool json_format) {
    ...
    if (coresym.empty()) {
        ...
        json = call(get_account_func, fc::mutable_variant_object("account_name", accountName)("expected_core_symbol",
                                                                                              symbol::from_string(
                                                                                                      coresym)));
    }
...
int main(int argc, char **argv) {
    ...
    app.add_option("-p,--port", obsoleted_option_host_port,
                   localized("the port where ${n} is running", ("n", node_executable_name)))->group("hidden");
    app.add_option("--wallet-host", obsoleted_option_host_port,
                   localized("the host where ${k} is running", ("k", key_store_executable_name)))->group("hidden");
    app.add_option("--wallet-port", obsoleted_option_host_port,
                   localized("the port where ${k} is running", ("k", key_store_executable_name)))->group("hidden");

    app.add_option("-u,--url", url, localized("the http/https URL where ${n} is running", ("n", node_executable_name)),
                   true);
    app.add_option("--wallet-url", wallet_url,
                   localized("the http/https URL where ${k} is running", ("k", key_store_executable_name)), true);
    ...
    app.add_flag("--no-auto-" + string(key_store_executable_name), no_auto_keosd,
                 localized("don't automatically launch a ${k} if one is not currently running",
    ...
    pack_transaction->add_flag("--pack-action-data", pack_action_data_flag,
                               localized("Pack all action data within transaction, needs interaction with ${n}",
    ...
    unpack_transaction->add_flag("--unpack-action-data", unpack_action_data_flag,
                                 localized("Unpack all action data within transaction, needs interaction with ${n}",
                                           ("n", node_executable_name)));
    ...
    getCode->set_callback([&] {
        ...
        try {
            ...
            if (code_as_wasm) {
                ...
                std::cout << localized("Warning: communicating to older ${n} which returns malformed binary wasm",
...
// request subcommand
  ...
  auto request = app.add_subcommand("request", localized("FIO Request contract commands"), false);

       request->require_subcommand();
  string payer_fio_address;
  string payee_fio_address;
  string content;
  string fio_request_id;
// new funds request
    ...
    newfunds_action->set_callback([&] {
      ...
      send_actions({chain::action{get_account_permissions(tx_permission, {actor, config::active_name}), "fio.reqobt", "newfundsreq",
                variant_to_bin(N(fio.reqobt), N(newfundsreq), newfunds)}});
    ...
    // reject funds request
        ...
        rejectfunds_action->set_callback([&] {

          auto rejectfunds = fc::mutable_variant_object
                    ("fio_request_id", fio_request_id)
                    ("actor", actor)
                    ("tpid", tpid)
                    ("max_fee", max_fee);

          send_actions({chain::action{get_account_permissions(tx_permission, {actor, config::active_name}), "fio.reqobt", "rejectfndreq",
                    variant_to_bin(N(fio.reqobt), N(rejectfndreq), rejectfunds)}});
        ...
        cancelfunds_action->set_callback([&] {

          auto cancelfunds = fc::mutable_variant_object
                    ("fio_request_id", fio_request_id)
                    ("actor", actor)
                    ("tpid", tpid)
                    ("max_fee", max_fee);

          send_actions({chain::action{get_account_permissions(tx_permission, {actor, config::active_name}), "fio.reqobt", "cancelfndreq",
                    variant_to_bin(N(fio.reqobt), N(cancelfndreq), cancelfunds)}});
...
// add address subcommand
    ...
    addaddress_action->add_option("addresses", addresses, localized("Blockchain addresses to add to the FIO address"))->required();
...
// renew address subcommand
    ...
    renewaddress_action->add_option("fio_address", fio_address,localized("The FIO address to renew"))->required();
    renewaddress_action->add_option("actor", actor, localized("actor (string)"))->required();
    renewaddress_action->add_option("tpid", tpid, localized("The TPID (Technology Provider ID)"));
    renewaddress_action->add_option("max_fee", max_fee, localized("the max fee desired in smallest units of FIO (SUFs)"));
...
//                                ->check(CLI::ExistingFile);
    ...
    auto set_code_callback = [&]() {
        ...
        if (!duplicate) {
            ...
            std::cerr << localized("Skipping set code because the new code is the same as the existing code")
                      << std::endl;
        }
    };
    ...
    recordobt_action->set_callback([&] {

      auto recordobt = fc::mutable_variant_object
                ("fio_request_id", fio_request_id)
                ("payer_fio_address", payer_fio_address)
                ("payee_fio_address", payee_fio_address)
                ("actor", actor)
    ...
    createWallet->set_callback([&wallet_name, &password_file, &print_console] {
        EOSC_ASSERT(!password_file.empty() ^ print_console,
                    "ERROR: Either indicate a file using \"--file\" or pass \"--to-console\"");
        EOSC_ASSERT(password_file.empty() || !std::ofstream(password_file.c_str()).fail(),
    ...
    review->add_flag("--show-approvals", show_approvals_in_multisig_review,
                     localized("Show the status of the approvals requested within the proposal"));
    ...
    invalidate->set_callback([&] {
        ...
        send_actions({chain::action{accountPermissions, "eosio.msig", "invalidate",
                                    variant_to_bin(N(eosio.msig), "invalidate", args)}});
    });
    ...
    executer = "";
    string trx_to_exec;
    auto wrap_exec = wrap->add_subcommand("exec",
    ...
    wrap_exec->add_option("executer", executer, localized(
