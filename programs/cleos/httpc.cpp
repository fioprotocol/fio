        ...
        namespace http {
            ...
            parsed_url parse_url(const string &server_url) {
                ...
                if (res.scheme != "http" && res.scheme != "https")
                    EOS_THROW(fail_to_resolve_host, "Unrecognized URL scheme (${s}) in URL \"${u}\"",
            ...
            resolved_url resolve_url(const http_context &context, const parsed_url &url) {
                ...
                if (ec) {
                    EOS_THROW(fail_to_resolve_host, "Error resolving \"${server}:${port}\" : ${m}",
                ...
                for (const auto &r : result) {
                    ...
                    if (resolved_port) {
                        EOS_ASSERT(*resolved_port == port, resolved_to_multiple_ports,
            ...
            fc::variant do_http_call(const connection_param &cp,
                ...
                try {
                    ...
                    e.append_log(FC_LOG_MESSAGE(info,
                ...
                if (status_code == 200 || status_code == 201 || status_code == 202) {
                    ...
                    if (url.path.compare(0, chain_func_base.size(), chain_func_base) == 0) {
                        throw chain::missing_chain_api_plugin_exception(
                                FC_LOG_MESSAGE(error, "Chain API plugin is not enabled"));
                    } else if (url.path.compare(0, wallet_func_base.size(), wallet_func_base) == 0) {
                        throw chain::missing_wallet_api_plugin_exception(
                                FC_LOG_MESSAGE(error, "Wallet is not available"));
                    } else if (url.path.compare(0, history_func_base.size(), history_func_base) == 0) {
                        throw chain::missing_history_api_plugin_exception(
                                FC_LOG_MESSAGE(error, "History API plugin is not enabled"));
