...
namespace eosiosystem {
     ...
     */
    void system_contract::rentcpu(const name &from, const name &receiver, const asset &loan_payment,
     ...
     * @param loan_fund - additional tokens added to loan fund and used later for loan renewal
     */
    void system_contract::rentnet(const name &from, const name &receiver, const asset &loan_payment,
                                  const asset &loan_fund) {
        require_auth(from);
     ...
     */
    void system_contract::update_resource_limits(const name &from, const name &receiver, int64_t delta_net,
                                                 int64_t delta_cpu) {
            ...
            for (uint16_t i = 0; i < max; ++i) {
                ...
                if (bitr != _rexbalance.end()) { // should always be true
                    ...
                    if (result.success) {
                        ...
                        rex_results::orderresult_action order_act(rex_account,
    ...
    template<typename T>
    int64_t system_contract::rent_rex(T &table, const name &from, const name &receiver, const asset &payment,
                                      const asset &fund) {
        runrex(2);
    ...
    rex_order_outcome system_contract::fill_rex_order(const rex_balance_table::const_iterator &bitr, const asset &rex) {
        ...
        const int64_t available_unlent =
                rexitr->total_unlent.amount - unlent_lower_bound; // available_unlent <= 0 is possible
    ...
    asset system_contract::add_to_rex_pool(const asset &payment) {
        ...
        const asset init_total_rent(20'000'0000,
                                    core_symbol()); /// base balance prevents renting profitably until at least a minimum number of core_symbol() is made available
