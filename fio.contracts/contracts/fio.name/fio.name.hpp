...
namespace fioio {
    ...
    struct [[eosio::action]] fioname {
        ...
        EOSLIB_SERIALIZE(fioname, (name)(namehash)(domain)(domainhash)(expiration)(account)(addresses)(bundleeligiblecountdown))
    };

    //Where fioname tokens are stored
    typedef multi_index<"fionames"_n, fioname,
            indexed_by<"bydomain"_n, const_mem_fun<fioname, uint64_t, &fioname::by_domain> > > fionames_table;
    ...
    typedef multi_index<"domains"_n, domain,
            indexed_by<"byaccount"_n, const_mem_fun<domain, uint64_t, &domain::by_account> > > domains_table;
    ...
    typedef multi_index<"keynames"_n, key_name,
            indexed_by<"bykey"_n, const_mem_fun<key_name, uint64_t, &key_name::by_keyhash> > > keynames_table;
