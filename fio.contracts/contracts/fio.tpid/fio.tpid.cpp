...
namespace fioio {
    ...
    public:
        ...
        void updatetpid(const string &tpid, const name owner, const uint64_t &amount) {
            ...
            eosio_assert(has_auth(SystemContract) || has_auth("fio.token"_n) || has_auth("fio.treasury"_n) ||
        ...
        void rewardspaid(const string &tpid) {
            require_auth(
                    "fio.treasury"_n); //This action can only be called by fio.treasury after successful rewards payment to tpid
