...
memcpy(&max, &amount,
sizeof(share_type)); // hack in an invalid amount

BOOST_CHECK_EXCEPTION( create(N(alice), max), asset_type_exception,
[](
...
issue( N(alice), N(alice), asset::from_string("500.000 TKN"),
...
issue( N(alice), N(alice), asset::from_string("1000 CERO"),
...
transfer( N(alice), N(bob), asset::from_string("300 CERO"),
...
auto alice_balance = get_account(N(alice), "0,CERO");
BOOST_REQUIRE_EQUAL(true, alice_balance.
...
auto bob_balance = get_account(N(bob), "0,CERO");
BOOST_REQUIRE_EQUAL(true, bob_balance.
