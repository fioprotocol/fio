    ...
    public:
        ...
        inline name accountmgnt(const name &actor, const string &owner_fio_public_key) {
            ...
            if (owner_fio_public_key.length() == 0) {
                ...
                eosio::check(owner_account.length() == 12, "Length of account name should be 12");
                ...
                if (other == accountmap.end()) { //the name is not in the table.
                    ...
                    fio_400_assert(accountExists, "owner_account", owner_account,
                                   ...
                                   ErrorPubAddressExist);
                    check(owner_fio_public_key == other->clientkey, "FIO account already bound",
                                              ErrorPubAddressExist);
                }
            }
        ...
        uint32_t fio_address_update(const name &owneractor, const name &actor, int64_t max_fee, const FioAddress &fa,
            ...
            fio_400_assert(now() <= domain_expiration, "fio_address", fa.fioaddress, "FIO Domain expired",
                           ErrorDomainExpired);
        ...
        chain_data_update(const string &fioaddress, const vector<tokenpubaddr> pubaddresses,
            ...
            fio_400_assert(now() <= name_expiration, "fio_address", fioaddress,
         ...
         * @return  the decremented now() by nyearsago
         */
        ...
        regaddress(const string &fio_address, const string &owner_fio_public_key, const int64_t &max_fee, const name &actor,
            ...
            eosio::internal_use_do_not_use::send_response(json.dump().c_str());
        }
        ...
        */
        ...
        void burnexpired() {
           ...
           // uint64_t kludgedNow = get_now_plus_years(10); // This is for testing only
            ...
            while (domainiter != domainexpidx.end()) {
                ...
                if ((expire + DOMAINWAITFORBURNDAYS) > now()){
                    break;
                } else {
            ...
            if (burnlist.size() < numbertoburn) {
                ...
                while (nameiter != nameexpidx.end()) {
                    ...
                    if ((expire + ADDRESSWAITFORBURNDAYS) > now()){
                        break;
                    } else {
        ...
        setdomainpub(const string &fio_domain, const int8_t is_public, const int64_t &max_fee, const name &actor,
            ...
            fio_400_assert(now() <= expiration, "fio_domain", fa.fiodomain, "FIO Domain expired",
                           ErrorDomainExpired);
        ...
        void bind2eosio(name account, const string &client_key, bool existing) {
            ...
            if (other != accountmap.end()) {
                check(existing && client_key == other->clientkey, "EOSIO account already bound",
                                          ErrorPubAddressExist);
            } else {
                check(!existing, "existing EOSIO account not bound to a key", ErrorPubAddressExist);
                accountmap.emplace(_self, [&](struct eosio_name &p) {
