...
namespace fioio {
            ...
            for (auto &feeval : fee_values) {
                ...
                while (votebyname_iter != feevotesbybpname.end())
                {
                    ...
                    if (votebyname_iter->end_point_hash == endPointHash) {
                        ...
                        if (lastupdate > (now() - TIME_BETWEEN_VOTES_SECONDS)) {
                            timeviolation = true;
                            break;
                        } else {
    ...
    EOSIO_DISPATCH(FioFee, (create)(setfeemult)(setfeevote)(bundlevote)(updatefees)
    )
}
