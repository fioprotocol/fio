...
public:
    ...
    void propose(name proposer, name proposal_name, vector <permission_level> requested_permissions,
    ...
    transaction wrap_exec(account_name executer, const transaction &trx,
                          uint32_t expiration = base_tester::DEFAULT_EXPIRATION_DELTA);
...
wrap_trx.
sign( get_private_key(actor, "active"), control
->
...
BOOST_REQUIRE_EQUAL( "eosio", name{
trace->action_traces[0].act.account} );
BOOST_REQUIRE_EQUAL( "reqauth", name{
trace->action_traces[0].act.name} );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig, eosio_wrap_tester
) try {
auto trx = reqauth(N(bob), {permission_level{N(bob), config::active_name}});
...
approve( N(carol), N(first), N(alice)

); // alice must approve since she is the executer of the wrap::exec action
...
BOOST_REQUIRE_EQUAL( 2, traces.

size()

);

BOOST_REQUIRE_EQUAL( 1, traces[0]->action_traces.

size()

);
BOOST_REQUIRE_EQUAL( "eosio.wrap", name{
traces[0]->action_traces[0].act.account} );
BOOST_REQUIRE_EQUAL( "exec", name{
traces[0]->action_traces[0].act.name} );
BOOST_REQUIRE_EQUAL( transaction_receipt::executed, traces[0]
->receipt->status );

BOOST_REQUIRE_EQUAL( 1, traces[1]->action_traces.

size()

);
BOOST_REQUIRE_EQUAL( "eosio", name{
traces[1]->action_traces[0].act.account} );
BOOST_REQUIRE_EQUAL( "reqauth", name{
traces[1]->action_traces[0].act.name} );
BOOST_REQUIRE_EQUAL( transaction_receipt::executed, traces[1]
->receipt->status );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_unapprove, eosio_wrap_tester
) try {
auto trx = reqauth(N(bob), {permission_level{N(bob), config::active_name}});
auto wrap_trx = wrap_exec(N(alice), trx);
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_producers_change, eosio_wrap_tester
) try {
create_accounts( {
N(newprod1)
} );
...
set_producers( {
N(prod1), N(prod2), N(prod3), N(prod4), N(prod5), N(newprod1)
} ); // With 6 producers, the 2/3+1 threshold becomes 5

while( control->pending_block_state()->active_schedule.producers.
