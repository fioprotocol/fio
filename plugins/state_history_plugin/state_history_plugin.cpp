...
namespace eosio {
    ...
    bool
    include_delta(const eosio::chain::protocol_state_object &old, const eosio::chain::protocol_state_object &curr) {
    ...
    struct state_history_plugin_impl : std::enable_shared_from_this<state_history_plugin_impl> {
        ...
        fc::optional<chain::block_id_type> get_block_id(uint32_t block_num) {
            ...
            if (chain_state_log && block_num >= chain_state_log->begin_block() &&
                block_num < chain_state_log->end_block())
                ...
                if (current_request->start_block_num <= current &&
                    ...
                    if (block_id) {
                        ...
                        if (current_request->fetch_deltas && plugin->chain_state_log)
                            plugin->get_log_entry(*plugin->chain_state_log, current_request->start_block_num,
                                                  result.deltas);
                    }
        ...
        void do_accept() {
            ...
            acceptor->async_accept(*socket,
                                   [self = shared_from_this(), socket, this](const boost::system::error_code &ec) {
                                       if (stopping)
                                           return;
                                       if (ec) {
        ...
        void store_traces(const block_state_ptr &block_state) {
            ...
            auto traces_bin = zlib_compress_bytes(
                    fc::raw::pack(make_history_context_wrapper(db, trace_debug_mode, traces)));
        ...
        void store_chain_state(const block_state_ptr &block_state) {
            ...
            process_table("resource_limits_state", db.get_index<resource_limits::resource_limits_state_index>(),
                          pack_row);
            process_table("resource_limits_config", db.get_index<resource_limits::resource_limits_config_index>(),
                          pack_row);
    ...
    void state_history_plugin::set_program_options(options_description &cli, options_description &cfg) {
        ...
        cli.add_options()("delete-state-history", bpo::bool_switch()->default_value(false),
    ...
    void state_history_plugin::plugin_initialize(const variables_map &options) {
        try {
            ...
            if (options.at("chain-state-history").as<bool>())
                my->chain_state_log.emplace("chain_state_history",
                                            (state_history_dir / "chain_state_history.log").string(),
