...
namespace fc {
    ...
    template<typename ST, typename T>
    datastream <ST> &operator<<(datastream <ST> &ds, const history_serial_wrapper<std::vector < T>>

    & obj) {
    return
    history_serialize_container(ds, obj
    .db, obj.obj);
}

template<typename ST, typename First, typename Second>
datastream <ST> &operator<<(datastream <ST> &ds, const history_serial_wrapper<std::pair < First, Second>>
...
datastream <ST> &operator<<(datastream <ST> &ds,
    ...
    history_serialize_container(ds, obj.db,
                                as_type < eosio::chain::shared_vector <
                                eosio::chain::producer_key >> (obj.obj.producers));
    return ds;
}
...
datastream <ST> &operator<<(datastream <ST> &ds,
    ...
    fc::raw::pack(
            ds,
            make_history_serial_wrapper(obj.db, as_type<eosio::chain::resource_limits::ratio>(obj.obj.contract_rate)));
    fc::raw::pack(
            ds,
            make_history_serial_wrapper(obj.db, as_type<eosio::chain::resource_limits::ratio>(obj.obj.expand_rate)));
    return ds;
}
...
operator<<(datastream <ST> &ds,
    ...
    fc::raw::pack(
            ds, make_history_serial_wrapper(
                    obj.db,
                    as_type<eosio::chain::resource_limits::elastic_limit_parameters>(obj.obj.cpu_limit_parameters)));
    fc::raw::pack(
            ds, make_history_serial_wrapper(
                    obj.db,
                    as_type<eosio::chain::resource_limits::elastic_limit_parameters>(obj.obj.net_limit_parameters)));
...
datastream <ST> &operator<<(datastream <ST> &ds, const history_serial_wrapper<eosio::chain::action_trace> &obj) {
    ...
    history_serialize_container(ds, obj.db,
                                as_type < std::vector < eosio::chain::action_trace >> (obj.obj.inline_traces));
    return ds;
}
...
datastream <ST> &operator<<(datastream <ST> &ds,
    ...
    history_serialize_container(ds, obj.db,
                                as_type < std::vector < eosio::chain::action_trace >> (obj.obj.action_traces));
