...
// todo: currently only checks version on the first record. Need in recover_blocks
    ...
    public:
        state_history_log(const char *const name, std::string log_filename, std::string index_filename)
                : name(name), log_filename(std::move(log_filename)), index_filename(std::move(index_filename)) {
            open_log();
            open_index();
        }
    ...
    private:
        ...
        void open_index() {
            ...
            while (pos < size) {
                state_history_log_header header;
                EOS_ASSERT(pos + sizeof(header) <= size, chain::plugin_exception, "corrupt ${name}.log (6)",
                           ("name", name));
                log.seekg(pos);
                log.read((char *) &header, sizeof(header));
        ...
        void truncate(uint32_t block_num) {
            ...
            if (block_num <= _begin_block) {
                ...
                boost::filesystem::resize_file(index_filename,
                                               (block_num - _begin_block) * sizeof(state_history_summary));
