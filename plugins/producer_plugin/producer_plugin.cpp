...
namespace eosio {
    ...
    BOOST_MULTI_INDEX_MEMBER(transaction_id_with_expiry, transaction_id_type, trx_id
    )>,
    ordered_non_unique<tag < by_expiry>,
    BOOST_MULTI_INDEX_MEMBER(transaction_id_with_expiry, fc::time_point, expiry
    )>
    >
    >;
...
#define CATCH_AND_CALL(NEXT)\
    ...
    public:
            ...
            std::set_intersection(_producers.begin(), _producers.end(),
                                  ...
                                  boost::make_function_output_iterator([&](const chain::account_name &producer) {
                                      if (producer != bsp->header.producer) {
                                          auto itr = std::find_if(active_producer_to_signing_key.begin(),
                                                                  active_producer_to_signing_key.end(),
                                                                  [&](const producer_key &k) {
                                                                      return k.producer_name == producer;
                                                                  });
        ...
        std::deque <std::tuple<transaction_metadata_ptr, bool, next_function < transaction_trace_ptr>>>
        _pending_incoming_transactions;
    ...
    void producer_plugin::set_program_options(
        ...
        producer_options.add_options()
                ...
                ("signature-provider", boost::program_options::value < vector < string
                         >> ()->composing()->multitoken()->default_value(
                                 {std::string(default_priv_key.get_public_key()) + "=KEY:" +
                                  std::string(default_priv_key)},
                                 std::string(default_priv_key.get_public_key()) + "=KEY:" +
                                 std::string(default_priv_key)),
...
if( options.count(name) ) { \
   ...
   std::copy(ops.begin(), ops.end(), std::inserter(container, container.end())); \
    ...
    void producer_plugin::plugin_initialize(const boost::program_options::variables_map &options) {
        try {
            ...
            if (options.count("private-key")) {
                ...
                for (const std::string &key_id_to_wif_pair_string : key_id_to_wif_pair_strings) {
                    try {
                        ...
                        dejsonify < std::pair < public_key_type, private_key_type >> (key_id_to_wif_pair_string);
            ...
            if (options.count("signature-provider")) {
                const std::vector <std::string> key_spec_pairs =
                        options["signature-provider"].as < std::vector < std::string >> ();
    ...
    producer_plugin::snapshot_information producer_plugin::create_snapshot() const {
        ...
        std::string snapshot_path = (my->_snapshots_dir / fc::format_string("snapshot-${id}.bin",
                                                                            fc::mutable_variant_object()("id",
                                                                                                         head_id))).generic_string();
    ...
    producer_plugin_impl::start_block_result producer_plugin_impl::start_block() {
            ...
            if (_pending_block_mode == pending_block_mode::producing) {
                ...
                if (currrent_watermark_itr != _producer_watermarks.end()) {
                    ...
                    if (watermark < hbs->block_num) {
                        blocks_to_confirm = std::min<uint16_t>(std::numeric_limits<uint16_t>::max(),
                                                               (uint16_t)(hbs->block_num - watermark));
                    }
                }
            }
        ...
        if (pbs) {
            ...
            if (_pending_block_mode == pending_block_mode::producing &&
                pbs->block_signing_key != scheduled_producer.block_signing_key) {
            ...
            if (!persisted_by_expiry.empty()) {
                ...
                while (!persisted_by_expiry.empty() &&
                       persisted_by_expiry.begin()->expiry <= pbs->header.timestamp.to_time_point()) {
                ...
                if (exhausted) {
                    fc_wlog(_log,
                            "Unable to process all ${n} persisted transactions before deadline, Expired ${expired}",
                            ("n", orig_count)
            ...
            try {
                ...
                if (_pending_block_mode == pending_block_mode::producing) {
                    ...
                    while (sch_itr != sch_idx.end()) {
                        ...
                        while (_incoming_trx_weight >= 1.0 && orig_pending_txn_size &&
                        ...
                        try {
                            auto deadline = fc::time_point::now() + fc::milliseconds(_max_transaction_time_ms);
                            bool deadline_is_subjective = false;
                            if (_max_transaction_time_ms < 0 || (_pending_block_mode == pending_block_mode::producing &&
                                                                 scheduled_trx_deadline < deadline)) {
                                deadline_is_subjective = true;
                ...
                if (exhausted || preprocess_deadline <= fc::time_point::now()) {
                    ...
                    if (!_pending_incoming_transactions.empty()) {
                        fc_dlog(_log, "Processing ${n} pending transactions",
                                ("n", _pending_incoming_transactions.size()));
    ...
    void producer_plugin_impl::schedule_production_loop() {
        ...
        if (result == start_block_result::failed) {
            ...
            if (deadline > fc::time_point::now()) {
                ...
                EOS_ASSERT(chain.pending_block_state(), missing_pending_block_state,
            ...
            _timer.async_wait(app().get_priority_queue().wrap(priority::high,
                                                              [&chain, weak_this, cid = ++_timer_corelation_id](
                                                                  ...
                                                                  if (self &&
                                                                      ...
                                                                      auto block_num = chain.pending_block_state()
    ...
    void producer_plugin_impl::produce_block() {
        ...
        const auto &pbs = chain.pending_block_state();
        const auto &hbs = chain.head_block_state();
        EOS_ASSERT(pbs, missing_pending_block_state,
