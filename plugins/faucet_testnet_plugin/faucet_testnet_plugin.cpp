   ...
   [this](string, string body, url_response_callback response_cb) mutable { \
    ...
    struct faucet_testnet_plugin_impl {
        ...
        faucet_testnet_plugin_impl(appbase::application &app)
                : _app(app), _timer{app.get_io_service()} {
        }
        ...
        results_pair create_account(const std::string &new_account_name, const fc::crypto::public_key &owner_pub_key,
                                    const fc::crypto::public_key &active_pub_key) {
            ...
            EOS_ASSERT(creating_account != nullptr, transaction_exception,
                       "To create account using the faucet, must already have created account \"${a}\"",
                       ("a", _create_account_name));
            ...
            auto memo = fc::variant(fc::time_point::now()).as_string() + " " +
        ...
        results_pair create_faucet_account(const std::string &body) {
            const eosio::detail::faucet_testnet_create_account_params params = fc::json::from_string(
                    body).as<eosio::detail::faucet_testnet_create_account_params>();
            return create_account(params.account, fc::crypto::public_key(params.keys.owner),
                                  fc::crypto::public_key(params.keys.active));
        }
...
// defaults to the public/private key of init accounts in private testnet genesis.json
    const key_pair faucet_testnet_plugin_impl::_default_key_pair = {
    ...
    void faucet_testnet_plugin::set_program_options(options_description &, options_description &cfg) {
        cfg.add_options()
                ("faucet-create-interval-ms",
                 bpo::value<uint32_t>()->default_value(faucet_testnet_plugin_impl::_default_create_interval_msec),
                 "Time to wait, in milliseconds, between creating next faucet created account.")
                ("faucet-name",
                 bpo::value<std::string>()->default_value(faucet_testnet_plugin_impl::_default_create_account_name),
                 "Name to use as creator for faucet created accounts.")
                ("faucet-private-key", boost::program_options::value<std::string>()->default_value(
                        fc::json::to_string(faucet_testnet_plugin_impl::_default_key_pair)),
    ...
    void faucet_testnet_plugin::plugin_initialize(const variables_map &options) {
        try {
            ...
            auto faucet_key_pair = fc::json::from_string(
                    options.at("faucet-private-key").as<std::string>()).as<key_pair>();
    ...
    void faucet_testnet_plugin::plugin_startup() {
        app().get_plugin<http_plugin>().add_api({
                                                        CALL(faucet, my, create_account,
                                                             faucet_testnet_plugin_impl::create_faucet_account)
                                                });
    }
