    ...
    void wallet_plugin::plugin_initialize(const variables_map &options) {
        ...
        try {
            ...
            if (options.count("unlock-timeout")) {
                ...
                EOS_ASSERT(timeout > 0, chain::invalid_lock_timeout_exception, "Please specify a positive timeout ${t}",
                           ("t", timeout));
            ...
            if (options.count("yubihsm-authkey")) {
                ...
                try {
                    wallet_manager_ptr->own_and_use_wallet("YubiHSM",
                                                           make_unique<yubihsm_wallet>(connector_endpoint, key));
                }
                FC_LOG_AND_RETHROW()
            }
        }
