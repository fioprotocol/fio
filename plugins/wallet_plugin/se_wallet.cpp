                ...
                static public_key_data get_public_key_data(SecKeyRef key) {
                    ...
                    if (error) {
                        ...
                        FC_THROW_EXCEPTION(chain::wallet_exception,
                                           "Failed to get public key from Secure Enclave: ${m}", ("m", error_string));
                    }
...
#define STR(A) #A
                ...
                void populate_existing_keys() {
                    ...
                    CFDictionaryRef keyAttrDic = CFDictionaryCreate(nullptr, keyAttrKeys, keyAttrValues,
                                                                    sizeof(keyAttrValues) / sizeof(keyAttrValues[0]),
                ...
                public_key_type create() {
                    SecAccessControlRef accessControlRef = SecAccessControlCreateWithFlags(nullptr,
                    ...
                    CFDictionaryRef keyAttrDic = CFDictionaryCreate(NULL, keyAttrKeys, keyAttrValues,
                                                                    sizeof(keyAttrValues) / sizeof(keyAttrValues[0]),
                                                                    &kCFTypeDictionaryKeyCallBacks,
                                                                    &kCFTypeDictionaryValueCallBacks);
                    ...
                    CFDictionaryRef attributesDic = CFDictionaryCreate(NULL, attrKeys, atrrValues,
                                                                       sizeof(attrKeys) / sizeof(attrKeys[0]),
                                                                       &kCFTypeDictionaryKeyCallBacks,
                                                                       &kCFTypeDictionaryValueCallBacks);
                    ...
                    if (error_string.size())
                        FC_THROW_EXCEPTION(chain::wallet_exception, "Failed to create key in Secure Enclave: ${m}",
                                           ("m", error_string));
                ...
                fc::optional<signature_type> try_sign_digest(const digest_type d, const public_key_type public_key) {
                    ...
                    CFDataRef digestData = CFDataCreateWithBytesNoCopy(nullptr, (UInt8 *) d.data(), d.data_size(),
                                                                       kCFAllocatorNull);
                    CFDataRef signature = SecKeyCreateSignature(it->second,
                    ...
                    if (error) {
                        ...
                        FC_THROW_EXCEPTION(chain::wallet_exception, "Failed to sign digest in Secure Enclave: ${m}",
                                           ("m", error_string));
                    }
                ...
                bool remove_key(string public_key) {
                    ...
                    if (!fut.get())
                        FC_THROW_EXCEPTION(chain::wallet_invalid_password_exception,
                                           "Local user authentication failed");

                    CFDictionaryRef deleteDic = CFDictionaryCreate(nullptr, (const void **) &kSecValueRef,
                                                                   (const void **) &it->second, 1,
                                                                   &kCFTypeDictionaryKeyCallBacks,
                                                                   &kCFTypeDictionaryValueCallBacks);
            ...
            static void check_signed() {
                ...
                CFDictionaryRef piddict = CFDictionaryCreate(kCFAllocatorDefault,
                                                             ...
                                                             (const void **) &pidnumber, 1, nullptr, nullptr);
