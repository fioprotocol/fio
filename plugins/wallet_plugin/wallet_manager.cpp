    ...
    namespace wallet {
        ...
        void wallet_manager::set_timeout(const std::chrono::seconds &t) {
            ...
            EOS_ASSERT(timeout_time >= now && timeout_time.time_since_epoch().count() > 0,
                       ...
                       ("t", t.count())("now", now.time_since_epoch().count())("timeout_time",
        ...
        std::string wallet_manager::create(const std::string &name) {
            ...
            EOS_ASSERT(valid_filename(name), wallet_exception, "Invalid filename, path not allowed in wallet name ${n}",
                       ("n", name));
        ...
        void wallet_manager::open(const std::string &name) {
            ...
            if (!wallet->load_wallet_file()) {
                EOS_THROW(chain::wallet_nonexistent_exception, "Unable to open file: ${f}",
        ...
        wallet_manager::sign_transaction(const chain::signed_transaction &txn, const flat_set<public_key_type> &keys,
            ...
            for (const auto &pk : keys) {
                ...
                for (const auto &i : wallets) {
                    if (!i.second->is_locked()) {
                        fc::optional<signature_type> sig = i.second->try_sign_digest(
                                stxn.sig_digest(id, stxn.context_free_data), pk);
                        if (sig) {
        ...
        void wallet_manager::initialize_lock() {
            ...
            lock_path = dir / "wallet.lock";
            {
                ...
                EOS_ASSERT(!x.fail(), wallet_exception, "Failed to open wallet lock file at ${f}",
                           ("f", lock_path.string()));
            }
