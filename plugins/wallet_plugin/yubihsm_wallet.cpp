...
namespace eosio {
    namespace wallet {
            ...
            struct yubihsm_wallet_impl {
                ...
                key_map_type::iterator populate_key_map_with_keyid(const uint16_t key_id) {
                    ...
                    if ((rc = yh_util_get_public_key(session, key_id, blob, &blob_sz, nullptr)))
                        FC_THROW_EXCEPTION(chain::wallet_exception, "yh_util_get_public_key failed: ${m}",
                                           ("m", yh_strerror(rc)));
                    if (blob_sz != 64)
                ...
                void unlock(const string &password) {
                    ...
                    try {
                        if ((rc = yh_init_connector(endpoint.c_str(), &connector)))
                            FC_THROW_EXCEPTION(chain::wallet_exception,
                                               "Failled to initialize yubihsm connector URL: ${c}",
                                               ("c", yh_strerror(rc)));
                        if ((rc = yh_connect(connector, 0)))
                            FC_THROW_EXCEPTION(chain::wallet_exception, "Failed to connect to YubiHSM connector: ${m}",
                                               ("m", yh_strerror(rc)));
                        if ((rc = yh_create_session_derived(connector, authkey, (const uint8_t *) password.data(),
                                                            password.size(), false, &session)))
                            FC_THROW_EXCEPTION(chain::wallet_exception, "Failed to create YubiHSM session: ${m}",
                                               ("m", yh_strerror(rc)));
                        if ((rc = yh_authenticate_session(session)))
                            FC_THROW_EXCEPTION(chain::wallet_exception, "Failed to authenticate YubiHSM session: ${m}",
                                               ("m", yh_strerror(rc)));

                        yh_object_descriptor authkey_desc;
                        if ((rc = yh_util_get_object_info(session, authkey, YH_AUTHENTICATION_KEY, &authkey_desc)))
                            FC_THROW_EXCEPTION(chain::wallet_exception, "Failed to get authkey info: ${m}",
                                               ("m", yh_strerror(rc)));
                        ...
                        if ((rc = yh_util_list_objects(session, 0, YH_ASYMMETRIC_KEY, 0, &find_caps, YH_ALGO_EC_P256,
                                                       nullptr, found_objs, &found_objects_n)))
                            FC_THROW_EXCEPTION(chain::wallet_exception, "yh_util_list_objects failed: ${m}",
                                               ("m", yh_strerror(rc)));
                ...
                fc::optional<signature_type> try_sign_digest(const digest_type d, const public_key_type public_key) {
                    ...
                    yh_rc rc;
                    if ((rc = yh_util_sign_ecdsa(session, it->second, (uint8_t *) d.data(), d.data_size(), der_sig,
                                                 &der_sig_sz))) {
                        lock();
                        FC_THROW_EXCEPTION(chain::wallet_exception, "yh_util_sign_ecdsa failed: ${m}",
                                           ("m", yh_strerror(rc)));
                    }
                ...
                public_key_type create() {
                    ...
                    try {
                        if ((rc = yh_util_generate_ec_key(session, &new_key_id, "keosd created key", authkey_domains,
                                                          &creation_caps, YH_ALGO_EC_P256)))
                            FC_THROW_EXCEPTION(chain::wallet_exception, "yh_util_generate_ec_key failed: ${m}",
                                               ("m", yh_strerror(rc)));
        ...
        yubihsm_wallet::yubihsm_wallet(const string &connector, const uint16_t authkey) : my(
                new detail::yubihsm_wallet_impl(connector, authkey)) {
        }
