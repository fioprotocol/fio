            ...
            public:
                ...
                string create_key(string key_type) {
                    ...
                    if (key_type == "K1")
                        ...
                        EOS_THROW(chain::unsupported_key_type_exception,
                                  "Key type \"${kt}\" not supported by software wallet", ("kt", key_type));
...
namespace eosio {
    namespace wallet {
        ...
        void soft_wallet::unlock(string password) {
            try {
                ...
                my->_checksum = pk.checksum;
                                     ...
                                     "Invalid password for wallet: \"${wallet_name}\"",
                                     ("wallet_name", get_wallet_filename()))
        }
        ...
        void soft_wallet::set_password(string password) {
            if (!is_new())
                EOS_ASSERT(!is_locked(), wallet_locked_exception,
