...
namespace eosio {
    ...
    void test_control_plugin_impl::process_next_block_state(const chain::block_header_state &bhs) {
        ...
        if (_producer != account_name())
            ilog("producer ${cprod}, looking for ${lprod}",
                 ("cprod", producer_name.to_string())("lprod", _producer.to_string()));
    ...
    namespace test_control_apis {
        read_write::kill_node_on_producer_results
        read_write::kill_node_on_producer(const read_write::kill_node_on_producer_params &params) const {

            if (params.based_on_lib) {
                ilog("kill on lib for producer: ${p} at their ${s} slot in sequence",
                     ("p", params.producer.to_string())("s", params.where_in_sequence));
                my->kill_on_lib(params.producer, params.where_in_sequence);
            } else {
                ilog("kill on head for producer: ${p} at their ${s} slot in sequence",
                     ("p", params.producer.to_string())("s", params.where_in_sequence));
