...
namespace eosio {
    ...
    void http_plugin::set_program_options(options_description &, options_description &cfg) {
        ...
        if (current_http_plugin_defaults.default_unix_socket_path.length())
            cfg.add_options()
                    (my->unix_socket_path_option_name.c_str(),
        ...
        if (current_http_plugin_defaults.default_http_port)
            cfg.add_options()
                    (my->http_server_address_option_name.c_str(), bpo::value<string>()->default_value(
    ...
    void http_plugin::plugin_initialize(const variables_map &options) {
        try {
            ...
            if (options.count(my->http_server_address_option_name) &&
                options.at(my->http_server_address_option_name).as<string>().length()) {
            ...
            if (options.count(my->unix_socket_path_option_name) &&
                !options.at(my->unix_socket_path_option_name).as<string>().empty()) {
            ...
            if (options.count(my->https_server_address_option_name) &&
                options.at(my->https_server_address_option_name).as<string>().length()) {
    ...
    void http_plugin::handle_exception(const char *api_name, const char *call_name, const string &body,
        ...
        try {
            try {
                ...
                error_results results{500, "Internal Service Error - std",
                                      error_results::error_info(fc::exception(FC_LOG_MESSAGE(error, e.what())),
                                                                verbose_http_errors)};
