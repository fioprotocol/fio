...
namespace eosio {
    ...
    public:
        ...
        const std::chrono::system_clock::duration peer_authentication_interval{std::chrono::seconds{
                1}}; ///< Peer clock may be no more than 1 second skewed from our clock, including network latency.
        ...
        void connect(const connection_ptr &c, const std::shared_ptr<tcp::resolver> &resolver,
       ...
       */
        ...
        void handle_message(const connection_ptr &c,
                            const signed_block &msg) = delete; // signed_block_ptr overload used instead
        void handle_message(const connection_ptr &c, const signed_block_ptr &msg);

        void handle_message(const connection_ptr &c,
                            const packed_transaction &msg) = delete; // packed_transaction_ptr overload used instead
  ...
  FC_MULTILINE_MACRO_END
    ...
    */
        ...
        try {
            if (last_handshake_recv.generation >= 1) {
                ...
                fc_dlog(logger, "maybe truncating branch at  = ${h}:${id}",
                        ("h", remote_head_num)("id", remote_head_id));
            }
    ...
    void connection::do_queue_write(int priority) {
        ...
        boost::asio::async_write(*socket, bufs,
                                 boost::asio::bind_executor(strand,
                                                            [c, socket = socket, priority](boost::system::error_code ec,
                                                                                           std::size_t w) {
                                                                app().post(priority, [c, priority, ec, w]() {
                                                                    try {
                                                                        ...
                                                                        if (ec) {
                                                                            string pname = conn ? conn->peer_name()
                                                                                                : "no connection name";
    ...
    sync_manager::sync_manager(uint32_t req_span)
            : sync_known_lib_num(0), sync_last_requested_num(0), sync_next_expected_num(1), sync_req_span(req_span),
    ...
    bool sync_manager::is_active(const connection_ptr &c) {
        if (state == head_catchup && c) {
            ...
            return c->fork_head != block_id_type() &&
                   c->fork_head_num < chain_plug->chain().fork_db_pending_head_block_num();
        }
    ...
    bool sync_manager::sync_required() {
        fc_dlog(logger, "last req = ${req}, last recv = ${recv} known = ${known} our head = ${head}",
                ("req", sync_last_requested_num)("recv", sync_next_expected_num)("known", sync_known_lib_num)("head",
                                                                                                              chain_plug->chain().fork_db_pending_head_block_num()));
    ...
    bool sync_manager::verify_catchup(const connection_ptr &c, uint32_t num, const block_id_type &id) {
        ...
        if (req.req_blocks.mode == catch_up) {
            fc_ilog(logger,
                    "got a catch_up notice while in ${s}, fork head num = ${fhn} target LIB = ${lib} next_expected = ${ne}",
    ...
    void sync_manager::rejected_block(const connection_ptr &c, uint32_t blk_num) {
        if (state != in_sync) {
            fc_wlog(logger, "block ${bn} not accepted from ${p}, closing connection",
                    ("bn", blk_num)("p", c->peer_name()));
            sync_last_requested_num = 0;
            source.reset();
            my_impl->close(c);
            set_state(in_sync);
    ...
    void net_plugin_impl::connect(const connection_ptr &c) {
        ...
        c->strand.post([this, c, resolver{std::move(resolver)}]() {
            ...
            resolver->async_resolve(query, boost::asio::bind_executor(c->strand,
                                                                      [weak_conn, resolver, this](
                                                                              const boost::system::error_code &err,
                                                                              tcp::resolver::results_type endpoints) {
                                                                          app().post(priority::low,
                                                                                     [err, resolver, endpoints, weak_conn, this]() {
                                                                                         ...
                                                                                         if (!err) {
                                                                                             ...
                                                                                             fc_elog(logger,
                                                                                                     "Unable to resolve ${peer_addr}: ${error}",
                                                                                                     ("peer_addr", c->peer_name())(
                                                                                                             "error",
                                                                                                             err.message()));
                                                                                         }
                                                                                     });
    ...
    void net_plugin_impl::connect(const connection_ptr &c, const std::shared_ptr<tcp::resolver> &resolver,
                                  tcp::resolver::results_type endpoints) {
        if (c->no_retry != go_away_reason::no_reason) {
        ...
        boost::asio::async_connect(*c->socket, endpoints,
                                   boost::asio::bind_executor(c->strand,
                                                              [weak_conn, resolver, socket = c->socket, this](
                                                                      const boost::system::error_code &err,
                ...
                if (!ec) {
                    ...
                    if (rec) {
                        ...
                        if (num_clients != visitors) {
                            fc_ilog(logger, "checking max client, visitors = ${v} num clients ${n}",
                                    ("v", visitors)("n", num_clients));
                            num_clients = visitors;
                        }
                        if (from_addr < max_nodes_per_host &&
                            (max_client_count == 0 || num_clients < max_client_count)) {
                            ++num_clients;
    ...
    void net_plugin_impl::start_read_message(const connection_ptr &conn) {
        ...
        try {
            ...
            std::size_t minimum_read = conn->outstanding_read_bytes ? *conn->outstanding_read_bytes
                                                                    : message_header_size;
            ...
            boost::asio::async_read(*conn->socket,
                                    ...
                                    boost::asio::bind_executor(conn->strand,
                                                               [this, weak_conn, socket = conn->socket](
                                                                       boost::system::error_code ec,
                                                                       std::size_t bytes_transferred) {
                                                                   app().post(priority::medium,
                                                                              [this, weak_conn, socket, ec, bytes_transferred]() {
                                                                                  ...
                                                                                  try {
                                                                                      if (!ec) {
                                                                                          ...
                                                                                          conn->pending_message_buffer.advance_write_ptr(
                                                                                                  bytes_transferred);
                                                                                          while (conn->pending_message_buffer.bytes_to_read() >
                                                                                                 0) {
                                                                                              uint32_t bytes_in_buffer = conn->pending_message_buffer.bytes_to_read();

                                                                                              if (bytes_in_buffer <
                                                                                                  message_header_size) {
                                                                                                  conn->outstanding_read_bytes.emplace(
                                                                                                          message_header_size -
                                                                                                          bytes_in_buffer);
                                                                                                  break;
                                                                                              } else {
                                                                                                  ...
                                                                                                  conn->pending_message_buffer.peek(
                                                                                                          &message_length,
                                                                                                  ...
                                                                                                  if (message_length >
                                                                                                      ...
                                                                                                      boost::system::error_code ec;
                                                                                                      fc_elog(logger,
                                                                                                              "incoming message length unexpected (${i}), from ${p}",
                                                                                                              ("i", message_length)(
                                                                                                                      "p",
                                                                                      ...
                                                                                      fc_elog(logger,
                                                                                              "Undefined exception handling the read data from ${p}",
                                                                                              ("p", conn->peer_name()));
                                                                                      close(conn);
                                                                                  }
                                                                              });
    ...
    template<typename VerifierFunc>
    void net_plugin_impl::send_transaction_to_all(const std::shared_ptr<std::vector<char>> &send_buffer,
                                                  VerifierFunc verify) {
        for (auto &c : connections) {
    ...
    bool net_plugin_impl::is_valid(const handshake_message &msg) {
        ...
        if ((msg.sig != chain::signature_type() || msg.token != sha256()) &&
            (msg.token != fc::sha256::hash(msg.time))) {
            ...
            if (c->peer_addr.empty() || c->last_handshake_recv.node_id == fc::sha256()) {
                ...
                for (const auto &check : connections) {
                    ...
                    if (check->connected() && check->peer_name() == msg.p2p_address) {
                        ...
                        if (msg.time + c->last_handshake_sent.time <=
                            check->last_handshake_sent.time + check->last_handshake_recv.time)
                            continue;
                ...
                fc_dlog(logger, "skipping duplicate check, addr == ${pa}, id = ${ni}",
                        ("pa", c->peer_addr)("ni", c->last_handshake_recv.node_id));
            }
       ...
       */
        ...
        if (logger.is_enabled(fc::log_level::all))
            logger.log(FC_LOG_MESSAGE(all, "Clock offset is ${o}ns (${us}us)",
                                      ("o", c->offset)("us", c->offset / NsecPerUsec)));
        c->org = 0;
        c->rec = 0;
    }
    ...
    void net_plugin_impl::handle_message(const connection_ptr &c, const notice_message &msg) {
        ...
        switch (msg.known_blocks.mode) {
            ...
            default: {
                peer_elog(c, "bad notice_message : invalid known_blocks.mode ${m}",
    ...
    void net_plugin_impl::handle_message(const connection_ptr &c, const packed_transaction_ptr &trx) {
        ...
        chain_plug->accept_transaction(ptrx, [c, this, ptrx](
                const static_variant<fc::exception_ptr, transaction_trace_ptr> &result) {
            c->trx_in_progress_size -= calc_trx_size(ptrx->packed_trx);
    ...
    void net_plugin_impl::handle_message(const connection_ptr &c, const signed_block_ptr &msg) {
        ...
        try {
            ...
            peer_elog(c, "bad signed_block : ${m}", ("m", ex.what()));
            fc_elog(logger, "block_validate_exception accept block #${n} syncing from ${p}",
                    ("n", blk_num)("p", c->peer_name()));
            reason = validation;
        } catch (const assert_exception &ex) {
            peer_elog(c, "bad signed_block : ${m}", ("m", ex.what()));
            fc_elog(logger, "unable to accept block on assert exception ${n} from ${p}",
                    ("n", ex.to_string())("p", c->peer_name()));
        } catch (const fc::exception &ex) {
            peer_elog(c, "bad signed_block : ${m}", ("m", ex.what()));
            fc_elog(logger, "accept_block threw a non-assert exception ${x} from ${p}",
                    ("x", ex.to_string())("p", c->peer_name()));
        } catch (...) {
    ...
    void net_plugin_impl::transaction_ack(const std::pair<fc::exception_ptr, transaction_metadata_ptr> &results) {
        ...
        if (results.first) {
            fc_ilog(logger, "signaled NACK, trx-id = ${id} : ${why}",
                    ("id", id)("why", results.first->to_detail_string()));
    ...
    bool net_plugin_impl::authenticate_peer(const handshake_message &msg) const {
        ...
        if (time - msg_time > peer_authentication_interval) {
            fc_elog(logger, "Peer ${peer} sent a handshake with a timestamp skewed by more than ${time}.",
                    ("peer", msg.p2p_address)("time",
                                              "1 second")); // TODO Add to_variant for std::chrono::system_clock::duration
            return false;
        }
    ...
    void net_plugin::set_program_options(options_description & /*cli*/, options_description &cfg) {
        cfg.add_options()
                ("p2p-listen-endpoint", bpo::value<string>()->default_value("0.0.0.0:9876"),
                ...
                ("p2p-max-nodes-per-host", bpo::value<int>()->default_value(def_max_nodes_per_host),
                ...
                ("use-socket-read-watermark", bpo::value<bool>()->default_value(false),
                 "Enable expirimental socket read watermark optimization")
