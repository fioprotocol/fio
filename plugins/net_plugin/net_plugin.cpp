...
#define peer_elog(PEER, FORMAT, ...) \
  ...
  FC_MULTILINE_MACRO_END
    ...
    */
    ...
    indexed_by<
            ordered_unique < tag <
            by_id>, member<transaction_state, transaction_id_type, &transaction_state::id>, sha256_less>,
    ...
    */
    ...
    indexed_by<
            ordered_unique < tag <
            by_id>, member<eosio::peer_block_state, block_id_type, &eosio::peer_block_state::id>, sha256_less>,
    ...
    */
    ...
    void connection::do_queue_write(int priority) {
        ...
        boost::asio::async_write(*socket, bufs,
                                 boost::asio::bind_executor(strand,
                                                            [c, priority](boost::system::error_code ec, std::size_t w) {
                    ...
                    [weak_this, close_after_send](boost::system::error_code ec, std::size_t) {
                        ...
                        if (conn) {
                            if (close_after_send != no_reason) {
                                elog("sent a go away message: ${r}, closing connection to ${p}",
    ...
    bool sync_manager::sync_required() {
        fc_dlog(logger, "last req = ${req}, last recv = ${recv} known = ${known} our head = ${head}",
                ("req", sync_last_requested_num)("recv", sync_next_expected_num)("known", sync_known_lib_num)("head",
                                                                                                              chain_plug->chain().fork_db_head_block_num()));
        ...
        resolver->async_resolve(query, boost::asio::bind_executor(c->strand,
                                                                  [weak_conn, this](
                                                                      ...
                                                                      app().post(priority::low,
                                                                                 [err, endpoint_itr, weak_conn, this]() {
    ...
    void net_plugin_impl::connect(const connection_ptr &c, tcp::resolver::iterator endpoint_itr) {
        ...
        c->socket->async_connect(current_endpoint, boost::asio::bind_executor(c->strand,
                                                                              [weak_conn, endpoint_itr, this](
                                                                                      const boost::system::error_code &err) {
                                                                                  app().post(priority::low,
                                                                                             [weak_conn, endpoint_itr, this, err]() {
                                                                                                 auto c = weak_conn.lock();
                                                                                                 if (!c) return;
                                                                                                 if (!err &&
                                                                                                     c->socket->is_open()) {
                                    ...
                                    boost::asio::bind_executor(conn->strand,
                                                               [this, weak_conn](boost::system::error_code ec,
                                                                   ...
                                                                   app().post(priority::medium,
                                                                              [this, weak_conn, ec, bytes_transferred]() {
                                                                                  auto conn = weak_conn.lock();
                                                                                  if (!conn || !conn->socket ||
                                                                                      !conn->socket->is_open()) {
                                                                                      return;
    ...
    void net_plugin::plugin_initialize(const variables_map &options) {
        ...
        try {
            ...
            if (options.count("allowed-connection")) {
                const std::vector <std::string> allowed_remotes =
                        options["allowed-connection"].as < std::vector < std::string >> ();
            ...
            if (options.count("peer-private-key")) {
                const std::vector <std::string> key_id_to_wif_pair_strings =
                        options["peer-private-key"].as < std::vector < std::string >> ();
