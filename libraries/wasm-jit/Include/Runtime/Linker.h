...
namespace Runtime {
    ...
    struct Resolver {
        virtual bool resolve(const std::string &moduleName, const std::string &exportName, IR::ObjectType type,
                             ObjectInstance *&outObject) = 0;
    };

    // A resolver for intrinsics.
    struct IntrinsicResolver : Resolver {
        static RUNTIME_API IntrinsicResolver
        singleton;

        RUNTIME_API bool resolve(const std::string &moduleName, const std::string &exportName, IR::ObjectType type,
    ...
    struct ModuleExportResolver : Resolver {
        ModuleExportResolver(const IR::Module &inModule, ModuleInstance *inModuleInstance) : module(inModule),
                                                                                             moduleInstance(
                                                                                                     inModuleInstance) {}
    ...
    struct LazyResolver : Resolver {
        ...
        bool resolve(const std::string &moduleName, const std::string &exportName, IR::ObjectType type,
                     Runtime::ObjectInstance *&outObject) override {
    ...
    RUNTIME_API LinkResult

    linkModule(const IR::Module &module, Resolver &resolver);
}