...
namespace Runtime {
    ...
    inline const char *describeExceptionCause(Exception::Cause cause) {
        switch (cause) {
            case Exception::Cause::accessViolation:
                return "access violation";
            case Exception::Cause::stackOverflow:
                return "stack overflow";
            case Exception::Cause::integerDivideByZeroOrIntegerOverflow:
                return "integer divide by zero or signed integer overflow";
            case Exception::Cause::invalidFloatOperation:
                return "invalid floating point operation";
            case Exception::Cause::invokeSignatureMismatch:
                return "invoke signature mismatch";
            case Exception::Cause::reachedUnreachable:
                return "reached unreachable code";
            case Exception::Cause::indirectCallSignatureMismatch:
                return "call_indirect to function with wrong signature";
            case Exception::Cause::undefinedTableElement:
                return "undefined function table element";
            case Exception::Cause::calledAbort:
                return "called abort";
            case Exception::Cause::calledUnimplementedIntrinsic:
                return "called unimplemented intrinsic";
            case Exception::Cause::outOfMemory:
                return "out of memory";
            case Exception::Cause::invalidSegmentOffset:
                return "invalid segment offset";
            case Exception::Cause::misalignedAtomicMemoryAccess:
    ...
    inline FunctionInstance *asFunction(ObjectInstance *object) {
        WAVM_ASSERT_THROW(object && object->kind == IR::ObjectKind::function);
        return (FunctionInstance *) object;
    }

    inline TableInstance *asTable(ObjectInstance *object) {
        WAVM_ASSERT_THROW(object && object->kind == IR::ObjectKind::table);
        return (TableInstance *) object;
    }

    inline MemoryInstance *asMemory(ObjectInstance *object) {
        WAVM_ASSERT_THROW(object && object->kind == IR::ObjectKind::memory);
        return (MemoryInstance *) object;
    }

    inline GlobalInstance *asGlobal(ObjectInstance *object) {
        WAVM_ASSERT_THROW(object && object->kind == IR::ObjectKind::global);
        return (GlobalInstance *) object;
    }

    inline ModuleInstance *asModule(ObjectInstance *object) {
        WAVM_ASSERT_THROW(object && object->kind == IR::ObjectKind::module);
    ...
    template<>
    inline FunctionInstance *as<FunctionInstance>(ObjectInstance *object) { return asFunction(object); }

    template<>
    inline TableInstance *as<TableInstance>(ObjectInstance *object) { return asTable(object); }

    template<>
    inline MemoryInstance *as<MemoryInstance>(ObjectInstance *object) { return asMemory(object); }

    template<>
    inline GlobalInstance *as<GlobalInstance>(ObjectInstance *object) { return asGlobal(object); }

    template<>
    inline ModuleInstance *as<ModuleInstance>(ObjectInstance *object) { return asModule(object); }
    ...
    RUNTIME_API Result
    invokeFunction(FunctionInstance
    * function,
    const std::vector <Value> &parameters
    );

    // Returns the type of a FunctionInstance.
    RUNTIME_API const IR::FunctionType
    *
    getFunctionType(FunctionInstance
    * function);
    ...
    RUNTIME_API TableInstance
    *
    createTable(IR::TableType
    type);

    // Reads an element from the table. Assumes that index is in bounds.
    RUNTIME_API ObjectInstance
    *
    getTableElement(TableInstance
    * table,
    Uptr index
    );
    ...
    RUNTIME_API ObjectInstance
    *
    setTableElement(TableInstance
    * table,
    Uptr index, ObjectInstance
    * newValue);

    // Gets the current or maximum size of the table.
    RUNTIME_API Uptr
    getTableNumElements(TableInstance
    * table);
    RUNTIME_API Uptr
    getTableMaxElements(TableInstance
    * table);

    // Grows or shrinks the size of a table by numElements. Returns the previous size of the table.
    RUNTIME_API Iptr
    growTable(TableInstance
    * table,
    Uptr numElements
    );
    RUNTIME_API Iptr
    shrinkTable(TableInstance
    * table,
    Uptr numElements
    );
    ...
    RUNTIME_API MemoryInstance
    *
    createMemory(IR::MemoryType
    type);

    // Gets the base address of the memory's data.
    RUNTIME_API U8
    *
    getMemoryBaseAddress(MemoryInstance
    * memory);

    // Gets the current or maximum size of the memory in pages.
    RUNTIME_API Uptr
    getMemoryNumPages(MemoryInstance
    * memory);
    RUNTIME_API Uptr
    getMemoryMaxPages(MemoryInstance
    * memory);

    // Grows or shrinks the size of a memory by numPages. Returns the previous size of the memory.
    RUNTIME_API Iptr
    growMemory(MemoryInstance
    * memory,
    Uptr numPages
    );
    RUNTIME_API Iptr
    shrinkMemory(MemoryInstance
    * memory,
    Uptr numPages
    );
    ...
    RUNTIME_API U8
    *
    getValidatedMemoryOffsetRange(MemoryInstance
    * memory,
    Uptr offset, Uptr
    numBytes);

    // Validates an access to a single element of memory at the given offset, and returns a reference to it.
    template<typename Value> Value &memoryRef(MemoryInstance * memory, U32
    offset) {
    return *(Value*)
    getValidatedMemoryOffsetRange(memory, offset,
    sizeof(Value));
}
...
RUNTIME_API ModuleInstance
*

instantiateModule(const IR::Module &module, ImportBindings &&imports);
...
* moduleInstance);
RUNTIME_API uint64_t
getDefaultMemorySize(ModuleInstance
* moduleInstance);
RUNTIME_API TableInstance
*
getDefaultTable(ModuleInstance
* moduleInstance);
...
RUNTIME_API ObjectInstance
*
getInstanceExport(ModuleInstance
* moduleInstance,
