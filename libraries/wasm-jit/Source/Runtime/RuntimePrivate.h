...
namespace Runtime {
    ...
    struct FunctionInstance : GCObject {
        ...
        FunctionInstance(ModuleInstance *inModuleInstance, const FunctionType *inType, void *inNativeFunction = nullptr,
                         const char *inDebugName = "<unidentified FunctionInstance>")
                : GCObject(ObjectKind::function), moduleInstance(inModuleInstance), type(inType),
                  nativeFunction(inNativeFunction), debugName(inDebugName) {}
    };
    ...
    struct TableInstance : GCObject {
        ...
        TableInstance(const TableType &inType) : GCObject(ObjectKind::table), type(inType), baseAddress(nullptr),
                                                 endOffset(0), reservedBaseAddress(nullptr),
    ...
    struct MemoryInstance : GCObject {
        ...
        MemoryInstance(const MemoryType &inType) : GCObject(ObjectKind::memory), type(inType), baseAddress(nullptr),
                                                   numPages(0), endOffset(0), reservedBaseAddress(nullptr),
                                                   reservedNumPlatformPages(0) {}
    ...
    struct GlobalInstance : GCObject {
        ...
        GlobalInstance(GlobalType inType, UntaggedValue inValue) : GCObject(ObjectKind::global), type(inType),
                                                                   value(inValue), initialValue(value) {}
    };
    ...
    struct ModuleInstance : GCObject {
        ...
        ModuleInstance(
                ...
                : GCObject(ObjectKind::module), functions(inFunctionImports), tables(inTableImports),
                  memories(inMemoryImports), globals(inGlobalImports), defaultMemory(nullptr), defaultTable(nullptr),
                  jitModule(nullptr) {}
    ...
    U8 *allocateVirtualPagesAligned(Uptr numBytes, Uptr alignmentBytes, U8 *&outUnalignedBaseAddress,
    ...
    [[noreturn]] void
    handleHardwareTrap(Platform::HardwareTrapType trapType, Platform::CallStack &&trapCallStack, Uptr trapOperand);
