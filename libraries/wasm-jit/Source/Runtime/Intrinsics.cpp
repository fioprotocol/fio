    ...
    Runtime::ObjectInstance *find(const std::string &name, const IR::ObjectType &type) {
        ...
        switch (type.kind) {
            case IR::ObjectKind::function: {
                ...
                result =
                        keyValue == Singleton::get().functionMap.end() ? nullptr : asObject(keyValue->second->function);
                break;
            }
            case IR::ObjectKind::table: {
                auto keyValue = Singleton::get().tableMap.find(decoratedName);
                result = keyValue == Singleton::get().tableMap.end() ? nullptr : asObject(
                        (Runtime::TableInstance *) *keyValue->second);
                break;
            }
            case IR::ObjectKind::memory: {
                auto keyValue = Singleton::get().memoryMap.find(decoratedName);
                result = keyValue == Singleton::get().memoryMap.end() ? nullptr : asObject(
                        (Runtime::MemoryInstance *) *keyValue->second);
                break;
            }
