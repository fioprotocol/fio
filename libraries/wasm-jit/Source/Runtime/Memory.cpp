...
namespace Runtime {
    ...
    U8 *allocateVirtualPagesAligned(Uptr numBytes, Uptr alignmentBytes, U8 *&outUnalignedBaseAddress,
        ...
        if (!outUnalignedBaseAddress) {
            outUnalignedNumPlatformPages = 0;
            return nullptr;
    ...
    MemoryInstance *createMemory(MemoryType type) {
        ...
        const Uptr alignmentBytes = HAS_64BIT_ADDRESS_SPACE ? Uptr(4ull * 1024 * 1024 * 1024) : ((Uptr) 1
                << Platform::getPageSizeLog2());
        memory->baseAddress = allocateVirtualPagesAligned(memoryMaxBytes, alignmentBytes, memory->reservedBaseAddress,
    ...
    MemoryInstance::~MemoryInstance() {
        ...
        if (numPages > 0) {
            Platform::decommitVirtualPages(baseAddress, numPages << getPlatformPagesPerWebAssemblyPageLog2());
        }
    ...
    bool isAddressOwnedByMemory(U8 *address) {
        ...
        for (auto memory : memories) {
            ...
            U8 *endAddress =
                    memory->reservedBaseAddress + (memory->reservedNumPlatformPages << Platform::getPageSizeLog2());
    ...
    Iptr growMemory(MemoryInstance *memory, Uptr numNewPages) {
        ...
        if (numNewPages > 0) {
            ...
            if (numNewPages > memory->type.size.max ||
                memory->numPages > memory->type.size.max - numNewPages) { return -1; }
            ...
            memset(memory->baseAddress + (memory->numPages << IR::numBytesPerPageLog2), 0,
                   numNewPages << IR::numBytesPerPageLog2);
    ...
    Iptr shrinkMemory(MemoryInstance *memory, Uptr numPagesToShrink) {
        ...
        if (numPagesToShrink > 0) {
            ...
            if (numPagesToShrink > memory->numPages
                || memory->numPages - numPagesToShrink < memory->type.size.min) { return -1; }
