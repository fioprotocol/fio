...
namespace Runtime {
        ...
        switch (trapType) {
            case Platform::HardwareTrapType::accessViolation: {
                ...
                if (isAddressOwnedByTable(reinterpret_cast<U8 *>(trapOperand))) {
                    throw Exception{Exception::Cause::undefinedTableElement, callStackDescription};
                }
                    ...
                    Log::printf(Log::Category::error,
                                "Access violation outside of table or memory reserved addresses. Call stack:\n");
            ...
            case Platform::HardwareTrapType::intDivideByZeroOrOverflow:
                throw Exception{Exception::Cause::integerDivideByZeroOrIntegerOverflow, callStackDescription};
            default:
                Errors::unreachable();
        };
    }
    ...
    Result invokeFunction(FunctionInstance *function, const std::vector <Value> &parameters) {
        ...
        U64 *thunkMemory = (U64 *) alloca(
                (functionType->parameters.size() + getArity(functionType->ret)) * sizeof(U64));
