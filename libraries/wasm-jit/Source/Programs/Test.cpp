...
ModuleInstance *getModuleContextByInternalName(TestScriptState &state, const TextFileLocus &locus, const char *context,
    ...
    switch (action->type) {
        case ActionType::_module: {
            ...
            if (linkResult.success) {
                ...
                state.lastModuleInstance = instantiateModule(*moduleAction->module,
        ...
        case ActionType::invoke: {
            ...
            // Look up the module this invoke uses.
            ModuleInstance *moduleInstance = getModuleContextByInternalName(state, invokeAction->locus, "invoke",
                                                                            invokeAction->internalModuleName);
            ...
            if (!functionInstance) {
                testErrorf(state, invokeAction->locus, "couldn't find exported function with name: %s",
        ...
        case ActionType::get: {
            ...
            ModuleInstance *moduleInstance = getModuleContextByInternalName(state, getAction->locus, "get",
                                                                            getAction->internalModuleName);
            ...
            if (!globalInstance) {
                testErrorf(state, getAction->locus, "couldn't find exported global with name: %s",
    ...
    try {
        switch (command->type) {
            case Command::_register: {
                ...
                ModuleInstance *moduleInstance = getModuleContextByInternalName(state, registerCommand->locus,
                                                                                "register",
            ...
            case Command::assert_return_arithmetic_nan: {
                ...
                if (processAction(state, assertCommand->action.get(), actionResult)) {
                    ...
                    const bool isError =
                            actionResult.type == ResultType::f32 ? !isCanonicalOrArithmeticNaN(actionResult.f32,
                                                                                               requireCanonicalNaN)
...
std::cout << a << " : f64" << std::endl << b << " : f64" <<
std::endl;
}
DEFINE_INTRINSIC_FUNCTION2(spectest, spectest_print, print, none, i32, a, f32, b
) {
std::cout << a << " : i32" << std::endl << b << " : f32" <<
std::endl;
}
DEFINE_INTRINSIC_FUNCTION2(spectest, spectest_print, print, none, i64, a, f64, b
) {
std::cout << a << " : i64" << std::endl << b << " : f64" <<
std::endl;
}

DEFINE_INTRINSIC_GLOBAL(spectest, spectest_globalI32, global, i32,
false,666)
DEFINE_INTRINSIC_GLOBAL(spectest, spectest_globalI64, global, i64,
false,0)
DEFINE_INTRINSIC_GLOBAL(spectest, spectest_globalF32, global, f32,
false,0.0f)
DEFINE_INTRINSIC_GLOBAL(spectest, spectest_globalF64, global, f64,
false,0.0)

DEFINE_INTRINSIC_TABLE(spectest, spectest_table, table, TableType(TableElementType::anyfunc, false,
