...
struct RootResolver : Resolver {
    ...
    bool resolve(const std::string &moduleName, const std::string &exportName, ObjectType type,
        ...
        if (type.kind == ObjectKind::function) {
            ...
            outObject = asObject(Runtime::createGlobal(asGlobalType(type), Runtime::Value(asGlobalType(type).valueType,
...
int mainBody(const char *filename, const char *functionName, bool onlyCheck, char **args) {
    ...
    if (!functionName) {
        if (functionType->parameters.size() == 2) {
            ...
            std::cerr << "WebAssembly function requires " << functionType->parameters.size()
