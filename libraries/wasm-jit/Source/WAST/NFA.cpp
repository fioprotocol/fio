...
namespace NFA {
    ...
    std::vector <DFAState> convertToDFA(Builder *builder) {
        ...
        while (pendingDFAStates.size()) {
            ...
            if (!stateIndexToLocalStateIndexMap.count(currentTerminalState)) {
                stateIndexToLocalStateIndexMap.emplace(currentTerminalState,
                                                       (StateIndex) localStateIndexToStateIndexMap.size());
    ...
    struct StateTransitionsByChar {
        ...
        StateTransitionsByChar(U8 inC, Uptr inNumStates) : c(inC), nextStateByInitialState(nullptr),
    ...
    Machine::Machine(Builder *builder) {
        ...
        for (Uptr charIndex = 0; charIndex < 256; ++charIndex) {
            ...
            for (Uptr stateIndex = 0; stateIndex <
                                      dfaStates.size(); ++stateIndex) { stateTransitionsByChar[charIndex].nextStateByInitialState[stateIndex] = dfaStates[stateIndex].nextStateByChar[charIndex]; }
        }
            ...
            for (auto transitionPair : transitions) {
                if ((transitionPair.first & ~edgeDoesntConsumeInputFlag) != unmatchedCharacterTerminal) {
                    ...
                    std::string nextStateName = transitionPair.first < 0
                                                ? "terminal" +
                                                  std::to_string(-(transitionPair.first & ~edgeDoesntConsumeInputFlag))
                                                : "state" + std::to_string(transitionPair.first);
                    result += "start" + std::to_string(startIndex)
                              + " -> "
                              + nextStateName + "[label=\""
                              + (transitionPair.first < 0 && (transitionPair.first & edgeDoesntConsumeInputFlag) != 0
                                 ? "&epsilon; " : "")
