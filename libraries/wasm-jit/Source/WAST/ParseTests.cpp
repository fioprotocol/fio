...
static void parseTestScriptModule(ParseState &state, IR::Module &outModule, std::vector<UnresolvedError> &outErrors,
    ...
    if (moduleState.nextToken->type == t_quote || moduleState.nextToken->type == t_binary) {
        ...
        if (quoteToken->type == t_quote) {
            ...
            try {
                Serialization::MemoryInputStream wasmInputStream((const U8 *) moduleQuotedString.data(),
                                                                 moduleQuotedString.size());
                WASM::serialize(wasmInputStream, outModule);
            }
            catch (Serialization::FatalSerializationException exception) {
                parseErrorf(moduleState, quoteToken, "error deserializing binary module: %s",
        ...
        try {
            ...
            outErrors.insert(outErrors.end(), std::make_move_iterator(moduleState.errors.begin()),
    ...
    parseParenthesized(state, [&] {
        ...
        switch (state.nextToken->type) {
            ...
            case t_invoke: {
                ...
                result = new InvokeAction(std::move(locus), std::move(nameString), std::move(exportName),
                                          std::move(arguments));
                break;
            }
            case t_module: {
            ...
            switch (state.nextToken->type) {
                ...
                case t_assert_return: {
                    ...
                    Runtime::Result expectedReturn =
                            state.nextToken->type == t_leftParenthesis ? parseConstExpression(state)
                                                                       : Runtime::Result();
                ...
                case t_assert_trap: {
                    ...
                    if (!strcmp(expectedErrorMessage.c_str(),
                                "out of bounds memory access")) { expectedCause = Runtime::Exception::Cause::accessViolation; }
                    else if (!strcmp(expectedErrorMessage.c_str(),
                                     "call stack exhausted")) { expectedCause = Runtime::Exception::Cause::stackOverflow; }
                    else if (!strcmp(expectedErrorMessage.c_str(),
                                     "integer overflow")) { expectedCause = Runtime::Exception::Cause::integerDivideByZeroOrIntegerOverflow; }
                    else if (!strcmp(expectedErrorMessage.c_str(),
                                     "integer divide by zero")) { expectedCause = Runtime::Exception::Cause::integerDivideByZeroOrIntegerOverflow; }
                    else if (!strcmp(expectedErrorMessage.c_str(),
                                     "invalid conversion to integer")) { expectedCause = Runtime::Exception::Cause::invalidFloatOperation; }
                    else if (!strcmp(expectedErrorMessage.c_str(),
                                     "unaligned atomic")) { expectedCause = Runtime::Exception::Cause::misalignedAtomicMemoryAccess; }
                    else if (stringStartsWith(expectedErrorMessage.c_str(),
                                              "unreachable")) { expectedCause = Runtime::Exception::Cause::reachedUnreachable; }
                    else if (stringStartsWith(expectedErrorMessage.c_str(),
                                              "indirect call")) { expectedCause = Runtime::Exception::Cause::indirectCallSignatureMismatch; }
                    else if (stringStartsWith(expectedErrorMessage.c_str(),
                                              "undefined")) { expectedCause = Runtime::Exception::Cause::undefinedTableElement; }
                    else if (stringStartsWith(expectedErrorMessage.c_str(),
                                              "uninitialized")) { expectedCause = Runtime::Exception::Cause::undefinedTableElement; }
                ...
                case t_assert_malformed: {
                    ...
                    result = new AssertInvalidOrMalformedCommand(commandType, std::move(locus),
                                                                 moduleErrors.size() != 0);
                    break;
                };
                default:
...
namespace WAST {
    void
    parseTestCommands(const char *string, Uptr stringLength, std::vector<std::unique_ptr<Command>> &outTestCommands,
