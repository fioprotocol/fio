    ...
    if (!tryParseI64(state, outSizeConstraints.min)) {
        ...
        if (!tryParseI64(state, outSizeConstraints.max)) { outSizeConstraints.max = UINT64_MAX; }
            ...
            if (outSizeConstraints.max > maxMax) {
                parseErrorf(state, state.nextToken - 1, "maximum size exceeds limit (%u>%u)", outSizeConstraints.max,
                            maxMax);
                outSizeConstraints.max = maxMax;
            } else if (outSizeConstraints.max < outSizeConstraints.min) {
                parseErrorf(state, state.nextToken - 1, "maximum size is less than minimum size (%u<%u)",
...
static Uptr createImport(
        ...
        NameToIndexMap &nameToIndexMap, IndexSpace <Def, Type> &indexSpace,
    ...
    parseParenthesized(state, [&] {
        ...
        switch (importKindToken->type) {
            case t_func: {
                ...
                const UnresolvedFunctionType unresolvedFunctionType = parseFunctionTypeRefAndOrDecl(state,
                                                                                                    localNameToIndexMap,
                                                                                                    localDissassemblyNames);
                const Uptr importIndex = createImport(state, name, std::move(moduleName), std::move(exportName),
                                                      ...
                                                      state.disassemblyNames.functions,
                                                      {UINT32_MAX});
                ...
                state.postTypeCallbacks.push_back([unresolvedFunctionType, importIndex](ModuleParseState &state) {
                    state.module.functions.imports[importIndex].type = resolveFunctionType(state,
        ...
        state.postDeclarationCallbacks.push_back([=](ModuleParseState &state) {
            ...
            switch (exportKind) {
                case ObjectKind::function:
                    exportedObjectIndex = resolveRef(state, state.functionNameToIndexMap, state.module.functions.size(),
                                                     exportRef);
                    break;
                case ObjectKind::table:
                    exportedObjectIndex = resolveRef(state, state.tableNameToIndexMap, state.module.tables.size(),
                                                     exportRef);
                    break;
                case ObjectKind::memory:
                    exportedObjectIndex = resolveRef(state, state.memoryNameToIndexMap, state.module.memories.size(),
                                                     exportRef);
                    break;
                case ObjectKind::global:
                    exportedObjectIndex = resolveRef(state, state.globalNameToIndexMap, state.module.globals.size(),
                                                     exportRef);
                    break;
                default:
                    Errors::unreachable();
    ...
    state.postDeclarationCallbacks.push_back([=](ModuleParseState &state) {
        if (!state.module.memories.size()) {
            ...
            state.module.dataSegments[dataSegmentIndex].memoryIndex =
                    hasMemoryRef ? resolveRef(state, state.memoryNameToIndexMap, state.module.memories.size(),
                                              memoryRef) : 0;
        }
    });
}

static Uptr parseElemSegmentBody(ModuleParseState &state, Reference tableRef, InitializerExpression baseIndex,
    ...
    state.postDeclarationCallbacks.push_back(
            [tableRef, tableSegmentIndex, elementReferences, elemToken](ModuleParseState &state) {
                if (!state.module.tables.size()) {
                    parseErrorf(state, elemToken,
                    ...
                    tableSegment.tableIndex = tableRef ? resolveRef(state, state.tableNameToIndexMap,
                                                                    state.module.tables.size(), tableRef) : 0;
...
static void parseFunc(ModuleParseState &state) {
    parseObjectDefOrImport(state, state.functionNameToIndexMap, state.module.functions,
                           ...
                           [&](ModuleParseState &state) {
                               ...
                               const UnresolvedFunctionType unresolvedFunctionType = parseFunctionTypeRefAndOrDecl(
                                       state, localNameToIndexMap, localDisassemblyNames);

                               // Resolve the function import type after all type declarations have been parsed.
                               const Uptr importIndex = state.module.functions.imports.size();
                               state.postTypeCallbacks.push_back(
                                       [unresolvedFunctionType, importIndex](ModuleParseState &state) {
                                           state.module.functions.imports[importIndex].type = resolveFunctionType(state,
                                                                                                                  unresolvedFunctionType);
                                       });
...
static void parseTable(ModuleParseState &state) {
    parseObjectDefOrImport(state, state.tableNameToIndexMap, state.module.tables, state.disassemblyNames.tables,
                           t_table, ObjectKind::table,
            ...
            // Parse a table definition.
                           [](ModuleParseState &state, const Token *) {
                               ...
                               const bool hasSizeConstraints = tryParseSizeConstraints(state, UINT32_MAX,
                                                                                       sizeConstraints);
                               ...
                               if (!hasSizeConstraints) {
                                   parseParenthesized(state, [&] {
                                       ...
                                       const Uptr numElements = parseElemSegmentBody(state, Reference(U32(tableIndex)),
...
static void parseMemory(ModuleParseState &state) {
    parseObjectDefOrImport(state, state.memoryNameToIndexMap, state.module.memories, state.disassemblyNames.memories,
                           t_memory, ObjectKind::memory,
                           ...
                           [](ModuleParseState &state, const Token *) {
                               ...
                               if (!tryParseSizeConstraints(state, IR::maxMemoryPages, sizeConstraints)) {
                                   ...
                                   state.module.dataSegments.push_back(
                                           {state.module.memories.size(), InitializerExpression(I32(0)),
                                            std::move(dataVector)});
                               }
...
static void parseGlobal(ModuleParseState &state) {
    parseObjectDefOrImport(state, state.globalNameToIndexMap, state.module.globals, state.disassemblyNames.globals,
...
static void parseStart(ModuleParseState &state) {
    ...
    state.postDeclarationCallbacks.push_back([functionRef](ModuleParseState &state) {
        state.module.startFunctionIndex = resolveRef(state, state.functionNameToIndexMap, state.module.functions.size(),
                                                     functionRef);
    });
}
