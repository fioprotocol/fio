...
namespace {
    ...
    struct FunctionParseState : ParseState {
        ...
        FunctionParseState(const ModuleParseState &inModuleState, NameToIndexMap *inLocalNameToIndexMap,
                ...
                : ParseState(inModuleState.string, inModuleState.lineInfo, inModuleState.errors, firstBodyToken),
                  moduleState(inModuleState), functionDef(inFunctionDef), localNameToIndexMap(inLocalNameToIndexMap),
...
static void parseAndValidateRedundantBranchTargetName(ParseState &state, Name branchTargetName, const char *context,
...
namespace WAST {
    FunctionDef parseFunctionDef(ModuleParseState &state, const Token *funcToken) {
        ...
        const UnresolvedFunctionType unresolvedFunctionType = parseFunctionTypeRefAndOrDecl(state, *localNameToIndexMap,
                            ...
                            [functionIndex, functionDefIndex, firstBodyToken, localNameToIndexMap, localDisassemblyNames, functionTypeIndex]
                                ...
                                while (tryParseParenthesizedTagged(localParseState, t_local, [&] {
                                    ...
                                    if (tryParseName(localParseState, localName)) {
                                        bindName(localParseState, *localNameToIndexMap, localName,
                                        ...
                                        while (localParseState.nextToken->type != t_rightParenthesis) {
                                            ...
                                            functionDef.nonParameterLocalTypes.push_back(
                                                    parseValueType(localParseState));
                                        };
                                    }
                                }));
                                state.disassemblyNames.functions[functionIndex].locals = std::move(
                                        *localDisassemblyNames);
                                delete localDisassemblyNames;

                                // Parse the function's code.
                                FunctionParseState functionState(state, localNameToIndexMap, localParseState.nextToken,
                                                                 functionDef);
                                try {
