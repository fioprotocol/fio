    ...
    struct FunctionPrintContext {
        ...
        FunctionPrintContext(ModulePrintContext &inModuleContext, Uptr functionDefIndex)
                : moduleContext(inModuleContext), module(inModuleContext.module),
                  ...
                  functionType(inModuleContext.module.types[functionDef.type.index]), string(inModuleContext.string),
                  localNames(
                          inModuleContext.names.functions[module.functions.imports.size() + functionDefIndex].locals),
                  labelNameScope('$') {}
...
#undef VALIDATE_OP
    ...
    template<typename Type>
    void printImport(std::string &string, const Module &module, const Import<Type> &import, Uptr importIndex,
                     const char *name, const char *typeTag) {
        string += '\n';
    ...
    void ModulePrintContext::printModule() {
        ...
        for (Uptr importIndex = 0; importIndex < module.functions.imports.size(); ++importIndex) {
            printImport(string, module, module.functions.imports[importIndex], importIndex,
                        names.functions[importIndex].name.c_str(), "func");
        }
        ...
        for (Uptr importIndex = 0; importIndex < module.memories.imports.size(); ++importIndex) {
            printImport(string, module, module.memories.imports[importIndex], importIndex,
                        names.memories[importIndex].c_str(), "memory");
        }
        for (Uptr importIndex = 0; importIndex < module.globals.imports.size(); ++importIndex) {
            printImport(string, module, module.globals.imports[importIndex], importIndex,
                        names.globals[importIndex].c_str(), "global");
        }
