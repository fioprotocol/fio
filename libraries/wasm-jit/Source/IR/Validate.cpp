...
#define VALIDATE_INDEX(index, arraySize) VALIDATE_UNLESS("invalid index: ",index>=arraySize)
    ...
    ValueType validateGlobalIndex(const Module &module, Uptr globalIndex, bool mustBeMutable, bool mustBeImmutable,
    ...
    void validateInitializer(const Module &module, const InitializerExpression &expression, ValueType expectedType,
                             const char *context) {
        switch (expression.type) {
            ...
            case InitializerExpression::Type::get_global: {
                const ValueType globalValueType = validateGlobalIndex(module, expression.globalIndex, false, true, true,
    ...
    struct FunctionValidationContext {
        ...
        void br_table(BranchTableImm imm) {
            ...
            const ResultType defaultTargetArgumentType = getBranchTargetByDepth(
                    imm.defaultTargetDepth).branchArgumentType;
            ...
            for (Uptr targetIndex = 0; targetIndex < targetDepths.size(); ++targetIndex) {
                const ResultType targetArgumentType = getBranchTargetByDepth(
                        targetDepths[targetIndex]).branchArgumentType;
        ...
        void call_indirect(CallIndirectImm imm) {
            ...
            popAndValidateOperands("call_indirect arguments", calleeType->parameters.data(),
                                   (Uptr) calleeType->parameters.size());
            pushOperand(calleeType->ret);
        }
    ...
    void validateDefinitions(const Module &module) {
        ...
        if (module.startFunctionIndex != UINTPTR_MAX) {
            ...
            const FunctionType *startFunctionType = module.types[module.functions.getType(
                    module.startFunctionIndex).index];
