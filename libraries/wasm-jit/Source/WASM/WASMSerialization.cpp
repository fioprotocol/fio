...
namespace IR {
    ...
    void serialize(Stream &stream, TableSegment &tableSegment) {
        ...
        serializeArray(stream, tableSegment.indices,
                       [](Stream &stream, Uptr &functionIndex) { serializeVarUInt32(stream, functionIndex); });
    }
}
...
namespace WASM {
    ...
    void serialize(Stream &stream, ControlStructureImm &imm, const FunctionDef &) {
        ...
        if (Stream::isInput) {
            imm.resultType = encodedResultType == -64 ? ResultType::none : (ResultType) -encodedResultType;
        }
    }
    ...
    void serialize(InputStream &stream, BranchTableImm &imm, FunctionDef &functionDef) {
        ...
        serializeArray(stream, branchTable,
                       [](InputStream &stream, U32 &targetDepth) { serializeVarUInt32(stream, targetDepth); });
    ...
    void serialize(OutputStream &stream, BranchTableImm &imm, FunctionDef &functionDef) {
        ...
        serializeArray(stream, branchTable,
                       [](OutputStream &stream, U32 &targetDepth) { serializeVarUInt32(stream, targetDepth); });
        serializeVarUInt32(stream, imm.defaultTargetDepth);
    }

    template<typename Stream>
    void serialize(Stream &stream, LiteralImm<I32> &imm, const FunctionDef &) { serializeVarInt32(stream, imm.value); }

    template<typename Stream>
    void serialize(Stream &stream, LiteralImm<I64> &imm, const FunctionDef &) { serializeVarInt64(stream, imm.value); }
    ...
    void serialize(Stream &stream, LiteralImm<Value> &imm, const FunctionDef &) { serialize(stream, imm.value); }
