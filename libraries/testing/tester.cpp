...
namespace eosio {
    namespace testing {
        ...
        bool expect_assert_message(const fc::exception &ex, string expected) {
            BOOST_TEST_MESSAGE(
                    "LOG : " << "expected: " << expected << ", actual: " << ex.get_log().at(0).get_message());
        ...
        make_protocol_feature_set(const subjective_restriction_map &custom_subjective_restrictions) {
        ...
        void
        base_tester::init(controller::config config, protocol_feature_set &&pfs, const snapshot_reader_ptr &snapshot) {
            cfg = config;
        ...
        void base_tester::execute_setup_policy(const setup_policy policy) {
            ...
            auto schedule_preactivate_protocol_feature = [&]() {
                auto preactivate_feature_digest = pfm.get_builtin_digest(
        ...
        signed_block_ptr base_tester::push_block(signed_block_ptr b) {
            ...
            if (itr == last_produced_block.end() ||
                block_header::num_from_id(b->id()) > block_header::num_from_id(itr->second)) {
        ...
        void base_tester::_start_block(fc::time_point block_time) {
            ...
            if (itr != last_produced_block.end()) {
                last_produced_block_num = std::max(control->last_irreversible_block_num(),
                                                   block_header::num_from_id(itr->second));
            }
        ...
        void base_tester::produce_min_num_of_blocks_to_spend_time_wo_inactive_prod(
            ...
            while (elapsed_time < target_elapsed_time) {
                for (uint32_t i = 0; i < control->head_block_state()->active_schedule.producers.size(); i++) {
                    const auto time_to_skip = fc::milliseconds(
                            config::producer_repetitions * config::block_interval_ms);
        ...
        base_tester::create_account(account_name a, account_name creator, bool multisig, bool include_code) {
        ...
        transaction_trace_ptr base_tester::push_transaction(packed_transaction &trx,
            ...
            try {
                ...
                return r;
            } FC_RETHROW_EXCEPTIONS(warn, "transaction_header: ${header}",
                                    ("header", transaction_header(trx.get_transaction())))
        }
        ...
        transaction_trace_ptr base_tester::push_reqauth(account_name from, const vector<permission_level> &auths,
        ...
        transaction_trace_ptr
        base_tester::transfer(account_name from, account_name to, string amount, string memo, account_name currency) {
            return transfer(from, to, asset::from_string(amount), memo, currency);
        }


        transaction_trace_ptr
        base_tester::transfer(account_name from, account_name to, asset amount, string memo, account_name currency) {
            variant pretty_trx = fc::mutable_variant_object()
                    ("actions", fc::variants({
                                                     fc::mutable_variant_object()
                                                             ("account", currency)
        ...
        void base_tester::set_authority(account_name account,
                                        ...
                                        authority auth,
                                        permission_name parent) {
            set_authority(account, perm, auth, parent, {{account, config::owner_name}},
        ...
        void base_tester::delete_authority(account_name account,
                                           permission_name perm) {
            delete_authority(account, perm, {permission_level{account, config::owner_name}},
                             {get_private_key(account, "owner")});
        }
        ...
        void
        base_tester::set_code(account_name account, const vector<uint8_t> wasm, const private_key_type *signer) try {
            signed_transaction trx;
            trx.actions.emplace_back(vector<permission_level>{{account, config::active_name}},
                                     setcode{
                                             .account    = account,
        ...
        asset base_tester::get_currency_balance(const account_name &code,
            ...
            const auto *tbl = db.template find<table_id_object, by_code_scope_table>(
                    boost::make_tuple(code, account, N(accounts)));
            share_type result = 0;

            // the balance is implied to be 0 if either the table or row does not exist
            if (tbl) {
                const auto *obj = db.template find<key_value_object, by_scope_primary>(
                        boost::make_tuple(tbl->id, asset_symbol.to_symbol_code().value));
                if (obj) {
        ...
        vector<char>
        base_tester::get_row_by_account(uint64_t code, uint64_t scope, uint64_t table, const account_name &act) const {
            vector<char> data;
            const auto &db = control->db();
            ...
            std::function<void(const digest_type &)> add_digests =
                    [&pfm, &pfs, current_block_num, current_block_time, &preactivation_set, &preactivations, &add_digests]
                        ...
                        if (!pf.enabled || pf.earliest_allowed_activation_time > current_block_time
                            || pfm.is_builtin_activated(*pf.builtin_feature, current_block_num))
                            return;
