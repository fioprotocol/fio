...
namespace eosio {
    namespace chain {
        ...
        recovery_keys_type transaction_metadata::recover_keys(const chain_id_type &chain_id) {
            ...
            fc::microseconds cpu_usage = trn.get_signature_keys(chain_id, fc::time_point::maximum(),
                                                                recovered_pub_keys);
        ...
        signing_keys_future_type transaction_metadata::start_recover_keys(const transaction_metadata_ptr &mtrx,
            ...
            if (mtrx->signing_keys_future.valid() &&
                std::get<0>(mtrx->signing_keys_future.get()) == chain_id) // already created
