...
#undef assert
        ...
        public:
            context_free_api(apply_context &ctx)
                ...
                EOS_ASSERT(context.context_free, unaccessible_api,
        ...
        public:
            privileged_api(apply_context &ctx)
                ...
                EOS_ASSERT(context.privileged, unaccessible_api, "${code} does not have permission to call this API",
                           ("code", context.receiver));
            }
...
#define DB_SECONDARY_INDEX_METHODS_ARRAY(IDX) \
      ...
      (db_##IDX##_previous,       int(int, int))
        ...
        int, int, int))
        (eosio_assert_message_code, void(
        int, int, int64_t))
        ...
        void(int64_t),
        "require_auth", void(authorization_api::*)(const account_name&))
        (require_authorization,
        void(int64_t, int64_t
        ), "require_auth2", void(authorization_api::*)(const account_name&,
        const permission_name &permission
        ))
        (has_authorization,
        int(int64_t),
        "has_auth", bool(authorization_api::*)(const account_name&)const )
        (is_account,
        int(int64_t)
        )
        );
