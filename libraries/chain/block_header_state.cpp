    ...
    namespace chain {
        ...
        uint32_t block_header_state::calc_dpos_last_irreversible(account_name producer_of_next_block) const {
            ...
            for (auto &i : producer_to_last_implied_irb) {
                blocknums.push_back(
                        (i.first == producer_of_next_block) ? dpos_proposed_irreversible_blocknum : i.second);
            }
            ...
            while (i >= 0 && blocks_to_confirm) {
                ...
                if (result.confirm_count[i] == 0) {
                    ...
                    if (i == static_cast<int32_t>(result.confirm_count.size() - 1)) {
                        ...
                        memmove(&result.confirm_count[0], &result.confirm_count[i + 1],
                                result.confirm_count.size() - i - 1);
        ...
        block_header_state pending_block_header_state::_finish_next(
            ...
            EOS_ASSERT(h.schedule_version == active_schedule_version, producer_schedule_exception,
            ...
            if (h.new_producers) {
                ...
                EOS_ASSERT(h.new_producers->version == active_schedule.version + 1, producer_schedule_exception,
