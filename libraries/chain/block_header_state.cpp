...
namespace eosio {
    namespace chain {
        ...
        void block_header_state::set_new_producers(producer_schedule_type pending) {
            EOS_ASSERT(pending.version == active_schedule.version + 1, producer_schedule_exception,
        ...
        block_header_state block_header_state::next(const signed_block_header &h, bool skip_validate_signee) const {
            ...
            EOS_ASSERT(result.header.schedule_version == h.schedule_version, producer_schedule_exception,
                       "schedule_version in signed block is corrupted");

            auto itr = producer_to_last_produced.find(h.producer);
            if (itr != producer_to_last_produced.end()) {
                EOS_ASSERT(itr->second < result.block_num - h.confirmed, producer_double_confirm,
                           "producer ${prod} double-confirming known range", ("prod", h.producer));
            }
            ...
            if (h.new_producers) {
                EOS_ASSERT(!was_pending_promoted, producer_schedule_exception,
        ...
        void block_header_state::sign(const std::function<signature_type(const digest_type &)> &signer) {
            ...
            EOS_ASSERT(block_signing_key == fc::crypto::public_key(header.producer_signature, d), wrong_signing_key,
        ...
        void block_header_state::add_confirmation(const header_confirmation &conf) {
            for (const auto &c : confirmations)
                EOS_ASSERT(c.producer != conf.producer, producer_double_confirm,
