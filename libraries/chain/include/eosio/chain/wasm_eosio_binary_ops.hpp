...
// using for instructions with no fields
            ...
            inline void pack(instruction_stream *stream, branchtabletype field) {
                const char packed[] = {char(field.target_depth), char(field.target_depth >> 8),
                                       ...
                                       char(field.target_depth >> 48), char(field.target_depth >> 56),
                                       char(field.table_index), char(field.table_index >> 8),
                                       char(field.table_index >> 16), char(field.table_index >> 24),
                                       char(field.table_index >> 32), char(field.table_index >> 40),
                                       char(field.table_index >> 48), char(field.table_index >> 56)};
                stream->set(sizeof(packed), packed);
            }
            ...
            struct field_specific_params {
                ...
                static void pack(instruction_stream *stream, Field &f) {
                    return eosio::chain::wasm_ops::pack(stream, f);
                }
...
// This code is from wasm-jit/Include/IR/Operators.h
            ...
            struct EOSIO_OperatorDecoderStream {
                EOSIO_OperatorDecoderStream(const std::vector <U8> &codeBytes)
                        : start(codeBytes.data()), nextByte(codeBytes.data()),
