...
namespace eosio {
    namespace chain {
        ...
        using generated_transaction_multi_index = chainbase::shared_multi_index_container<
                ...
                indexed_by<
                        ordered_unique<tag<by_id>, BOOST_MULTI_INDEX_MEMBER(generated_transaction_object,
                                                                            generated_transaction_object::id_type,
                                                                            id) >,
                        ordered_unique<tag<by_trx_id>, BOOST_MULTI_INDEX_MEMBER(generated_transaction_object,
        ...
        public:
            generated_transaction(const generated_transaction_object &gto)
                    : trx_id(gto.trx_id), sender(gto.sender), sender_id(gto.sender_id), payer(gto.payer),
                      delay_until(gto.delay_until), expiration(gto.expiration), published(gto.published),
        ...
        namespace config {
            ...
            struct billable_size<generated_transaction_object> {
                ...
                static const uint64_t value = 96 + 4 +
                                              overhead; ///< 96 bytes for our constant size fields, 4 bytes for a varint for packed_trx size and 96 bytes of implementation overhead
            };
        }
    }
} // eosio::chain
