    ...
    namespace chain {
        namespace config {
            ...
            const static auto default_reversible_cache_size =
                    340 * 1024 * 1024ll;/// 1MB * 340 blocks based on 21 producer BFT delay
            const static auto default_reversible_guard_size =
                    2 * 1024 * 1024ll;/// 1MB * 340 blocks based on 21 producer BFT delay

            const static auto default_state_dir_name = "state";
            const static auto default_history_dir_name   = "history";
            const static auto default_history_index_dir_name = "history_index";
            ...
            const static auto default_history_index_size = 1*1024*1024*1024ll;
            const static auto default_history_index_guard_size = 128*1024*1024ll;
...
// Active permission of producers account requires greater than 2/3 of the producers to authorize
            const static uint64_t majority_producers_permission_name = N(
                    prod.major); // greater than 1/2 of producers needed to authorize
            const static uint64_t minority_producers_permission_name = N(
                    prod.minor); // greater than 1/3 of producers needed to authorize0
...
//const static uint32_t   default_max_gen_trx_size       = 64 * 1024;
            ...
            const static uint32_t default_max_block_net_usage =
                    1024 * 1024; /// at 500ms blocks and 200byte trx, this enables ~10,000 TPS burst
            ...
            const static uint32_t default_sig_cpu_bill_pct =
                    50 * percent_1; // billable percentage of signature recovery
...
// (unless net_usage_leeway is set to 0 and so are the net limits of all accounts that can help with resetting blockchain parameters).
            ...
            const static uint32_t overhead_per_account_ram_bytes = 2 *
                                                                   1024; ///< overhead accounts for basic account storage and pre-pays features like account recovery
            ...
            const static uint32_t hashing_checktime_block_size =
                    10 * 1024;  /// call checktime from hashing intrinsic once per this number of bytes

            const static eosio::chain::wasm_interface::vm_type default_wasm_runtime = eosio::chain::wasm_interface::vm_type::wabt;
            const static uint32_t default_abi_serializer_max_time_ms =
                    15 * 1000; ///< default deadline for abi serialization methods
...
//const static int blocks_per_round = producer_count * producer_repetitions;
            ...
            constexpr uint64_t billable_size_v =
                    ((billable_size<T>::value + billable_alignment - 1) / billable_alignment) * billable_alignment;
...
Number EOS_PERCENT_CEIL(Number value, uint32_t percentage) {
    return ((value * percentage) + eosio::chain::config::percent_100 - eosio::chain::config::percent_1) /
           eosio::chain::config::percent_100;
}
