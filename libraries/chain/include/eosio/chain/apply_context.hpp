        ...
        private:
            ...
            public:
                ...
                const table_id_object &get_table(table_id_object::id_type i) const {
                    ...
                    EOS_ASSERT(itr != _table_cache.end(), table_not_in_cache,
...
//               context.require_write_lock( table_obj.scope );
                ...
                int
                find_secondary(uint64_t code, uint64_t scope, uint64_t table, secondary_key_proxy_const_type secondary,
                               uint64_t &primary) {
                    ...
                    const auto *obj = context.db.find<ObjectType, by_secondary>(
                            secondary_key_helper_t::create_tuple(*tab, secondary));
                ...
                int
                lowerbound_secondary(uint64_t code, uint64_t scope, uint64_t table, secondary_key_proxy_type secondary,
                                     uint64_t &primary) {
                    auto tab = context.find_table(code, scope, table);
                    if (!tab) return -1;
                ...
                int
                upperbound_secondary(uint64_t code, uint64_t scope, uint64_t table, secondary_key_proxy_type secondary,
                                     uint64_t &primary) {
                    auto tab = context.find_table(code, scope, table);
                    if (!tab) return -1;
                ...
                int find_primary(uint64_t code, uint64_t scope, uint64_t table, secondary_key_proxy_type secondary,
                                 uint64_t primary) {
                    auto tab = context.find_table(code, scope, table);
                    if (!tab) return -1;
        ...
        public:
            apply_context(controller &con, transaction_context &trx_ctx, const action &a, uint32_t depth = 0)
                    : control(con), db(con.mutable_db()), trx_context(trx_ctx), act(a), receiver(act.account),
                      used_authorizations(act.authorization.size(), false), recurse_depth(depth), idx64(*this),
        ...
        public:
            ...
            int db_store_i64(uint64_t scope, uint64_t table, const account_name &payer, uint64_t id, const char *buffer,
                             size_t buffer_size);
