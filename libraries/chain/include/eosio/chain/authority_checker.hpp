    ...
    namespace chain {
        ...
        public:
            authority_checker(PermissionToAuthorityFunc permission_to_authority,
                    ...
                    : permission_to_authority(permission_to_authority), checktime(checktime),
                      provided_keys(provided_keys.begin(), provided_keys.end()),
                      provided_permissions(provided_permissions), _used_keys(provided_keys.size(), false),
                      provided_delay(provided_delay), recursion_depth_limit(recursion_depth_limit) {
        ...
        private:
            ...
            struct weight_tally_visitor {
                ...
                weight_tally_visitor(authority_checker &checker, permission_cache_type &cached_permissions,
                                     uint16_t recursion_depth)
                        : checker(checker), cached_permissions(cached_permissions), recursion_depth(recursion_depth) {}
                ...
                uint32_t operator()(const permission_level_weight &permission) {
                    auto status = authority_checker::permission_status_in_cache(cached_permissions,
                    ...
                    if (!status) {
                        if (recursion_depth < checker.recursion_depth_limit) {
                            ...
                            try {
                                ...
                                itr = res.first;
                                r = checker.satisfied(std::forward<decltype(auth)>(auth), cached_permissions,
                                                      recursion_depth + 1);
