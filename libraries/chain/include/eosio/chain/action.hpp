...
namespace eosio {
    namespace chain {
        ...
        struct action {
            ...
            template<typename T>
            T data_as() const {
                EOS_ASSERT(account == T::get_account(), action_type_exception,
                           "account is not consistent with action struct");
                EOS_ASSERT(name == T::get_name(), action_type_exception,
...
FC_REFLECT( eosio::chain::action, (account)(name)(authorization)
(data))
