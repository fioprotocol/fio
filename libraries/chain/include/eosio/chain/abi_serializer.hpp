 ...
 */
        struct abi_serializer {
            ...
            fc::variant binary_to_variant(const type_name &type, const bytes &binary,
                                          const fc::microseconds &max_serialization_time,
                                          bool short_path = false) const;

            fc::variant binary_to_variant(const type_name &type, fc::datastream<const char *> &binary,
                                          const fc::microseconds &max_serialization_time,
                                          bool short_path = false) const;

            bytes variant_to_binary(const type_name &type, const fc::variant &var,
                                    const fc::microseconds &max_serialization_time, bool short_path = false) const;

            void variant_to_binary(const type_name &type, const fc::variant &var, fc::datastream<char *> &ds,
                                   const fc::microseconds &max_serialization_time, bool short_path = false) const;
        ...
        private:
            ...
            fc::variant
            _binary_to_variant(const type_name &type, const bytes &binary, impl::binary_to_variant_context &ctx) const;
        ...
        namespace impl {
            ...
            struct abi_traverse_context_with_path : public abi_traverse_context {
                abi_traverse_context_with_path(const abi_serializer &abis, fc::microseconds max_serialization_time,
                ...
                abi_traverse_context_with_path(const abi_serializer &abis, fc::microseconds max_serialization_time,
                                               fc::time_point deadline, const type_name &type)
                        : abi_traverse_context(max_serialization_time, deadline), abis(abis) {
                    set_path_root(type);
                }

                abi_traverse_context_with_path(const abi_serializer &abis, const abi_traverse_context &ctx,
                                               const type_name &type)
            ...
            struct abi_to_variant {
                ...
                static void
                add(mutable_variant_object &mvo, const char *name, const M &v, Resolver, abi_traverse_context &ctx) {
                ...
                template<typename M, typename Resolver, require_abi_t<M> = 1>
                static void
                add(mutable_variant_object &mvo, const char *name, const std::shared_ptr <M> &v, Resolver resolver,
                    abi_traverse_context &ctx) {
                    auto h = ctx.enter_scope();
                    if (!v) return;
                ...
                static void add(mutable_variant_object &mvo, const char *name, const fc::static_variant<Args...> &v,
                                Resolver resolver, abi_traverse_context &ctx) {
                    auto h = ctx.enter_scope();
                    mutable_variant_object obj_mvo;
            ...
            public:
                abi_to_variant_visitor(mutable_variant_object &_mvo, const T &_val, Resolver _resolver,
                        ...
                        : _vo(_mvo), _val(_val), _resolver(_resolver), _ctx(_ctx) {}
            ...
            struct abi_from_variant {
                ...
                template<typename M, typename Resolver, require_abi_t<M> = 1>
                static void
                extract(const variant &v, std::shared_ptr <M> &o, Resolver resolver, abi_traverse_context &ctx) {
                    auto h = ctx.enter_scope();
                ...
                static void extract(const variant &v, action &act, Resolver resolver, abi_traverse_context &ctx) {
                    ...
                    EOS_ASSERT(valid_empty_data || !act.data.empty(), packed_transaction_type_exception,
                               "Failed to deserialize data for ${account}:${name}",
                               ("account", act.account)("name", act.name));
                }

                template<typename Resolver>
                static void
                extract(const variant &v, packed_transaction &ptrx, Resolver resolver, abi_traverse_context &ctx) {
                    auto h = ctx.enter_scope();
                    const variant_object &vo = v.get_object();
                    ...
                    if (vo.contains("packed_context_free_data") && vo["packed_context_free_data"].is_string() &&
                        !vo["packed_context_free_data"].as_string().empty()) {
                    ...
                    if (vo.contains("packed_trx") && vo["packed_trx"].is_string() &&
                        ...
                        if (use_packed_cfd) {
                            ptrx = packed_transaction(std::move(packed_trx), std::move(signatures),
                                                      std::move(packed_cfd), compression);
                        } else {
                            ptrx = packed_transaction(std::move(packed_trx), std::move(signatures), std::move(cfd),
                                                      compression);
                        }
                    } else {
                        EOS_ASSERT(vo.contains("transaction"), packed_transaction_type_exception,
                                   "Missing transaction");
                        if (use_packed_cfd) {
                            transaction trx;
                            extract(vo["transaction"], trx, resolver, ctx);
                            ptrx = packed_transaction(std::move(trx), std::move(signatures), std::move(packed_cfd),
                                                      compression);
                        } else {
            ...
            public:
                abi_from_variant_visitor(const variant_object &_vo, T &v, Resolver _resolver,
                                         abi_traverse_context &_ctx)
                        : reflector_init_visitor<T>(v), _vo(_vo), _resolver(_resolver), _ctx(_ctx) {}
        ...
        template<typename T, typename Resolver>
        void abi_serializer::to_variant(const T &o, variant &vo, Resolver resolver,
                                        const fc::microseconds &max_serialization_time) try {
            mutable_variant_object mvo;
        ...
        "Failed to serialize: ${type}", ("type", boost::core::demangle( typeid(o).
        ...
        template<typename T, typename Resolver>
        void abi_serializer::from_variant(const variant &v, T &o, Resolver resolver,
                                          const fc::microseconds &max_serialization_time) try {
            impl::abi_traverse_context ctx(max_serialization_time);
        ...
        FC_RETHROW_EXCEPTIONS(error,
        "Failed to deserialize variant", ("variant",v))
