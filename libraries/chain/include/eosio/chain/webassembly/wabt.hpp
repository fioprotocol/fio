 ...
 */
                ...
                inline array_ptr <T> array_ptr_impl(wabt_apply_instance_vars &vars, uint32_t ptr, uint32_t length) {
                    EOS_ASSERT(length < INT_MAX / (uint32_t)
                    sizeof(T), binaryen_exception, "length will overflow" );
                    return array_ptr<T>((T *) (vars.get_validated_pointer(ptr, length * (uint32_t)
                    sizeof(T))));
                }
 ...
 */
            ...
            using next_step = intrinsic_invoker_impl<Ret, std::tuple < Inputs...>>;
            using then_type = Ret(*)(wabt_apply_instance_vars &, array_ptr <T>, size_t, Inputs..., const TypedValues &,
                                     int);
 ...
 */
    ...
    static Ret translate_one(wabt_apply_instance_vars &vars, Inputs... rest, const TypedValues &args, int offset) {
        ...
        size_t length = args.at((uint32_t) offset).get_i32();
        static_assert(std::is_same < std::remove_const_t < T > ,
                      char > ::value && std::is_same < std::remove_const_t < U > , char > ::value,
                      "Currently only support array of (const)chars");
        return Then(vars, array_ptr_impl<T>(vars, ptr_t, length), array_ptr_impl<U>(vars, ptr_u, length), length, args,
                    (uint32_t) offset - 3);
    };
...
struct intrinsic_invoker_impl<Ret, std::tuple<const fc::time_point_sec &, Inputs...>> {
    using next_step = intrinsic_invoker_impl<Ret, std::tuple < Inputs...>>;
    using then_type = Ret (*)(wabt_apply_instance_vars &, const fc::time_point_sec &, Inputs..., const TypedValues &,
                              int);
