...

            {
                ...
                for (const auto &k : auth.keys) {
                    if (prev && !(prev->key < k.key))
                        return false; // TODO: require keys to be sorted in ascending order rather than descending (requires modifying many tests)
            ...
            }
            {
                ...
                for (const auto &a : auth.accounts) {
                    if (prev && (prev->permission >= a.permission))
                        return false; // TODO: require permission_levels to be sorted in ascending order rather than descending (requires modifying many tests)
