...
inline void key_to_account_v1(const std::string &pubkey, std::string &new_account) {
      ...
      std::string output = shorter.erase(acctcap,acctcap+1); //Get rid of the occasional 13th character
      new_account = output;
   }
...
inline void key_to_account(const std::string &pubkey, std::string &new_account) {
  ...
  auto res = shorten_key(pub_key_bytes);
  std::stringstream newaccount;
  newaccount << eosio::name(res);
  new_account = newaccount.str().erase(acctcap,acctcap+1);
  }
}
