...
namespace eosio {
    namespace chain {
        ...
        apply_context::apply_context(controller &con, transaction_context &trx_ctx, uint32_t action_ordinal,
                                     uint32_t depth)
                : control(con), db(con.mutable_db()), trx_context(trx_ctx), recurse_depth(depth),
                  first_receiver_action_ordinal(action_ordinal), action_ordinal(action_ordinal), idx64(*this),
                  idx128(*this), idx256(*this), idx_double(*this), idx_long_double(*this) {
        ...
        void apply_context::exec_one() {
            ...
            int32_t HF1_BLOCK_TIME = 1596456000; //Aug 3 2020 6am MST 12pm UTC, test net forking deadline
            ...
            try {
                try {
                    ...
                    if ((receiver_account->code_hash != digest_type()) &&
                        ...
                        try {
                            control.get_wasm_interface().apply(receiver_account->code_hash, receiver_account->vm_type,
                                                               receiver_account->vm_version, *this);
            ...
            //    * and, the *receiver_account object itself cannot be removed because accounts cannot be deleted in FIO.
 ...
 */
        void apply_context::execute_inline(action &&a) {
            ...
            bool disallow_send_to_self_bypass = control.is_builtin_activated(
            ...
            bool inherit_parent_authorizations = (!disallow_send_to_self_bypass && send_to_self &&
                                                  (receiver == act->account) && control.is_producing_block());
            ...
            for (const auto &auth : a.authorization) {
                ...
                if (inherit_parent_authorizations &&
                    std::find(act->authorization.begin(), act->authorization.end(), auth) != act->authorization.end()) {
            ...
            if (!control.skip_auth_check() && !privileged) {
                try {
                    ...
                    if (disallow_send_to_self_bypass || !send_to_self) {
                        ...
                        subjective_block_production_exception new_exception(
                                FC_LOG_MESSAGE(error, "Authorization failure with inline action sent to self"));
        ...
        apply_context::schedule_deferred_transaction(const uint128_t &sender_id, account_name payer, transaction &&trx,
            ...
            if (control.is_builtin_activated(builtin_protocol_feature_t::no_duplicate_deferred_id)) {
                ...
                if (exts.size() > 0) {
                    ...
                    EOS_ASSERT(context.sender_trx_id == trx_context.id,
            ...
            bool ram_restrictions_activated = control.is_builtin_activated(
            ...
            if (!control.skip_auth_check() &&
                ...
                try {
                    ...
                    if (disallow_send_to_self_bypass || !is_sending_only_to_self(receiver)) {
                        ...
                        subjective_block_production_exception new_exception(FC_LOG_MESSAGE(error,
            ...
            if (auto ptr = db.find<generated_transaction_object, by_sender_id>(
                ...
                bool replace_deferred_activated = control.is_builtin_activated(
        ...
        uint32_t apply_context::schedule_action(uint32_t ordinal_of_action_to_schedule, account_name receiver,
                                                bool context_free) {
            uint32_t scheduled_action_ordinal = trx_context.schedule_action(ordinal_of_action_to_schedule,
                                                                            receiver, context_free,
                                                                            action_ordinal,
                                                                            first_receiver_action_ordinal);
