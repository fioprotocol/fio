...
namespace eosio {
    namespace chain {
        ...
        void abi_serializer::set_abi(const abi_def &abi, const fc::microseconds &max_serialization_time) {
            ...
            EOS_ASSERT(starts_with(abi.version, "eosio::abi/1."), unsupported_abi_version_exception,
            ...
            for (const auto &td : abi.types) {
                ...
                EOS_ASSERT(!_is_type(td.new_type_name, ctx), duplicate_abi_type_def_exception, "type already exists",
                           ("new_type_name", td.new_type_name));
             ...
             *  an invalid ABI
             */
            EOS_ASSERT(typedefs.size() == abi.types.size(), duplicate_abi_type_def_exception,
                       "duplicate type definition detected");
            EOS_ASSERT(structs.size() == abi.structs.size(), duplicate_abi_struct_def_exception,
                       "duplicate struct definition detected");
            EOS_ASSERT(actions.size() == abi.actions.size(), duplicate_abi_action_def_exception,
                       "duplicate action definition detected");
            EOS_ASSERT(tables.size() == abi.tables.size(), duplicate_abi_table_def_exception,
                       "duplicate table definition detected");
            EOS_ASSERT(error_messages.size() == abi.error_messages.size(), duplicate_abi_err_msg_def_exception,
                       "duplicate error message definition detected");
            EOS_ASSERT(variants.size() == abi.variants.value.size(), duplicate_abi_variant_def_exception,
            ...
            for (const auto &t : typedefs) {
                try {
                    ...
                    while (itr != typedefs.end()) {
                        ctx.check_deadline();
                        EOS_ASSERT(find(types_seen.begin(), types_seen.end(), itr->second) == types_seen.end(),
                                   abi_circular_def_exception, "Circular reference in type ${type}", ("type", t.first));
                    ...
                    if (s.second.base != type_name()) {
                        ...
                        while (current.base != type_name()) {
                            ...
                            EOS_ASSERT(find(types_seen.begin(), types_seen.end(), base.name) == types_seen.end(),
                                       abi_circular_def_exception, "Circular reference in struct ${type}",
                                       ("type", s.second.name));
        ...
        void abi_serializer::_binary_to_variant(const type_name &type, fc::datastream<const char *> &stream,
            ...
            EOS_ASSERT(s_itr != structs.end(), invalid_type_inside_abi, "Unknown type ${type}",
            ...
            for (uint32_t i = 0; i < st.fields.size(); ++i) {
                ...
                obj(field.name,
                    _binary_to_variant(resolve_type(extension ? _remove_bin_extension(field.type) : field.type), stream,
                                       ctx));
            }
        }
        ...
        fc::variant abi_serializer::_binary_to_variant(const type_name &type, fc::datastream<const char *> &stream,
            ...
            if (btype != built_in_types.end()) {
                ...
                EOS_RETHROW_EXCEPTIONS(unpack_exception,
                                       "Unable to unpack ${class} type '${type}' while processing '${p}'",
                                       ("class", is_array(rtype) ? "array of built-in" : is_optional(rtype)
            ...
            if (is_array(rtype)) {
                ...
                EOS_RETHROW_EXCEPTIONS(unpack_exception, "Unable to unpack size of array '${p}'",
                ...
                for (decltype(size.value) i = 0; i < size; ++i) {
                    ...
                    EOS_ASSERT(!v.is_null(), unpack_exception, "Invalid packed array '${p}'",
                               ("p", ctx.get_path_string()));
                ...
                EOS_RETHROW_EXCEPTIONS(unpack_exception, "Unable to unpack presence flag of optional '${p}'",
                                       ("p", ctx.get_path_string()))
                ...
                if (v_itr != variants.end()) {
                    ...
                    EOS_RETHROW_EXCEPTIONS(unpack_exception, "Unable to unpack tag of variant '${p}'",
                                           ("p", ctx.get_path_string()))
                    ...
                    return vector < fc::variant >
                           {v_itr->second.types[select], _binary_to_variant(v_itr->second.types[select], stream, ctx)};
                }
            }
            ...
            EOS_ASSERT(mvo.size() > 0, unpack_exception, "Unable to unpack '${p}' from stream",
                       ("p", ctx.get_path_string()));
        ...
        fc::variant abi_serializer::binary_to_variant(const type_name &type, const bytes &binary,
        ...
        fc::variant abi_serializer::binary_to_variant(const type_name &type, fc::datastream<const char *> &binary,
                                                      const fc::microseconds &max_serialization_time,
                                                      bool short_path) const {
            impl::binary_to_variant_context ctx(*this, max_serialization_time, type);
            ctx.short_path = short_path;
        ...
        void
        abi_serializer::_variant_to_binary(const type_name &type, const fc::variant &var, fc::datastream<char *> &ds,
            ...
            try {
                ...
                if (btype != built_in_types.end()) {
                    ...
                    EOS_ASSERT(var.is_array() && var.size() == 2, pack_exception,
                               "Expected input to be an array of two items while processing variant '${p}'",
                               ("p", ctx.get_path_string()));
                    EOS_ASSERT(var[size_t(0)].is_string(), pack_exception,
                               "Encountered non-string as first item of input array while processing variant '${p}'",
                               ("p", ctx.get_path_string()));
                    ...
                    auto h1 = ctx.push_to_path(
                            impl::variant_path_item{.variant_itr = v_itr, .variant_ordinal = static_cast<uint32_t>(it -
                                                                                                                   v.types.begin())});
                    ...
                    if (var.is_object()) {
                        ...
                        EOS_THROW(pack_exception, "Unexpected input encountered while processing struct '${p}'",
                                  ("p", ctx.get_path_string()));
                    }
                } else {
        ...
        bytes abi_serializer::variant_to_binary(const type_name &type, const fc::variant &var,
                                                const fc::microseconds &max_serialization_time, bool short_path) const {
        ...
        void
        abi_serializer::variant_to_binary(const type_name &type, const fc::variant &var, fc::datastream<char *> &ds,
                                          const fc::microseconds &max_serialization_time, bool short_path) const {
            impl::variant_to_binary_context ctx(*this, max_serialization_time, type);
            ctx.short_path = short_path;
            _variant_to_binary(type, var, ds, ctx);
        ...
        namespace impl {
            ...
            fc::scoped_exit <std::function<void()>> abi_traverse_context::enter_scope() {
                ...
                EOS_ASSERT(recursion_depth < abi_serializer::max_recursion_depth, abi_recursion_depth_exception,
                           "recursive definition, max_recursion_depth ${r} ",
                           ("r", abi_serializer::max_recursion_depth));
            ...
            void abi_traverse_context_with_path::set_array_index_of_path_back(uint32_t i) {
                ...
                EOS_ASSERT(b.contains<array_index_path_item>(), abi_exception,
            ...
            fc::scoped_exit <std::function<void()>>
            variant_to_binary_context::disallow_extensions_unless(bool condition) {
