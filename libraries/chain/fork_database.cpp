    ...
    namespace chain {
                    ...
                    for (uint32_t i = 0, n = size.value; i < n; ++i) {
                        ...
                        for (const auto &receipt : s.block->transactions) {
                            if (receipt.trx.contains<packed_transaction>()) {
                                ...
                                s.trxs.push_back(std::make_shared<transaction_metadata>(
        ...
        void fork_database::advance_root(const block_id_type &id) {
            ...
            for (auto b = new_root; b;) {
                ...
                EOS_ASSERT(b || blocks_to_remove.back() == my->root->id, fork_database_exception,
            ...
            if (validate) {
                try {
                    ...
                    if (exts.size() > 0) {
                        ...
                        validator(n->header.timestamp, prev_bh->activated_protocol_features->protocol_features,
