    ...
    namespace chain {
        ...
        void variant_snapshot_writer::write_end_section() {
            snapshot["sections"].get_array().emplace_back(
                    fc::mutable_variant_object()("name", std::move(current_section_name))("rows",
                                                                                          std::move(current_rows)));
        }
        ...
        void variant_snapshot_reader::validate() const {
            ...
            for (const auto &section: section_array) {
                EOS_ASSERT(section.is_object(), snapshot_validation_exception,
        ...
        ostream_snapshot_writer::ostream_snapshot_writer(std::ostream &snapshot)
                : snapshot(snapshot), header_pos(snapshot.tellp()), section_pos(-1), row_count(0) {
            // write magic number
        ...
        void ostream_snapshot_writer::write_start_section(const std::string &section_name) {
            EOS_ASSERT(section_pos == std::streampos(-1), snapshot_exception,
        ...
        istream_snapshot_reader::istream_snapshot_reader(std::istream &snapshot)
                : snapshot(snapshot), header_pos(snapshot.tellg()), num_rows(0), cur_row(0) {
        ...
        void istream_snapshot_reader::validate() const {
            ...
            try {
                ...
                \
      snapshot_exception fce(FC_LOG_MESSAGE(warn, "Binary snapshot validation threw IO exception (${what})",
                                            ("what", e.what())));
                throw fce;
            }
        }
