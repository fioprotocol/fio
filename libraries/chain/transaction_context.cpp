            ...
            deadline_timer_verify() {
                ...
                for (int &interval : test_intervals) {
                    ...
                    for (unsigned int i = 0; i < loops; ++i) {
                        ...
                        int timer_slop =
                                std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() - interval;
                ...
                timer_overhead = bacc::mean(samples) +
                                 sqrt(bacc::variance(samples)) * 2; //target 95% of expirations before deadline
...
#define TIMER_STATS \
        ...
        void deadline_timer::start(fc::time_point tp) {
            ...
            if (x.count() <= deadline_timer_verification.timer_overhead)
                ...
                struct itimerval enable = {{0, 0},
                                           {0, (int) x.count() - deadline_timer_verification.timer_overhead}};
                expired = 0;
        ...
        void transaction_context::init(uint64_t initial_net_usage) {
            ...
            std::tie(account_net_limit, account_cpu_limit, greylisted_net,
                     greylisted_cpu) = max_bandwidth_billed_accounts_can_pay();
            ...
            auto new_eager_net_limit = std::min(eager_net_limit,
                                                static_cast<uint64_t>(account_net_limit + cfg.net_usage_leeway));
            ...
            eager_net_limit = (eager_net_limit / 8) *
                              8; // Round down to nearest multiple of word size (8 bytes) so check_net_usage can be efficient

            if (initial_net_usage > 0)
                add_net_usage(
                        initial_net_usage);  // Fail early if current net usage is already greater than the calculated limit
        ...
        void transaction_context::init_for_input_trx(uint64_t packed_trx_unprunable_size,
            ...
            if (cfg.context_free_discount_net_usage_den > 0
                ...
                discounted_size_for_pruned_data =
                        (discounted_size_for_pruned_data + cfg.context_free_discount_net_usage_den - 1)
        ...
        void transaction_context::checktime() const {
            ...
            if (BOOST_UNLIKELY(now > _deadline)) {
                ...
                if (explicit_billed_cpu_time || deadline_exception_code == deadline_exception::code_value) {
                    EOS_THROW(deadline_exception, "deadline exceeded",
                    ...
                    if (cpu_limit_due_to_greylist) {
                        EOS_THROW(greylist_cpu_usage_exceeded,
                                  ...
                                  ("now", now)("deadline", _deadline)("start", start)("billing_timer",
                                                                                      now - pseudo_start));
                    } else {
        ...
        uint32_t transaction_context::update_billed_cpu_time(fc::time_point now) {
            ...
            const auto &cfg = control.get_global_properties().configuration;
            billed_cpu_time_us = std::max((now - pseudo_start).count(),
                                          static_cast<int64_t>(cfg.min_transaction_cpu_usage));

            return static_cast<uint32_t>(billed_cpu_time_us);
        }

        std::tuple<int64_t, int64_t, bool, bool>
        transaction_context::max_bandwidth_billed_accounts_can_pay(bool force_elastic_limits) const {
            ...
            for (const auto &a : bill_to_accounts) {
                bool elastic =
                        force_elastic_limits || !(control.is_producing_block() && control.is_resource_greylisted(a));
        ...
        void transaction_context::dispatch_action(action_trace &trace, const action &a, account_name receiver,
                                                  bool context_free, uint32_t recurse_depth) {
        ...
        void transaction_context::schedule_transaction() {
            ...
            // whether that be by successfully executing, soft failure, hard failure, or expiration.
            if (trx.delay_sec.value ==
                0) { // Do not double bill. Only charge if we have not already charged for the delay.
                const auto &cfg = control.get_global_properties().configuration;
            ...
            const auto &cgto = control.mutable_db().create<generated_transaction_object>([&](auto &gto) {
                ...
                gto.expiration = gto.delay_until + fc::seconds(
