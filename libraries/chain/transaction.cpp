...
uint32_t packed_fio_transaction::get_unprunable_size()const {
  uint64_t size = config::fixed_net_overhead_of_packed_trx;
  size += packed_trx.size();
  EOS_ASSERT( size <= std::numeric_limits<uint32_t>::max(), tx_too_big, "packed_fio_transaction is too big" );
  return static_cast<uint32_t>(size);
}

uint32_t packed_fio_transaction::get_prunable_size()const {
  uint64_t size = fc::raw::pack_size(signatures);
  size += packed_context_free_data.size();
  EOS_ASSERT( size <= std::numeric_limits<uint32_t>::max(), tx_too_big, "packed_fio_transaction is too big" );
  return static_cast<uint32_t>(size);
...
digest_type packed_fio_transaction::packed_digest()const {
  digest_type::encoder prunable;
  fc::raw::pack( prunable, signatures );
  fc::raw::pack( prunable, packed_context_free_data );
...
bytes packed_fio_transaction::get_raw_transaction() const
{
  try {
     switch(compression) {
        case none:
...
vector<bytes> packed_fio_transaction::get_context_free_data()const
{
  try {
     switch(compression) {
        case none:
...
time_point_sec packed_fio_transaction::expiration()const
{
  local_unpack();
  return unpacked_trx->expiration;
}

transaction_id_type packed_fio_transaction::id()const
{
  local_unpack();
  return get_transaction().id();
}

transaction_id_type packed_fio_transaction::get_uncached_id()const
{
  const auto raw = get_raw_transaction();
  return fc::raw::unpack<transaction>( raw ).id();
}

void packed_fio_transaction::local_unpack()const
{
  if (!unpacked_trx) {
     try {
        switch(compression) {
...
transaction packed_fio_transaction::get_transaction()const
{
  local_unpack();
  return transaction(*unpacked_trx);
}

signed_transaction packed_fio_transaction::get_signed_transaction() const
{
  try {
     switch(compression) {
        case none:
...
void packed_fio_transaction::set_transaction(const transaction& t, packed_fio_transaction::compression_type _compression)
{
  try {
     switch(_compression) {
        case none:
...
void packed_fio_transaction::set_transaction(const transaction& t, const vector<bytes>& cfd, packed_fio_transaction::compression_type _compression)
{
  try {
     switch(_compression) {
        case none:
