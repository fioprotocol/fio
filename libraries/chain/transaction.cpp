...
namespace eosio {
    namespace chain {
        ...
        fc::microseconds transaction::get_signature_keys(const vector<signature_type> &signatures,
            ...
            try {
                ...
                for (const signature_type &sig : signatures) {
                    ...
                    EOS_ASSERT(sig_start < deadline, tx_cpu_usage_exceeded,
                               "transaction signature verification executed for too long",
                    ...
                    if (it == recovery_cache.get<by_sig>().end() || it->trx_id != tid) {
                        ...
                        lock.lock();
                        recovery_cache.emplace_back(cached_pub_key{tid, recov, sig,
                                                                   cpu_usage}); //could fail on dup signatures; not a problem
        ...
        signed_transaction::get_signature_keys(const chain_id_type &chain_id, fc::time_point deadline,
                                               flat_set<public_key_type> &recovered_pub_keys,
                                               bool allow_duplicate_keys) const {
            return transaction::get_signature_keys(signatures, chain_id, deadline, context_free_data,
                                                   recovered_pub_keys, allow_duplicate_keys);
        }
        ...
        struct read_limiter {
            ...
            size_t write(Sink &sink, const char *s, size_t count) {
                EOS_ASSERT(_total + count <= Limit, tx_decompression_error,
        ...
        packed_transaction::packed_transaction(bytes &&packed_txn, vector<signature_type> &&sigs, bytes &&packed_cfd,
                                               compression_type _compression)
                : signatures(std::move(sigs)), compression(_compression),
                  packed_context_free_data(std::move(packed_cfd)), packed_trx(std::move(packed_txn)) {
        ...
        packed_transaction::packed_transaction(bytes &&packed_txn, vector<signature_type> &&sigs, vector<bytes> &&cfd,
                                               compression_type _compression)
        ...
        packed_transaction::packed_transaction(transaction &&t, vector<signature_type> &&sigs, bytes &&packed_cfd,
                                               compression_type _compression)
                : signatures(std::move(sigs)), compression(_compression),
                  packed_context_free_data(std::move(packed_cfd)), unpacked_trx(std::move(t), signatures, {}) {
        ...
        void packed_transaction::reflector_init() {
            ...
            EOS_ASSERT(unpacked_trx.expiration == time_point_sec(), tx_decompression_error,
        ...
        void packed_transaction::local_unpack_context_free_data() {
            try {
                EOS_ASSERT(unpacked_trx.context_free_data.empty(), tx_decompression_error,
                           "packed_transaction.context_free_data not empty");
                switch (compression) {
                    case none:
