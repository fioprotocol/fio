    ...
    namespace chain {
        ...
        void block_log::open(const fc::path &data_dir) {
            ...
            if (log_size) {
                ...
                EOS_ASSERT(my->version >= min_supported_version && my->version <= max_supported_version,
                           ...
                           ("version", my->version)("min", block_log::min_supported_version)("max",
                ...
                if (my->version > 1) {
                    ...
                    EOS_ASSERT(my->first_block_num > 0, block_log_exception,
        ...
        uint64_t block_log::append(const signed_block_ptr &b) {
            try {
                EOS_ASSERT(my->genesis_written_to_block_log, block_log_append_fail,
        ...
        fc::path block_log::repair_log(const fc::path &data_dir, uint32_t truncate_at_block) {
            ...
            EOS_ASSERT(version >= min_supported_version && version <= max_supported_version,
                       block_log_unsupported_version,
                       "Unsupported version of block log. Block log version is ${version} while code supports version(s) [${min},${max}]",
                       ("version", version)("min", block_log::min_supported_version)("max",
                                                                                     block_log::max_supported_version));
            ...
            if (version != 1) {
                ...
                EOS_ASSERT(actual_totem == expected_totem, block_log_exception,
                           ...
                           ("e", fc::to_hex((char *) &expected_totem, sizeof(expected_totem)))("a", fc::to_hex(
                                   (char *) &actual_totem, sizeof(actual_totem))));
            ...
            while (pos < end_pos) {
                ...
                if (previous != tmp.previous) {
                    elog("Block ${num} (${id}) does not link back to previous block. "
                         ...
                         ("num", block_header::num_from_id(id))("id", id)("expected", previous)("actual",
                                                                                                tmp.previous));
                }
                previous = id;
