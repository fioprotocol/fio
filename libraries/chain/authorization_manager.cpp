    ...
    namespace chain {
        ...
        namespace detail {
            ...
            struct snapshot_row_traits<permission_object> {
                ...
                static snapshot_permission_object
                to_snapshot_row(const permission_object &value, const chainbase::database &db) {
                ...
                static void
                from_snapshot_row(snapshot_permission_object &&row, permission_object &value, chainbase::database &db) {
                    value.name = row.name;
                    ...
                    value.parent = 0;
                    if (value.id == 0) {
                        EOS_ASSERT(row.parent == permission_name(), snapshot_exception,
                                   "Unexpected parent name on reserved permission 0");
                        EOS_ASSERT(row.name == permission_name(), snapshot_exception,
                                   "Unexpected permission name on reserved permission 0");
                        EOS_ASSERT(row.owner == name(), snapshot_exception,
                                   "Unexpected owner name on reserved permission 0");
                        EOS_ASSERT(row.auth.accounts.size() == 0, snapshot_exception,
                                   "Unexpected auth accounts on reserved permission 0");
                        EOS_ASSERT(row.auth.keys.size() == 0, snapshot_exception,
                                   "Unexpected auth keys on reserved permission 0");
                        EOS_ASSERT(row.auth.waits.size() == 0, snapshot_exception,
                                   "Unexpected auth waits on reserved permission 0");
                        EOS_ASSERT(row.auth.threshold == 0, snapshot_exception,
                                   "Unexpected auth threshold on reserved permission 0");
                        EOS_ASSERT(row.last_updated == time_point(), snapshot_exception,
                                   "Unexpected auth last updated on reserved permission 0");
                        value.parent = 0;
                    } else if (row.parent != permission_name()) {
                        const auto &parent = db.get<permission_object, by_owner>(
                                boost::make_tuple(row.owner, row.parent));
        ...
        const permission_object *authorization_manager::find_permission(const permission_level &level) const {
            ...
            } EOS_RETHROW_EXCEPTIONS(chain::permission_query_exception, "Failed to retrieve permission: ${level}",
                                     ("level", level))
        }
        ...
        optional<permission_name> authorization_manager::lookup_minimum_permission(account_name authorizer_account,
            ...
            try {
                optional<permission_name> linked_permission = lookup_linked_permission(authorizer_account, scope,
                                                                                       act_name);
            ...
            for (const auto &act : actions) {
                ...
                if (act.account == config::system_account_name) {
                    ...
                    if (act.name == updateauth::get_name()) {
                        ...
                        delay = std::max(delay, check_canceldelay_authorization(act.data_as<canceldelay>(),
                                                                                act.authorization));
                    } else {
                        special_case = false;
                    }
                ...
                for (const auto &declared_auth : act.authorization) {
                    ...
                    if (!special_case) {
                        auto min_permission_name = lookup_minimum_permission(declared_auth.actor, act.account,
                                                                             act.name);
                        if (min_permission_name) { // since special cases were already handled, it should only be false if the permission is eosio.any
                            ...
                            EOS_ASSERT(get_permission(declared_auth).satisfies(min_permission,
                                       ...
                                       ("auth", declared_auth)("min", permission_level{min_permission.owner,
                                                                                       min_permission.name}));
                        }
                    }
        ...
        authorization_manager::check_authorization(account_name account,
            ...
            auto checker = make_auth_checker([&](const permission_level &p) { return get_permission(p).auth; },
                                             ...
                                             provided_keys,
                                             provided_permissions,
                                             (provided_delay >= delay_max_limit) ? fc::microseconds::maximum()
                                                                                 : provided_delay,
                                             checktime
            );
