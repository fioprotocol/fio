...
namespace eosio {
    namespace chain {
        ...
        struct pending_state {
            ...
            bool is_protocol_feature_activated(const digest_type &feature_digest) const {
                if (_block_stage.contains<building_block>()) {
                    ...
                    auto end = bb._new_protocol_feature_activations.begin() +
        ...
        struct controller_impl {
            ...
            unordered_map<builtin_protocol_feature_t, std::function<void(
                    controller_impl &)>, enum_hash<builtin_protocol_feature_t> > protocol_feature_activation_handlers;
    ...
    */
            ...
            void pop_block() {
                ...
                if (!prev) {
                    EOS_ASSERT(fork_db.root()->id == head->header.previous, block_validate_exception,
...
*/
    ...
    */
            ...
            void log_irreversible() {
                ...
                if (log_head) {
                    EOS_ASSERT(root_id == log_head->id(), fork_database_exception,
                               "fork database root does not match block log head");
                } else {
            ...
            void init(std::function<bool()> shutdown, const snapshot_reader_ptr &snapshot) {
                ...
                if (read_mode == db_read_mode::IRREVERSIBLE) {
                    ...
                    if (ritr != rbi.rend()
                        ...
                        EOS_ASSERT(rbitr != rbi.end(), reversible_blocks_exception,
                                   "pending head block not found in reversible blocks");
      ...
      FC_ASSERT(db.revision() == self.head_block_num(),
            ...
            transaction_trace_ptr apply_onerror(const generated_transaction &gtrx,
                ...
                if (self.is_builtin_activated(builtin_protocol_feature_t::no_duplicate_deferred_id)) {
                    ...
                    etrx.expiration = self.pending_block_time() + fc::microseconds(
                            999'999); // Round up to nearest second to avoid appearing expired
                ...
                try {
                    ...
                    trx_context.execute_action(
                            trx_context.schedule_action(etrx.actions.back(), gtrx.sender, false, 0, 0), 0);
            ...
            push_scheduled_transaction(const generated_transaction_object &gto, fc::time_point deadline,
                ...
                try {
                    ...
                    try {
                        ...
                        fc::move_append(pending->_block_stage.get<building_block>()._actions,
                                        move(trx_context.executed));
    ...
    */
            ...
            void start_block(block_timestamp_type when,
                ...
                if (read_mode == db_read_mode::SPECULATIVE ||
                    ...
                    if (gpo.proposed_schedule_block_num.valid() &&
                        ...
                        (*gpo.proposed_schedule_block_num <= pbhs.dpos_irreversible_blocknum) &&
                        // ... that has now become irreversible ...
                        pbhs.prev_pending_schedule.schedule.producers.size() ==
                        0 // ... and there was room for a new pending schedule prior to any possible promotion
                            ) {
            ...
            void finalize_block() {
                ...
                EOS_ASSERT(pending->_block_stage.contains<building_block>(), block_validate_exception,
                           "already called finalize_block");
                ...
                for (auto itr = new_protocol_features.begin(); itr != new_protocol_features.end(); ++itr) {
                    ...
                    switch (status) {
                        ...
                        case protocol_feature_set::recognized_t::too_early:
                            EOS_THROW(protocol_feature_exception,
                                      "${timestamp} is too early for the earliest allowed activation time of the protocol feature with digest '${digest}'",
                                      ("digest", f)("timestamp", timestamp));
                            break;
                    ...
                    EOS_ASSERT(currently_activated_protocol_features.find(f) ==
                               currently_activated_protocol_features.end(),
                    ...
                    auto dependency_checker = [&currently_activated_protocol_features, &new_protocol_features, &itr]
                        ...
                        if (currently_activated_protocol_features.find(f) !=
                            currently_activated_protocol_features.end())
                            return true;
                        ...
                        for (const auto &receipt : b->transactions) {
                            if (receipt.trx.contains<packed_transaction>()) {
                                ...
                                if (!self.skip_auth_check()) {
                                    transaction_metadata::start_recover_keys(mtrx, thread_pool.get_executor(), chain_id,
   ...
   */
            ...
            switch (status) {
                case protocol_feature_set::recognized_t::unrecognized:
                    if (is_producing_block()) {
                        EOS_THROW(subjective_block_production_exception,
                                  "protocol feature with digest '${digest}' is unrecognized",
                                  ("digest", feature_digest));
                    } else {
                ...
                case protocol_feature_set::recognized_t::too_early:
                    if (is_producing_block()) {
                        EOS_THROW(subjective_block_production_exception,
                                  "${timestamp} is too early for the earliest allowed activation time of the protocol feature with digest '${digest}'",
                                  ("digest", feature_digest)("timestamp", cur_time));
                    } else {
                        EOS_THROW(protocol_feature_bad_block_exception,
                                  "${timestamp} is too early for the earliest allowed activation time of the protocol feature with digest '${digest}'",
                                  ("digest", feature_digest)("timestamp", cur_time));
        ...
        int64_t controller::set_proposed_producers(vector<producer_key> producers) {
            ...
            if (producers.size() == 0 &&
                is_builtin_activated(builtin_protocol_feature_t::disallow_empty_producer_schedule)) {
                return -1;
            }
        ...
        bool controller::is_ram_billing_in_notify_allowed() const {
            return my->conf.disable_all_subjective_mitigations || !is_producing_block() ||
                   my->conf.allow_ram_billing_in_notify;
        }
