...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, walletMgr=None, enableMongo=False, mongoHost="localhost",
                 mongoPort=27017, mongoDb="EOStest"):
        self.host = host
        self.port = port
        self.pid = pid
        self.cmd = cmd
        if Utils.Debug: Utils.Print(
            "new Node host=%s, port=%s, pid=%s, cmd=%s" % (self.host, self.port, self.pid, self.cmd))
    ...
    class Context:
        ...
        def add(self, newKey):
            ...
            assert isinstance(subSection, dict), print(
                "ERROR: Calling \"add\" method when context is not a dictionary. %s in %s" % (
                self.__contextDesc(), self.__json()))
            assert newKey in subSection, print(
                "ERROR: %s%s does not contain key \"%s\". %s" % (self.__contextDesc(), key, self.__json()))
        ...
        def index(self, i):
            ...
            assert isinstance(cur, list), print(
                "ERROR: Calling \"index\" method when context is not a list.  %s in %s" % (
                self.__contextDesc(), self.__json()))
            listLen = len(cur)
    ...
    def getBlockByIdMdb(self, blockId, silentErrors=False):
        ...
        try:
            ...
            if not silentErrors:
                ...
                msg = ex.output.decode("utf-8")
                Utils.Print(
                    "ERROR: Exception during db get block by id.  cmd Duration: %.3f sec. %s" % (end - start, msg))
            return None
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False,
                                stakeNet=100, stakeCPU=100, buyRAM=10000, exitOnError=False):
        ...
        if stakedDeposit > 0:
            self.waitForTransInBlock(
                transId)  # seems like account creation needs to be finalized before transfer can happen
            trans = self.transferFunds(creatorAccount, account, Node.currencyIntToStr(stakedDeposit, CORE_SYMBOL),
                                       "init")
            transId = Node.getTransId(trans)
    ...
    def getAccountCodeHash(self, account):
        ...
        try:
            ...
            msg = ex.output.decode("utf-8")
            Utils.Print(
                "ERROR: Exception during code hash retrieval.  cmd Duration: %.3f sec.  %s" % (end - start, msg))
            return None
    ...
    def delegatebw(self, fromAccount, netQuantity, cpuQuantity, toAccount=None, transferTo=False,
                   waitForTransBlock=False, exitOnError=False):
    ...
    def undelegatebw(self, fromAccount, netQuantity, cpuQuantity, toAccount=None, waitForTransBlock=False,
                     exitOnError=False):
        if toAccount is None:
            toAccount = fromAccount
    ...
    def killNodeOnProducer(self, producer, whereInSequence, blockType=BlockType.head, silentErrors=True,
                           exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        ...
        return self.processCurlCmd("test_control", "kill_node_on_producer", payload, silentErrors=silentErrors,
                                   exitOnError=exitOnError, exitMsg=exitMsg, returnType=returnType)

    def processCurlCmd(self, resource, command, payload, silentErrors=True, exitOnError=False, exitMsg=None,
                       returnType=ReturnType.json):
    ...
    def txnGenCreateTestAccounts(self, genAccount, genKey, silentErrors=True, exitOnError=False, exitMsg=None,
                                 returnType=ReturnType.json):
        ...
        return self.processCurlCmd("txn_test_gen", "create_test_accounts", payload, silentErrors=silentErrors,
                                   exitOnError=exitOnError, exitMsg=exitMsg, returnType=returnType)

    def txnGenStart(self, salt, period, batchSize, silentErrors=True, exitOnError=False, exitMsg=None,
                    returnType=ReturnType.json):
        ...
        return self.processCurlCmd("txn_test_gen", "start_generation", payload, silentErrors=silentErrors,
                                   exitOnError=exitOnError, exitMsg=exitMsg, returnType=returnType)
    ...
    def verifyAlive(self, silent=False):
        if not silent and Utils.Debug: Utils.Print(
            "Checking if node(pid=%s) is alive(killed=%s): %s" % (self.pid, self.killed, self.cmd))
    ...
    def getNextCleanProductionCycle(self, trans):
        ...
        Utils.Print(
            "Searching for clean production cycle blockNum=%s ibn=%s  transId=%s  promoted bn=%s  ibn for schedule active=%s" % (
    ...
    def relaunch(self, nodeId, chainArg=None, newChain=False, timeout=Utils.systemWaitTimeout, addOrSwapFlags=None,
                 cachePopen=False, nodeosPath=None):
    ...
    def launchCmd(self, cmd, nodeId, cachePopen=False):
        ...
        with open(stdoutFile, 'w') as sout, open(stderrFile, 'w') as serr:
            ...
            self.pid = popen.pid
            if Utils.Debug: Utils.Print(
                "start Node host=%s, port=%s, pid=%s, cmd=%s" % (self.host, self.port, self.pid, self.cmd))
    ...
    def trackCmdTransaction(self, trans, ignoreNonTrans=False):
        ...
        if Utils.Debug:
            ...
            if transId in self.transCache.keys():
                replaceMsg = "replacing previous trans=\n%s" % json.dumps(self.transCache[transId], indent=2,
                                                                          sort_keys=True)
            else:
                replaceMsg = ""
