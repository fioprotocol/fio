...
args = TestHelper.parse_args(
    {"--host", "--port", "--prod-count", "--defproducera_prvt_key", "--defproducerb_prvt_key", "--mongodb"
...
cluster = Cluster(host=server, port=port, walletd=True, enableMongo=enableMongo,
                  defproduceraPrvtKey=defproduceraPrvtKey, defproducerbPrvtKey=defproducerbPrvtKey)
...
try:
    ...
    if localTest and not dontLaunch:
        ...
        if cluster.launch(prodCount=prodCount, onlyBios=onlyBios, dontBootstrap=dontBootstrap,
    ...
    transId = node.createInitializeAccount(testeraAccount, cluster.defproduceraAccount, stakedDeposit=0,
                                           waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.defproduceraAccount.name))
    transId = node.createInitializeAccount(currencyAccount, cluster.defproduceraAccount, buyRAM=200000,
                                           stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.defproduceraAccount.name))
    transId = node.createInitializeAccount(exchangeAccount, cluster.defproduceraAccount, buyRAM=200000,
                                           waitForTransBlock=True, exitOnError=True)
    ...
    actual = amountStr
    if actual != expected:
        errorExit("FAILURE - currency1111 balance check failed. Expected: %s, Recieved %s" % (expected, actual),
                  raw=True)
    ...
    for _ in range(5):
        ...
        if transDuplicate is not None and transDuplicate[0]:
            ...
            if transId != transDuplicateId:
                Print(
                    "%s push message currency1111 duplicate transfer incorrectly accepted, but they were generated with different transaction ids, this is a timing setup issue, trying again" % (
                        ClientName))
    ...
    try:
        ...
        Print("ERROR: Failed get currecy balance assertion. (expected=<%s>, actual=<%s>)" % (
            expectedCurrency1111Balance, actual))
        raise
    ...
    try:
        ...
        Print(
            "ERROR: Failed get currecy balance assertion. (expected=<%s>, actual=<%s>)" % (str(expected), str(actual)))
        raise
    ...
    actual = amountStr
    if actual != expected:
        errorExit("FAILURE - Wrong currency1111 balance (expected=%s, actual=%s)" % (str(expected), str(actual)),
                  raw=True)
    ...
    if not enableMongo:
        ...
        assert match is not None, "did not find the core liquid balance (\"liquid:\") of %d in \"%s\"" % (
            coreLiquidBalance, rawAccount)
