...
def activateAccounts(accounts):
    ...
    for i in range(len(accounts)):
        ...
        cmd='programs/cleos/cleos --url http://%s:8888  --wallet-url http://%s:9899 system newaccount -j eosio %s %s %s --stake-net "100 SYS" --stake-cpu "100 SYS" --buy-ram "100 SYS"' % (hostname,hostname,accounts[i].name, accounts[i].ownerPublicKey, accounts[i].activePublicKey)
        ...
        cmd='programs/cleos/cleos --url http://%s:8888  --wallet-url http://%s:9899 push action -j eosio.token transfer \'{"from":"eosio","to":"%s","quantity":"10000.0000 SYS","memo":"init transfer"}\' --permission eosio@active' % (hostname,hostname,accounts[i].name)
