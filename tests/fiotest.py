...
programs/cleos/cleos --url http://%(hostname)s:8888 --wallet-url http://%(hostname)s:9899 wallet import --private-key 5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3
    """ % {"hostname": hostname}
...
def activateAccounts(accounts):
    ...
    for i in range(len(accounts)):
        cmd = "programs/cleos/cleos --url http://%(hostname)s:8888 --wallet-url http://%(hostname)s:9899 wallet import --private-key %(key)s" % {
            "hostname": hostname, "key": accounts[i].ownerPrivateKey}
        print(cmd)
        ...
        cmd = "programs/cleos/cleos --url http://%(hostname)s:8888 --wallet-url http://%(hostname)s:9899 wallet import --private-key %(key)s" % {
            "hostname": hostname, "key": accounts[i].activePrivateKey}
        print(cmd)
        print(cmd, file=log_file)
        log_file.flush()
        ret = subprocess.call(cmd.split(), stdout=log_file, stderr=log_file)
        ...
        cmd = 'programs/cleos/cleos --url http://%s:8888  --wallet-url http://%s:9899 system newaccount -j eosio %s %s %s --stake-net "100 SYS" --stake-cpu "100 SYS" --buy-ram "100 SYS"' % (
        hostname, hostname, accounts[i].name, accounts[i].ownerPublicKey, accounts[i].activePublicKey)
        ...
        cmd = 'programs/cleos/cleos --url http://%s:8888  --wallet-url http://%s:9899 push action -j eosio.token transfer \'{"from":"eosio","to":"%s","quantity":"10000.0000 SYS","memo":"init transfer"}\' --permission eosio@active' % (
        hostname, hostname, accounts[i].name)
...
if __name__ == '__main__':
    try:
        ...
        # names=('partner11111', 'partner22222', 'partner33333', 'mymultisig11')
        names = ('fioname11111', 'user11111111', 'user22222222')
        # names=('exchange1111', 'currency1111')
