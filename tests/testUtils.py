...
class Utils:
    ...
    def pgrepCmd(serverName):
        ...
        try:
            pgrepHelp = re.search('-a', subprocess.Popen("pgrep --help 2>/dev/null", shell=True,
    ...
    def compare(obj1, obj2, context):
        ...
        type2 = type(obj2)
        if type1 != type2:
            return "obj1(%s) and obj2(%s) are different types, so cannot be compared, context=%s" % (
            type1, type2, context)
        ...
        if type1 == list:
            ...
            if diffSizes:
                return "left and right side %s comparison have different sizes %d != %d, context=%s" % (
                typeName, len1, len2, context)
            return None
        ...
        if type1 == dict:
            ...
            for i in range(minLen):
                ...
                if key not in obj2:
                    return "right side does not contain key=%s (has %s) that left side does, context=%s" % (
                    key, keys2, context)
            ...
            if diffSizes:
                return "left and right side %s comparison have different number of keys %d != %d, context=%s" % (
                typeName, len1, len2, context)
