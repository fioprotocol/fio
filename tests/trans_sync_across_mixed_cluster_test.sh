...
getHeadBlockNum()
{
  INFO="$(programs/clio/clio get info)"
  verifyErrorCode "clio get info"
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clio/clio --port $port2 get block 1)"
    verifyErrorCode "clio get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clio/clio create key)"
verifyErrorCode "clio create key"
...
PASSWORD_INITA="$(programs/clio/clio wallet create --name inita)"
verifyErrorCode "clio wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clio/clio wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clio wallet import"
programs/clio/clio wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clio wallet import"
programs/clio/clio wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clio wallet import"
programs/clio/clio wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clio wallet import"
...
ACCOUNT_INFO="$(programs/clio/clio create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clio create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clio/clio get account testera)"
verifyErrorCode "clio get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clio/clio transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clio transfer"
   ...
   TRANS_INFO="$(programs/clio/clio --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clio get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
