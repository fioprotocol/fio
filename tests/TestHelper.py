...
class TestHelper(object):
    ...
    def parse_args(includeArgs, applicationSpecificArgs=AppArgs()):
        ...
        if "--kill-sig" in includeArgs:
            parser.add_argument("--kill-sig", type=str, choices=[Utils.SigKillTag, Utils.SigTermTag],
                                help="kill signal.",
        ...
        if "--leave-running" in includeArgs:
            parser.add_argument("--leave-running", help="Leave cluster running after test finishes",
                                action='store_true')
        ...
        if "--sanity-test" in includeArgs:
            parser.add_argument("--sanity-test", help="Validates nodeos and kleos are in path and can be started up.",
                                action='store_true')
        if "--alternate-version-labels-file" in includeArgs:
            parser.add_argument("--alternate-version-labels-file", type=str,
                                help="Provide a file to define the labels that can be used in the test and the path to the version installation associated with that.")
    ...
    def shutdown(cluster, walletMgr, testSuccessful=True, killEosInstances=True, killWallet=True, keepLogs=False,
                 cleanRun=True, dumpErrorDetails=False):
