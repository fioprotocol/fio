...
BOOST_REQUIRE_EQUAL(scheduled_trxs
.
...
code(), missing_auth_exception::code_value
...
asset get_currency_balance(const TESTER &chain, account_name account) {
    return chain.get_currency_balance(N(fio.token), symbol(SY(4, CUR)), account);
}

const std::string fio_token = name(N(fio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) {try {
    ...
    chain.create_account(N(fio.token));
    chain.produce_blocks(10);

    chain.set_code(N(fio.token), contracts::fio_token_wasm());
    chain.set_abi(N(fio.token), contracts::fio_token_abi().data());
    ...
    chain.push_action(config::system_account_name, linkauth::get_name(), tester_account, fc::mutable_variant_object()
            ...
            ("requirement", "first"));
    chain.produce_blocks();
    chain.push_action(N(fio.token), N(create), N(fio.token), mutable_variant_object()
            ("issuer", fio_token)
    ...
    auto trace = chain.push_action(N(fio.token), name("transfer"), N(fio.token), fc::mutable_variant_object()
            ("from", fio_token)
            ("to", "tester")
    ...
    auto liquid_balance = get_currency_balance(chain, N(fio.token));
    ...
    trace = chain.push_action(N(fio.token), name("transfer"), N(tester), fc::mutable_variant_object()
            ("from", "tester")
            ("to", "tester2")
            ("quantity", "1.0000 CUR")
            ("memo", "hi")
    ...
    liquid_balance = get_currency_balance(chain, N(fio.token));
    BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
    liquid_balance = get_currency_balance(chain, N(tester));
...
BOOST_AUTO_TEST_CASE( mindelay_test ) {try {
    ...
    const auto &acnt = chain.control->db().get<account_object, by_name>(N(fio.token));
    const auto abi = acnt.get_abi();
    chain::abi_serializer abis(abi, chain.abi_serializer_max_time);
    const auto a = chain.control->db().get<account_object, by_name>(N(fio.token)).get_abi();

    string action_type_name = abis.get_action_type(name("transfer"));

    action act;
    act.account = N(fio.token);
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) {try {
...

    {
        ...
        trace = chain.push_action(N(fio.token), name("transfer"), vector < permission_level > {{N(tester), N(first)}},
                                  fc::mutable_variant_object()
                                          ("from", "tester")
                                          ("to", "tester2")
                                          ("quantity", "1.0000 CUR")
        ...
        // attempt canceldelay with "second" permission for delayed transfer of 1.0000 CUR
        {
            ...
            BOOST_REQUIRE_EXCEPTION(chain.push_transaction(trx), irrelevant_auth_exception,
                                    fc_exception_message_starts_with(
                                            "canceldelay action declares irrelevant authority"));
        }
    ...
    {
        // this transaction will be delayed 10 blocks
        trace = chain.push_action(N(fio.token), name("transfer"),
                                  vector < permission_level > {{N(tester), N(second)}}, fc::mutable_variant_object()
                                          ("from", "tester")
                                          ("to", "tester2")
                                          ("quantity", "5.0000 CUR")
                                          ("memo", "hi"),
...

    {
        // this transaction will be delayed 10 blocks
        trace = chain.push_action(N(fio.token), name("transfer"),
                                  vector < permission_level > {{N(tester), config::owner_name}},
                                  fc::mutable_variant_object()
                                          ("from", "tester")
                                          ("to", "tester2")
                                          ("quantity", "10.0000 CUR")
...
BOOST_REQUIRE_EQUAL(transaction_receipt_header::delayed, trace
->receipt->status);
...
sb = produce_block();
BOOST_REQUIRE_EQUAL(1, sb->transactions.

size()

);
BOOST_REQUIRE_EQUAL(transaction_receipt_header::expired, sb
