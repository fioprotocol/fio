...
public:
    ...
    void init(bool bootstrap = true) {
        ...
        chain->create_accounts({N(fio.token), N(alice), N(bob), N(charlie)});
        chain->set_code(N(fio.token), contracts::fio_token_wasm());
        chain->set_abi(N(fio.token), contracts::fio_token_abi().data());
        chain->push_action(N(fio.token), N(create), N(fio.token), mvo()
                ("issuer", "fio.token")
                ("maximum_supply", "1000000.00 TOK")
        );
        chain->push_action(N(fio.token), N(issue), N(fio.token), mvo()
                ("to", "fio.token")
    ...
    transaction_trace_ptr transfer(account_name from, account_name to, string quantity = "1.00 TOK") {
        return chain->push_action(N(fio.token), N(transfer), from, mvo()
                ("from", from)
                ("to", to)
                ("quantity", quantity)
                ("memo", "")
...
FC_REFLECT( transfer_args, (from)(to)(quantity)
(memo))
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) {try {
    ...
    test.actor_whitelist = {config::system_account_name, N(fio.token), N(alice)};
    test.init();

    test.transfer(N(fio.token), N(alice), "1000.00 TOK");
...
BOOST_AUTO_TEST_CASE( actor_blacklist ) {try {
    ...
    BOOST_CHECK_EXCEPTION(test.transfer(N(bob), N(alice)),
                          ...
                          fc_exception_message_starts_with(
                                  "authorizing actor(s) in transaction are on the actor blacklist: [\"bob\"]")
    );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) {try {
    whitelist_blacklist_tester<> test;
    test.contract_whitelist = {config::system_account_name, N(fio.token), N(bob)};
    test.init();

    test.transfer(N(fio.token), N(alice), "1000.00 TOK");

    test.transfer(N(alice), N(fio.token));
    ...
    test.chain->set_code(N(bob), contracts::fio_token_wasm());
    test.chain->set_abi(N(bob), contracts::fio_token_abi().data());

    test.chain->produce_blocks();

    test.chain->set_code(N(charlie), contracts::fio_token_wasm());
    test.chain->set_abi(N(charlie), contracts::fio_token_abi().data());
...
BOOST_AUTO_TEST_CASE( action_blacklist ) {try {
    whitelist_blacklist_tester<> test;
    test.contract_whitelist = {config::system_account_name, N(fio.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_eosio ) {try {
    ...
    tester1.chain->set_code(config::system_account_name, contracts::fio_token_wasm());
    tester1.chain->produce_blocks();
    tester1.shutdown();
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) {try {
    ...
    auto log_trxs = [&](const transaction_trace_ptr &t) {
        ...
        if (t->receipt && t->receipt->status == transaction_receipt::executed) {
            wlog("${trx_type} ${id} executed (first action is ${code}::${action})",
                 ("trx_type", t->scheduled ? "scheduled trx" : "trx")("id", t->id)("code", act.account)("action",
                                                                                                        act.name));
        } else {
    ...
    BOOST_CHECK_EXCEPTION(tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is(
                                  "authorizing actor(s) in transaction are on the actor blacklist: [\"charlie\"]")
    );
