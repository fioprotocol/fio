...
BOOST_AUTO_TEST_SUITE(whitelist_blacklist_tests)
        ...
        try {
            ...
            BOOST_CHECK_EXCEPTION(test.chain->push_action(N(charlie), N(create), N(charlie), mvo()
                    ...
                    ("maximum_supply", "1000000.00 CUR")
                                  ...
                                  fc_exception_message_starts_with(
                                          "action 'charlie::create' is on the action blacklist")
            );
            test.chain->produce_blocks();
        } FC_LOG_AND_RETHROW()
    }
            ...
            auto log_trxs = [&](std::tuple<const transaction_trace_ptr &, const signed_transaction &> x) {
                ...
                if (t->receipt && t->receipt->status == transaction_receipt::executed) {
                    wlog("${trx_type} ${id} executed (first action is ${code}::${action})",
                         ("trx_type", t->scheduled ? "scheduled trx" : "trx")("id", t->id)("code", act.account)(
                                 "action", act.name));
                } else {
    ...
    BOOST_AUTO_TEST_CASE(greylist_limit_tests) {
        try {
            ...
            cfg.target_block_cpu_usage_pct =
                    config::percent_1 / 10; // More than 150 us of CPU to keep the block above the target.
            ...
            c.push_action(config::system_account_name, N(setalimits), config::system_account_name,
