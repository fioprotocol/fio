...
public:
    snapshotted_tester(controller::config config, const snapshot_reader_ptr &snapshot, int ordinal) {
        ...
        copied_config.blocks_dir =
                config.blocks_dir.parent_path() /
                std::to_string(ordinal).append(config.blocks_dir.filename().generic_string());
        copied_config.state_dir =
                config.state_dir.parent_path() /
                std::to_string(ordinal).append(config.state_dir.filename().generic_string());

        init(copied_config, snapshot);
    }

    snapshotted_tester(controller::config config, const snapshot_reader_ptr &snapshot, int ordinal,
                       int copy_block_log_from_ordinal) {
        FC_ASSERT(config.blocks_dir.filename().generic_string() != "."
                  && config.state_dir.filename().generic_string() != ".", "invalid path names in controller::config");
        ...
        auto block_log_path = config.blocks_dir.parent_path() / std::to_string(copy_block_log_from_ordinal).append(
                config.blocks_dir.filename().generic_string());
        fc::create_directories(copied_config.blocks_dir);
        fc::copy(block_log_path / "blocks.log", copied_config.blocks_dir / "blocks.log");
        fc::copy(block_log_path / config::reversible_blocks_dir_name,
                 copied_config.blocks_dir / config::reversible_blocks_dir_name);
    ...
    signed_block_ptr produce_empty_block(fc::microseconds skip_time = fc::milliseconds(config::block_interval_ms),
                                         uint32_t skip_flag = 0/*skip_missed_block_penalty*/ ) override {
        control->abort_block();
...
struct variant_snapshot_suite {
    ...
    struct writer : public writer_t {
        writer(const std::shared_ptr <write_storage_t> &storage)
                : writer_t(*storage), storage(storage) {
...
struct buffered_snapshot_suite {
    ...
    struct reader : public reader_t {
        explicit reader(const std::shared_ptr <read_storage_t> &storage)
                : reader_t(*storage), storage(storage) {}
...
create_account (N(snapshot));

chain.produce_blocks(1);
chain.

set_code(N(snapshot), contracts::snapshot_test_wasm()

);
chain.

set_abi(N(snapshot), contracts::snapshot_test_abi()
...
get_config(), SNAPSHOT_SUITE::get_reader(snapshot), generation

);

// increment the test contract
chain.
push_action(N(snapshot), N(increment), N(snapshot), mutable_variant_object()
( "value", 1 )
);
...
BOOST_REQUIRE_EQUAL(integrity_value
.
...
BOOST_REQUIRE_EQUAL(expected_pre_integrity_hash
.
...
BOOST_REQUIRE_EQUAL(expected_post_integrity_hash
.
