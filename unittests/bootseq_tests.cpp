...
BOOST_AUTO_TEST_SUITE(bootseq_tests)
    ...
    BOOST_FIXTURE_TEST_CASE(bootseq_test, bootseq_tester) {
        try {
            ...
            create_accounts(
                    {N(eosio.msig), N(eosio.token), N(eosio.ram), N(eosio.ramfee), N(eosio.stake), N(eosio.vpay),
                     N(eosio.bpay), N(eosio.saving)});
...
// Issue the genesis supply of 1 billion SYS tokens to fio.system
            ...
            produce_blocks_for_n_rounds(
                    2); // 2 rounds since new producer schedule is set when the first block of next round is irreversible
            ...
            BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"),
                                                     core_from_string("49999500.0000")),
