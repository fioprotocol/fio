...
BOOST_REQUIRE_EQUAL( true, control->
...
compare_schedules( sch1, *control
->

proposed_producers()

));
BOOST_CHECK_EQUAL( control
->
...
BOOST_CHECK_EQUAL( true,
compare_schedules( sch2, *control
->
...
BOOST_REQUIRE_EQUAL( control
->

head_block_producer(), N(alice)

);
BOOST_REQUIRE_EQUAL( control
->pending_block_state()->header.producer,
N(bob)
);
BOOST_CHECK_EQUAL( control
->
...
head_block_producer(), N(carol)

); // And that next block happens to be produced by Carol.
...
BOOST_FIXTURE_TEST_CASE( empty_producer_schedule_has_no_effect, tester
) try {
create_accounts( {
N(alice), N(bob), N(carol)
} );
...
BOOST_CHECK_EQUAL( control
->proposed_producers()->producers.

size(),

0u );
BOOST_CHECK_EQUAL( control
->proposed_producers()->version, 2u );
...
.version, 2u );
BOOST_CHECK_EQUAL( false, control->
