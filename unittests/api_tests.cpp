    ...
    BOOST_FIXTURE_TEST_CASE(action_receipt_tests, TESTER) {
        try {
            ...
            auto call_doit_and_check = [&](account_name contract, account_name signer, auto &&checker) {
                signed_transaction trx;
                trx.actions.emplace_back(vector<permission_level>{{signer, config::active_name}}, contract, N(doit),
                                         bytes{});
            ...
            auto call_provereset_and_check = [&](account_name contract, account_name signer, auto &&checker) {
                signed_transaction trx;
                trx.actions.emplace_back(vector<permission_level>{{signer, config::active_name}}, contract,
                                         N(provereset), bytes{});
                this->set_transaction_headers(trx, this->DEFAULT_EXPIRATION_DELTA);
                trx.sign(this->get_private_key(signer, "active"), control->get_chain_id());
                auto res = this->push_transaction(trx);
 ...
 *************************************************************************************/
    BOOST_FIXTURE_TEST_CASE(action_tests, TESTER) {
        try {
            ...
            BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_action", "require_auth", fc::raw::pack(a3only)),
            ...
            BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_action", "require_auth", fc::raw::pack(a4only)),
                                  missing_auth_exception,
                                  [](const missing_auth_exception &e) {
                                      return expect_assert_message(e, "missing authority of");
                                  }
    ...
    BOOST_AUTO_TEST_CASE(ram_billing_in_notify_tests) {
        try {
            ...
            BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(chain, "test_action", "test_ram_billing_in_notify",
                                                     fc::raw::pack(
                                                             ((unsigned __int128) N(testapi2) << 64) | N(testapi))),
                                  subjective_block_production_exception,
                                  fc_exception_message_is("Cannot charge RAM to other accounts during notify.")
            );


            CALL_TEST_FUNCTION(chain, "test_action", "test_ram_billing_in_notify",
 ...
 *************************************************************************************/
    ...
    BOOST_FIXTURE_TEST_CASE(cfa_tx_signature, TESTER) try {
        ...
        BOOST_TEST((std::string) tx1.sign(priv_key, control->get_chain_id()) !=
                   (std::string) tx2.sign(priv_key, control->get_chain_id()));
 ...
 *************************************************************************************/
    ...
    void call_test(TESTER &test, T ac, uint32_t billed_cpu_time_us, uint32_t max_cpu_usage_ms = 200,
        ...
        flat_set<public_key_type> keys;
        trx.get_signature_keys(test.control->get_chain_id(), fc::time_point::maximum(), keys);
        auto res = test.push_transaction(trx, fc::time_point::now() + fc::milliseconds(max_cpu_usage_ms),
                                         billed_cpu_time_us);
        BOOST_CHECK_EQUAL(res->receipt->status, transaction_receipt::executed);
        test.produce_block();
    };
...
#warning TODO validate that the contract was successfully cached
    ...
    BOOST_FIXTURE_TEST_CASE(checktime_hashing_fail, TESTER) {
        try {
            ...
            BOOST_CHECK_EXCEPTION(
                    call_test(*this, test_api_action<TEST_METHOD("test_checktime", "checktime_sha1_failure")>{},
                              5000, 3),
                    deadline_exception, is_deadline_exception);
...
#warning TODO validate that the contract was successfully cached
            ...
            BOOST_CHECK_EXCEPTION(
                    call_test(*this, test_api_action<TEST_METHOD("test_checktime", "checktime_assert_sha1_failure")>{},
                              5000, 3),
                    deadline_exception, is_deadline_exception);

            BOOST_CHECK_EXCEPTION(
                    call_test(*this, test_api_action<TEST_METHOD("test_checktime", "checktime_sha256_failure")>{},
                              5000, 3),
                    deadline_exception, is_deadline_exception);

            BOOST_CHECK_EXCEPTION(call_test(*this, test_api_action<TEST_METHOD("test_checktime",
                                                                               "checktime_assert_sha256_failure")>{},
                                            5000, 3),
                                  deadline_exception, is_deadline_exception);

            BOOST_CHECK_EXCEPTION(
                    call_test(*this, test_api_action<TEST_METHOD("test_checktime", "checktime_sha512_failure")>{},
                              5000, 3),
                    deadline_exception, is_deadline_exception);

            BOOST_CHECK_EXCEPTION(call_test(*this, test_api_action<TEST_METHOD("test_checktime",
                                                                               "checktime_assert_sha512_failure")>{},
                                            5000, 3),
                                  deadline_exception, is_deadline_exception);

            BOOST_CHECK_EXCEPTION(
                    call_test(*this, test_api_action<TEST_METHOD("test_checktime", "checktime_ripemd160_failure")>{},
                              5000, 3),
                    deadline_exception, is_deadline_exception);

            BOOST_CHECK_EXCEPTION(call_test(*this, test_api_action<TEST_METHOD("test_checktime",
                                                                               "checktime_assert_ripemd160_failure")>{},
                                            5000, 3),
                                  deadline_exception, is_deadline_exception);
 ...
 *************************************************************************************/
    BOOST_FIXTURE_TEST_CASE(transaction_tests, TESTER) {
        try {
            ...
            CALL_TEST_FUNCTION(*this, "test_transaction", "test_tapos_block_num",
                               fc::raw::pack(control->head_block_num()));
    ...
    BOOST_FIXTURE_TEST_CASE(deferred_transaction_tests, TESTER) {
        try {
            ...
            //schedule twice without replace_existing flag (second deferred transaction should replace first one)
            {
                ...
                CALL_TEST_FUNCTION(*this, "test_transaction", "send_deferred_transaction", {});
                BOOST_CHECK_THROW(CALL_TEST_FUNCTION(*this, "test_transaction", "send_deferred_transaction", {}),
            ...
            //repeated deferred transactions
            {
                ...
                CALL_TEST_FUNCTION(*this, "test_transaction", "repeat_deferred_transaction",
                                   fc::raw::pack((uint32_t) 5));
...

            {
                ...
                BOOST_CHECK_THROW(CALL_TEST_FUNCTION(*this, "test_transaction", "send_deferred_tx_with_dtt_action",
                                                     fc::raw::pack(dtt_act1)), action_validate_exception);
                ...
                CALL_TEST_FUNCTION(*this, "test_transaction", "send_deferred_tx_with_dtt_action",
                                   fc::raw::pack(dtt_act2));
                ...
                push_action(config::system_account_name, N(setpriv), config::system_account_name,
                            mutable_variant_object()
                                    ("account", "testapi")
                                    ("is_priv", 1));
                CALL_TEST_FUNCTION(*this, "test_transaction", "send_deferred_tx_with_dtt_action",
                                   fc::raw::pack(dtt_act1));
                CALL_TEST_FUNCTION(*this, "test_transaction", "send_deferred_tx_with_dtt_action",
                                   fc::raw::pack(dtt_act2));
            }
 ...
 *************************************************************************************/
    BOOST_FIXTURE_TEST_CASE(db_tests, TESTER) {
        try {
            ...
            push_action(N(testapi), N(sk32align), N(testapi),
 ...
 *************************************************************************************/
    BOOST_FIXTURE_TEST_CASE(print_tests, TESTER) {
        try {
            ...
            BOOST_CHECK_EQUAL(tx7_act_cnsl.substr(start, end - start),
            ...
            const char *abi_string = R"=====(
{
   ...
   "abi_extensions": []
            ...
            auto trace = CALL_TEST_FUNCTION_NO_THROW(*this, "test_action", "test_assert_code",
                                                     fc::raw::pack((uint64_t) 42));
            BOOST_REQUIRE(trace);
            ...
            BOOST_REQUIRE_EQUAL(*trace2->action_traces[0].error_code,
                                static_cast<uint64_t>(system_error_code::contract_restricted_error_code));
