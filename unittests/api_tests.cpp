...
FC_REFLECT( dummy_action, (a)(b)
(c))
FC_REFLECT( u128_action, (values)
)
FC_REFLECT( cf_action, (payload)(cfd_idx)
)
FC_REFLECT( dtt_action, (payer)(deferred_account)(deferred_action)(permission_name)
(delay_sec))
FC_REFLECT( invalid_access_action, (code)(val)(index)
(store))
...
FC_REFLECT_TEMPLATE((uint64_t
T), test_api_action<T>, BOOST_PP_SEQ_NIL)
...
FC_REFLECT_TEMPLATE((uint64_t
T), test_chain_action<T>, BOOST_PP_SEQ_NIL)
...
FC_REFLECT(check_auth, (account)(permission)
(pubkeys))
...
FC_REFLECT( test_permission_last_used_action, (account)(permission)
(last_used_time))
...
auto call_provereset_and_check = [&](account_name contract, account_name signer, auto &&checker) {
    signed_transaction trx;
    trx.actions.emplace_back(vector < permission_level > {{signer, config::active_name}}, contract, N(provereset),
                             bytes{});
    this->set_transaction_headers(trx, this->DEFAULT_EXPIRATION_DELTA);
    trx.sign(this->get_private_key(signer, "active"), control->get_chain_id());
    auto res = this->push_transaction(trx);
...
->receipt->status, transaction_receipt::executed );
BOOST_REQUIRE( result
->action_traces[0].receipt.auth_sequence.
find( config::system_account_name )
!= result->action_traces[0].receipt.auth_sequence.
...
call_doit_and_check( N(test), N(test),
...
->receipt->status, transaction_receipt::executed );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.global_sequence, base_global_sequence_num + 1 );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.code_sequence.value, 1 );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.abi_sequence.value, 0 );
base_test_recv_seq_num = res->action_traces[0].receipt.recv_sequence;
BOOST_CHECK( base_test_recv_seq_num
> 0 );
...
BOOST_CHECK( base_test_auth_seq_num
> 0 );
--
...
call_provereset_and_check( N(test), N(test),
...
->receipt->status, transaction_receipt::executed );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.global_sequence, base_global_sequence_num + 4 );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.recv_sequence, base_test_recv_seq_num + 2 );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.code_sequence.value, 2 );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.abi_sequence.value, 0 );
const auto &m = res->action_traces[0].receipt.auth_sequence;
BOOST_CHECK_EQUAL( m
...
"test" );
BOOST_CHECK_EQUAL( m
.begin()->second, base_test_auth_seq_num + 3 );
} );
...
call_doit_and_check( config::system_account_name, N(test),
[&](
const transaction_trace_ptr &res
) {
BOOST_CHECK_EQUAL( res
->receipt->status, transaction_receipt::executed );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.global_sequence, base_global_sequence_num + 6 );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.recv_sequence, base_system_recv_seq_num + 4 );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.code_sequence.value, base_system_code_seq_num + 1 );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.abi_sequence.value, base_system_abi_seq_num );
const auto &m = res->action_traces[0].receipt.auth_sequence;
BOOST_CHECK_EQUAL( m
.
...
"test" );
BOOST_CHECK_EQUAL( m
.begin()->second, base_test_auth_seq_num + 4 );
} );
...
set_abi( N(test), contracts::eosio_bios_abi()
...
->receipt->status, transaction_receipt::executed);
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.global_sequence, base_global_sequence_num + 11 );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.recv_sequence, base_test_recv_seq_num + 3 );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.code_sequence.value, 4 );
BOOST_CHECK_EQUAL( res
->action_traces[0].receipt.abi_sequence.value, 1 );
const auto &m = res->action_traces[0].receipt.auth_sequence;
BOOST_CHECK_EQUAL( m
.
...
"test" );
BOOST_CHECK_EQUAL( m
.begin()->second, base_test_auth_seq_num + 8 );
} );
...
BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_action", "read_action_to_0",
                                         raw_bytes), eosio::chain::wasm_execution_error,
[](
const eosio::chain::wasm_execution_error &e
) {
return
expect_assert_message(e,
"access violation");
}
);
...
// test read_action_to_64k
raw_bytes.resize(3);
BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_action", "read_action_to_64k",
                                         raw_bytes), eosio::chain::wasm_execution_error,
[](
const eosio::chain::wasm_execution_error &e
) {
...
BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_action", "require_auth",
                                         fc::raw::pack(a3only)), missing_auth_exception,
[](
const missing_auth_exception &e
) {
return
...
BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_action", "require_auth",
                                         fc::raw::pack(a4only)), missing_auth_exception,
[](
const missing_auth_exception &e
) {
return
...
// test require_auth
auto a3a4 = std::vector < permission_level > {{N(acc3), config::active_name},
                                              {N(acc4), config::active_name}};
...
i++)
pl.push_back({
a3a4_scope[i], config::active_name});
...
end(), std::back_inserter(dest)

);
act.
authorization = {{N(testapi), config::active_name},
                 {N(acc3),    config::active_name},
                 {N(acc4),    config::active_name}};
trx.actions.
push_back(act);
...
trx.
sign(get_private_key(N(testapi), "active"), control
->
...
trx.
sign(get_private_key(N(acc3), "active"), control
->
...
trx.
sign(get_private_key(N(acc4), "active"), control
->
...
BOOST_FIXTURE_TEST_CASE(ram_billing_in_notify_tests, TESTER
) {
try {
produce_blocks(2);
...
BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION(*this, "test_action", "test_ram_billing_in_notify",
                                          fc::raw::pack(((unsigned __int128) N(testapi2) << 64) | N(testapi))),
        subjective_block_production_exception, fc_exception_message_is(
        "Cannot charge RAM to other accounts during notify.")
);


CALL_TEST_FUNCTION(*this, "test_action", "test_ram_billing_in_notify",
                   fc::raw::pack(((unsigned __int128) N(testapi2) << 64) | 0));

CALL_TEST_FUNCTION(*this, "test_action", "test_ram_billing_in_notify",
                   fc::raw::pack(((unsigned __int128) N(testapi2) << 64) | N(testapi2)));
...
emplace_back(fc::raw::pack<uint32_t>(100)
); // verify payload matches context free data
trx.context_free_data.
emplace_back(fc::raw::pack<uint32_t>(200)
);
set_transaction_headers(trx);
...
1);
BOOST_CHECK_EQUAL(ttrace
->action_traces[0].inline_traces.

size(),

1);
BOOST_CHECK_EQUAL(ttrace
->action_traces[0].inline_traces[0].receipt.receiver, account_name("dummy"));
BOOST_CHECK_EQUAL(ttrace
->action_traces[0].inline_traces[0].act.account, account_name("dummy"));
BOOST_CHECK_EQUAL(ttrace
->action_traces[0].inline_traces[0].act.name, account_name("event1"));
BOOST_CHECK_EQUAL(ttrace
->action_traces[0].inline_traces[0].act.authorization.
...
trx.actions.
emplace_back( vector<permission_level>{{creator, config::active_name}},
        newaccount{
        .creator  = creator,
        .name     = a,
...
set_transaction_headers(trx);
trx.
sign( get_private_key(creator, "active"), control
->
...
!= nullptr);
if (trace) {
BOOST_REQUIRE_EQUAL(transaction_receipt_header::status_enum::delayed, trace
->receipt->status);
...
template<class T>
void call_test(TESTER & test, T
ac,
uint32_t billed_cpu_time_us, uint32_t
max_cpu_usage_ms = 200, std::vector<char>
payload = {}
) {
signed_transaction trx;
...
flat_set <public_key_type> keys;
trx.
get_signature_keys(test
.control->

get_chain_id(), fc::time_point::maximum(), keys
...
BOOST_CHECK_EXCEPTION( call_test(*this,
...
->receipt->status, transaction_receipt::soft_fail);
c.
...
CALL_TEST_FUNCTION(*this, "test_transaction", "test_transaction_size",
                   fc::raw::pack(54)); // TODO: Need a better way to test this.
...
BOOST_TEST_MESSAGE( "tx_trace->action_traces.front().console: = " << tx_trace->action_traces.

front()

.console );
BOOST_TEST_MESSAGE( "sha_expect = " << sha_expect );
BOOST_CHECK_EQUAL(tx_trace
->action_traces.

front()

.console == sha_expect, true);
...
CALL_TEST_FUNCTION(*this, "test_transaction", "test_tapos_block_prefix",
...
BOOST_TEST(!trace->action_traces.
...
: dtrxs) {
control->
push_scheduled_transaction(trx, fc::time_point::maximum()
);
}
BOOST_CHECK_EQUAL(1, count);
BOOST_REQUIRE(trace);
BOOST_CHECK_EQUAL( 1, trace->action_traces.
...
{
transaction_trace_ptr trace;
auto c = control->applied_transaction.connect(
        [&](const transaction_trace_ptr &t) { if (t && t->scheduled) { trace = t; }});
CALL_TEST_FUNCTION(*this, "test_transaction", "send_deferred_transaction", {});
...
CALL_TEST_FUNCTION(*this, "test_transaction", "repeat_deferred_transaction", fc::raw::pack((uint32_t)
        5 ));
...
dtt_act1.
payer = N(alice);
BOOST_CHECK_THROW(CALL_TEST_FUNCTION(*this, "test_transaction", "send_deferred_tx_with_dtt_action",
                                     fc::raw::pack(dtt_act1)), missing_auth_exception
);
...
dtt_action dtt_act2;
dtt_act2.
deferred_account = N(testapi2);
dtt_act2.
permission_name = N(additional);
dtt_act2.
delay_sec = 5;

auto auth = authority(get_public_key("testapi", name(dtt_act2.permission_name).to_string()), 10);
auth.accounts.
push_back( permission_level_weight{{N(testapi), config::eosio_code_name}, 1}
);

push_action(config::system_account_name, updateauth::get_name(),
"testapi",
...
push_action(config::system_account_name, linkauth::get_name(),
"testapi",
...
.permission_name)));
BOOST_CHECK_THROW(CALL_TEST_FUNCTION(*this, "test_transaction", "send_deferred_tx_with_dtt_action",
                                     fc::raw::pack(dtt_act2)), unsatisfied_authorization
);
...
.permission_name)));
BOOST_CHECK_THROW(CALL_TEST_FUNCTION(*this, "test_transaction", "send_deferred_tx_with_dtt_action",
                                     fc::raw::pack(dtt_act3)), subjective_block_production_exception
);
...
set_abi(  N(testapi), contracts::test_api_db_abi()
.
...
set_abi(  N(testapi2), contracts::test_api_db_abi()
.
...
push_action( N(testapi), N(pg), N(testapi), mutable_variant_object()
); // primary_i64_general
push_action( N(testapi), N(pl), N(testapi), mutable_variant_object()
); // primary_i64_lowerbound
push_action( N(testapi), N(pu), N(testapi), mutable_variant_object()
); // primary_i64_upperbound
push_action( N(testapi), N(s1g), N(testapi), mutable_variant_object()
); // idx64_general
push_action( N(testapi), N(s1l), N(testapi), mutable_variant_object()
); // idx64_lowerbound
push_action( N(testapi), N(s1u), N(testapi), mutable_variant_object()
); // idx64_upperbound
...
push_action( N(testapi), N(sk32align), N(testapi), mutable_variant_object()
...
set_abi( N(testapi), contracts::test_api_multi_index_abi()
.
...
push_action( N(testapi), N(s1g), N(testapi),
{
} );        // idx64_general
push_action( N(testapi), N(s1store), N(testapi),
{
} );    // idx64_store_only
push_action( N(testapi), N(s1check), N(testapi),
{
} );    // idx64_check_without_storing
push_action( N(testapi), N(s2g), N(testapi),
{
} );        // idx128_general
push_action( N(testapi), N(s2store), N(testapi),
{
} );    // idx128_store_only
push_action( N(testapi), N(s2check), N(testapi),
{
} );    // idx128_check_without_storing
push_action( N(testapi), N(s2autoinc), N(testapi),
{
} );  // idx128_autoincrement_test
push_action( N(testapi), N(s2autoinc1), N(testapi),
{
} ); // idx128_autoincrement_test_part1
push_action( N(testapi), N(s2autoinc2), N(testapi),
{
} ); // idx128_autoincrement_test_part2
push_action( N(testapi), N(s3g), N(testapi),
{
} );        // idx256_general
push_action( N(testapi), N(sdg), N(testapi),
{
} );        // idx_double_general
push_action( N(testapi), N(sldg), N(testapi),
{
} );       // idx_long_double_general

check_failure( N(s1pkend),

"cannot increment end iterator" ); // idx64_pk_iterator_exceed_end
check_failure( N(s1skend),

"cannot increment end iterator" ); // idx64_sk_iterator_exceed_end
check_failure( N(s1pkbegin),

"cannot decrement iterator at beginning of table" ); // idx64_pk_iterator_exceed_begin
check_failure( N(s1skbegin),

"cannot decrement iterator at beginning of index" ); // idx64_sk_iterator_exceed_begin
check_failure( N(s1pkref),

"object passed to iterator_to is not in multi_index" ); // idx64_pass_pk_ref_to_other_table
check_failure( N(s1skref),

"object passed to iterator_to is not in multi_index" ); // idx64_pass_sk_ref_to_other_table
check_failure( N(s1pkitrto),

"object passed to iterator_to is not in multi_index" ); // idx64_pass_pk_end_itr_to_iterator_to
check_failure( N(s1pkmodify),

"cannot pass end iterator to modify" ); // idx64_pass_pk_end_itr_to_modify
check_failure( N(s1pkerase),

"cannot pass end iterator to erase" ); // idx64_pass_pk_end_itr_to_erase
check_failure( N(s1skitrto),

"object passed to iterator_to is not in multi_index" ); // idx64_pass_sk_end_itr_to_iterator_to
check_failure( N(s1skmodify),

"cannot pass end iterator to modify" ); // idx64_pass_sk_end_itr_to_modify
check_failure( N(s1skerase),

"cannot pass end iterator to erase" ); // idx64_pass_sk_end_itr_to_erase
check_failure( N(s1modpk),

"updater cannot change primary key when modifying an object" ); // idx64_modify_primary_key
check_failure( N(s1exhaustpk),

"next primary key in table is at autoincrement limit" ); // idx64_run_out_of_avl_pk
check_failure( N(s1findfail1),

"unable to find key" ); // idx64_require_find_fail
check_failure( N(s1findfail2),

"unable to find primary key in require_find" );// idx64_require_find_fail_with_msg
check_failure( N(s1findfail3),

"unable to find secondary key" ); // idx64_require_find_sk_fail
check_failure( N(s1findfail4),

"unable to find sec key" ); // idx64_require_find_sk_fail_with_msg

push_action( N(testapi), N(s1skcache), N(testapi),
{
} ); // idx64_sk_cache_pk_lookup
push_action( N(testapi), N(s1pkcache), N(testapi),
{
...
BOOST_CHECK_EQUAL( tx5_act_cnsl
.substr(24,13), "12345abcdefgj" );
BOOST_CHECK_EQUAL( tx5_act_cnsl
.substr(37,13), "ijklmnopqrstj" );
BOOST_CHECK_EQUAL( tx5_act_cnsl
.substr(50,13), "vwxyz.12345aj" );

BOOST_CHECK_EQUAL( tx5_act_cnsl
.substr(63, 13), "111111111111j" );
BOOST_CHECK_EQUAL( tx5_act_cnsl
.substr(76, 13), "555555555555j" );
BOOST_CHECK_EQUAL( tx5_act_cnsl
.substr(89, 13), "aaaaaaaaaaaaj" );
BOOST_CHECK_EQUAL( tx5_act_cnsl
.substr(102,13), "zzzzzzzzzzzzj" );
...
BOOST_CHECK_EQUAL( tx6_act_cnsl
.
substr(start, end
-start), U128Str(1));
start = end + 1;
...
.
substr(start, end
-start), "-" + U128Str(static_cast

<unsigned __int128> (std::numeric_limits<__int128>::lowest())
...
.
substr(start, end
-start), "-" + U128Str(87654323456));

// test printui128
auto tx7_trace = CALL_TEST_FUNCTION(*this, "test_print", "test_printui128", {});
auto tx7_act_cnsl = tx7_trace->action_traces.front().console;
start = 0;
end = tx7_act_cnsl.find('\n', start);
BOOST_CHECK_EQUAL( tx7_act_cnsl
.
substr(start, end
-start),

U128Str (std::numeric_limits<unsigned __int128>::max())
...
start = 0;
end = tx8_act_cnsl.find('\n', start);
BOOST_CHECK_EQUAL( tx8_act_cnsl
.
substr(start, end
-start), "5.000000e-01" );
start = end + 1;
...
start = 0;
end = tx9_act_cnsl.find('\n', start);
BOOST_CHECK_EQUAL( tx9_act_cnsl
.
substr(start, end
-start), "5.000000000000000e-01" );
start = end + 1;
end = tx9_act_cnsl.find('\n', start);
BOOST_CHECK_EQUAL( tx9_act_cnsl
.
substr(start, end
-start), "-3.750000000000000e+00" );
start = end + 1;
end = tx9_act_cnsl.find('\n', start);
BOOST_CHECK_EQUAL( tx9_act_cnsl
.
substr(start, end
-start), "6.666666666666666e-07" );
...
start = 0;
end = tx10_act_cnsl.find('\n', start);
BOOST_CHECK_EQUAL( tx10_act_cnsl
.
substr(start, end
-start), expect1 );
start = end + 1;
...
auto get_result_int64 = [&]() -> int64_t {
    ...
    const auto *t_id = db.find<table_id_object, by_code_scope_table>(
            boost::make_tuple(N(testapi), N(testapi), N(testapi)));
...
BOOST_CHECK_EQUAL( int64_t(0), get_result_int64()
); // Failure due to irrelevant signatures
...
push_reqauth( N(alice),

{
{
N(alice), config::active_name
}}, {
get_private_key(N(alice),
"active")} );
...
BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION(*this, "test_action", "test_assert_code", fc::raw::pack((uint64_t)
        42)),
...
BOOST_REQUIRE_EQUAL( omsg1
.

valid(),

true );
BOOST_CHECK_EQUAL( *omsg1,
"standard error message" );

auto omsg2 = abis.get_error_message(2);
BOOST_CHECK_EQUAL( omsg2
.
...
BOOST_REQUIRE_EQUAL( omsg3
.

valid(),

true );
BOOST_CHECK_EQUAL( *omsg3,
