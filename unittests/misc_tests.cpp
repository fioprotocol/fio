        ...
        BOOST_AUTO_TEST_CASE(variant_format_string_limited) {
            const string format = "${a} ${b} ${c}";
            {
                ...
                BOOST_CHECK_EQUAL(result,
                                  string(256, 'a') + "... " + string(256, 'b') + "... " + string(256, 'c') + "...");
            }
            {
                fc::mutable_variant_object mu;
                signed_block a;
...
// Test overflow handling in asset::from_string
        BOOST_AUTO_TEST_CASE(asset_from_string_overflow) {
            ...
            BOOST_CHECK_EXCEPTION(asset::from_string("0.1000000000000000000 CUR"), symbol_type_exception,
                                  [](const auto &e) {
            ...
            BOOST_CHECK_EXCEPTION(asset::from_string("1.0000000000000000000 CUR"), symbol_type_exception,
                                  [](const auto &e) {
                                      return expect_assert_message(e, "precision 19 should be <= 18");
                                  });
            BOOST_CHECK_EXCEPTION(asset::from_string("-1.0000000000000000000 CUR"), symbol_type_exception,
            ...
            BOOST_CHECK_EXCEPTION(asset::from_string("4.611686018427387904 CUR"), asset_type_exception,
            ...
            BOOST_CHECK_EXCEPTION(asset::from_string("4611686018427387.904 CUR"), asset_type_exception,
                                  [](const asset_type_exception &e) {
                                      return expect_assert_message(e,
                                                                   "magnitude of asset amount must be less than 2^62");
                                  });
            ...
            BOOST_CHECK_EXCEPTION(asset::from_string("4611686018427387904 CUR"), asset_type_exception,
                                  [](const asset_type_exception &e) {
                                      return expect_assert_message(e,
                                                                   "magnitude of asset amount must be less than 2^62");
                                  });
            ...
            BOOST_CHECK_EXCEPTION(asset::from_string("36.893488147419103232 CUR"), overflow_exception,
                                  [](const overflow_exception &e) {
                                      return true;
                                  });
            BOOST_CHECK_EXCEPTION(asset::from_string("-36.893488147419103232 CUR"), underflow_exception,
                                  [](const underflow_exception &e) {
                                      return true;
                                  });

            // precision = 14, magnitude > 2^76
            BOOST_CHECK_EXCEPTION(asset::from_string("1000000000.00000000000000 CUR"), overflow_exception,
                                  [](const overflow_exception &e) {
                                      return true;
                                  });
            BOOST_CHECK_EXCEPTION(asset::from_string("-1000000000.00000000000000 CUR"), underflow_exception,
                                  [](const underflow_exception &e) {
                                      return true;
                                  });

            // precision = 0, magnitude > 2^76
            BOOST_CHECK_EXCEPTION(asset::from_string("100000000000000000000000 CUR"), parse_error_exception,
            ...
            BOOST_CHECK_EXCEPTION(asset::from_string("100000000000000000000000.00000000000000000000 CUR"),
                                  symbol_type_exception, [](const auto &e) {
        ...
        BOOST_AUTO_TEST_CASE(authority_checker) {
            try {
                ...
                A = authority(2, {key_weight{a, 2}, key_weight{b, 1}},
                ...
                A = authority(3, {key_weight{a, 2}, key_weight{b, 1}},
                              {permission_level_weight{{"hello", "world"}, 3}});
                {
                ...
                A = authority(2, {key_weight{a, 1}, key_weight{b, 1}},
                              {permission_level_weight{{"hello", "world"}, 1}});
                ...
                A = authority(5, {key_weight{a, 2}, key_weight{b, 2}, key_weight{c, 2}},
                              {permission_level_weight{{"top", "top"}, 5}});
                {
        ...
        BOOST_AUTO_TEST_CASE(transaction_test) {
            try {
                ...
                ((const signed_transaction &) trx).sign(test.get_private_key(config::system_account_name, "active"),
                ...
                auto cpu_time1 = pkt.get_signed_transaction().get_signature_keys(test.control->get_chain_id(),
                                                                                 fc::time_point::maximum(), keys);
                BOOST_CHECK_EQUAL(1u, keys.size());
                BOOST_CHECK_EQUAL(public_key, *keys.begin());
                keys.clear();
                auto cpu_time2 = pkt.get_signed_transaction().get_signature_keys(test.control->get_chain_id(),
                                                                                 fc::time_point::maximum(), keys);
                BOOST_CHECK_EQUAL(1u, keys.size());
                BOOST_CHECK_EQUAL(public_key, *keys.begin());
                ...
                BOOST_CHECK_EQUAL(pkt.get_signed_transaction().id(),
                                  pkt5.get_signed_transaction().id()); // failure indicates reflector_init not working
                ...
                keys.clear();
                pkt4.get_signed_transaction().get_signature_keys(test.control->get_chain_id(),
                                                                 fc::time_point::maximum(), keys);
                BOOST_CHECK_EQUAL(1u, keys.size());
                BOOST_CHECK_EQUAL(public_key, *keys.begin());
        ...
        BOOST_AUTO_TEST_CASE(transaction_metadata_test) {
            try {
                ...
                variant pretty_trx = fc::mutable_variant_object()
                        ...
                        ("context_free_actions", fc::variants({
                                                                      fc::mutable_variant_object()
                                                                              ...
                                                                              ("name", "nonce")
                                                                              ("data",
                                                                               fc::raw::pack(std::string("dummy data")))
                                                              })
                        );
                ...
                transaction_metadata_ptr mtrx = std::make_shared<transaction_metadata>(
                        std::make_shared<packed_transaction>(trx, packed_transaction::none));
                transaction_metadata_ptr mtrx2 = std::make_shared<transaction_metadata>(
                        std::make_shared<packed_transaction>(trx, packed_transaction::zlib));
                ...
                transaction_metadata::start_recover_keys(mtrx, thread_pool.get_executor(), test.control->get_chain_id(),
                                                         fc::microseconds::maximum());
                transaction_metadata::start_recover_keys(mtrx2, thread_pool.get_executor(),
                ...
                transaction_metadata_ptr mtrx4 = std::make_shared<transaction_metadata>(
                        std::make_shared<packed_transaction>(trx, packed_transaction::none));
                transaction_metadata_ptr mtrx5 = std::make_shared<transaction_metadata>(
                        std::make_shared<packed_transaction>(trx, packed_transaction::zlib));
...
// in order, FIFO, as submitted.
        ...
        int i = 0;
        i < 50; ++i ) {
        boost::asio::post(*io_serv, pri_queue
        .
        wrap(appbase::priority::low,
        [io_serv, &mx, &ran, &results, i]() {
