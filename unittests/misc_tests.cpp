...
namespace eosio {
    ...
    BOOST_AUTO_TEST_CASE(name_suffix_tests)
            ...
            BOOST_CHECK_EQUAL( name{
                name_suffix(N(abcdefhi
                .123))
            }, name{ N(123) } );
            BOOST_CHECK_EQUAL( name{
                name_suffix(N(abcdefhij
                .123))
            }, name{ N(123) } );
            }
...
// Test overflow handling in asset::from_string
    ...
    BOOST_AUTO_TEST_CASE(transaction_metadata_test) {try {
        ...
        transaction_metadata::start_recover_keys(mtrx, thread_pool, test.control->get_chain_id(),
                                                 fc::microseconds::maximum());
        transaction_metadata::start_recover_keys(mtrx2, thread_pool, test.control->get_chain_id(),
                                                 fc::microseconds::maximum());
