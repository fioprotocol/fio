...
public:
    ...
    auto transfer(const account_name &from, const account_name &to, const std::string &quantity,
...
BOOST_REQUIRE_EQUAL(SY(2, DLLR), dollar
.

value()

);
BOOST_REQUIRE_EQUAL(2, dollar.

decimals()

);
BOOST_REQUIRE_EQUAL(100, dollar.

precision()

);
BOOST_REQUIRE_EQUAL("DLLR", dollar.

name()

);
BOOST_REQUIRE_EQUAL(true, dollar.
...
{
symbol sys(4, "SYS");
BOOST_REQUIRE_EQUAL(SY(4, SYS), sys
.

value()

);
BOOST_REQUIRE_EQUAL("4,SYS", sys.

to_string()

);
BOOST_REQUIRE_EQUAL("SYS", sys.

name()

);
BOOST_REQUIRE_EQUAL(4, sys.
...
{
symbol def;
BOOST_REQUIRE_EQUAL(4, def.

decimals()

);
BOOST_REQUIRE_EQUAL(CORE_SYMBOL_NAME, def
.
...
BOOST_REQUIRE_EQUAL(3, y.

decimals()

);
BOOST_REQUIRE_EQUAL("YEN", y.
...
BOOST_REQUIRE_EQUAL(0, sym.

decimals()

);
BOOST_REQUIRE_EQUAL("EURO", sym.
...
name = N(setowner);
setowner_act.
authorization = vector < permission_level > {{N(proxy), config::active_name}};
setowner_act.
data = proxy_abi_ser.variant_to_binary("setowner", mutable_variant_object()
                                               ("owner", "alice")
                                               ("delay", 10),
                                       abi_serializer_max_time
);
trx.actions.
emplace_back(std::move(setowner_act)
);

set_transaction_headers(trx);
trx.
sign(get_private_key(N(proxy), "active"), control
->
...
chain_has_transaction(deferred_id)
);
...
); // Still one because the first deferred transaction retires but the second is created at the same time.
BOOST_REQUIRE_EQUAL(get_transaction_receipt(deferred_id)
.status, transaction_receipt::soft_fail);
...
name = N(setowner);
setowner_act.
authorization = vector < permission_level > {{N(bob), config::active_name}};
setowner_act.
data = proxy_abi_ser.variant_to_binary("setowner", mutable_variant_object()
                                               ("owner", "alice")
                                               ("delay", 0),
...
set_transaction_headers(trx);
trx.
sign(get_private_key(N(bob), "active"), control
->
...
BOOST_REQUIRE_EQUAL(false,
chain_has_transaction(deferred2_id)
);
}
...
BOOST_REQUIRE_EQUAL(get_transaction_receipt(deferred2_id)
.status, transaction_receipt::executed);
