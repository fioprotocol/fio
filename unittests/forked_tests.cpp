...
while( bios.control->pending_block_state()->header.producer.

to_string()

!= "a" || bios.control->head_block_state()->header.producer.
...
BOOST_REQUIRE_EQUAL( bios
.control->head_block_state()->header.producer.
...
// corrupt this block
fork.
block_merkle = remote.control->head_block_state()->blockroot_merkle;
copy_b->action_mroot._hash[0] ^= 0x1ULL;
} else if (j<i) {
// link to a corrupted chain
copy_b->
previous = fork.blocks.back()->id();
}
...
auto sig_digest = digest_type::hash(
        std::make_pair(header_bmroot, remote.control->head_block_state()->pending_schedule_hash));
copy_b->
producer_signature = remote.get_private_key(N(b), "active").sign(sig_digest);
...
for(
uint32_t start = fork_block_num + 1, end = c2.control->head_block_num();
start <=
end;
++start ) {
wdump((start));
...
produce_block ( fc::milliseconds(config::block_interval_ms *

25)); // cam skips over sam and pam's blocks
...
25)); // pam skips over dan and sam's blocks
c2.produce_blocks(11); // pam finishes the remaining 11 blocks
c2.

produce_block ( fc::milliseconds(config::block_interval_ms *

25)); // sam skips over cam and dan's blocks
...
13)); // cam skips over pam's blocks (this block triggers a block on this branch to become irreversible)
...
for(
uint32_t start = fork_block_num + 1, end = c2.control->head_block_num() - 1;
start <=
end;
++start ) {
auto fb = c2.control->fetch_block_by_number(start);
...
bad_block.
transaction_mroot = bad_block.previous;
auto bad_block_bs = c.control->create_block_state_future(std::make_shared<signed_block>(std::move(bad_block)));
c.control->

abort_block();

BOOST_REQUIRE_EXCEPTION(c
.control->
push_block( bad_block_bs ), fc::exception,
[] (
const fc::exception &ex
)->bool {
return ex.

to_detail_string()

.find("block not signed by expected key") !=
std::string::npos;
});
}
...
BOOST_REQUIRE_EQUAL(61u, c.control->

head_block_num()

);
BOOST_REQUIRE_EQUAL(61u, c2.control->
...
produce_block (fc::milliseconds(config::block_interval_ms *skip1));
...
produce_block (fc::milliseconds(config::block_interval_ms *skip2));
...
BOOST_REQUIRE_EQUAL(87u, c.control->

head_block_num()

);
BOOST_REQUIRE_EQUAL(73u, c2.control->
...
BOOST_REQUIRE_EQUAL(73u, c.control->
...
BOOST_REQUIRE_EQUAL(head_block_num, head
.control->
...
BOOST_REQUIRE_EQUAL(head_block_num, read_only
.control->
...
BOOST_REQUIRE_EQUAL(head_block_num, irreversible
.control->

fork_db_head_block_num()

);
BOOST_REQUIRE_EQUAL(head_block_num
- 49, irreversible.control->
