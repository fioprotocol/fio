...
namespace _test_multi_index {
    ...
    void idx64_check_without_storing(name receiver) {
        ...
        // find by primary key
        {
            ...
            check(itr != table.end() && itr->sec == "emily"_n.value,
                  "idx64_general - table.require_find() of existing primary key");
        ...
        // iterate forward starting with charlie
        {
            ...
            check(itr != secondary_index.end() && itr->sec == "charlie"_n.value,
                  "idx64_general - secondary_index.lower_bound()");

            ++itr;
            check(itr != secondary_index.end() && itr->id == 976 && itr->sec == "emily"_n.value,
                  "idx64_general - increment secondary iterator");

            ++itr;
            check(itr != secondary_index.end() && itr->id == 110 && itr->sec == "joe"_n.value,
        ...
        // iterate backward starting with second bob
        {
            ...
            check(pk_itr != table.end() && pk_itr->sec == "bob"_n.value,
                  "idx64_general - table.find() of existing primary key");

            auto itr = secondary_index.iterator_to(*pk_itr);
            check(itr->id == 781 && itr->sec == "bob"_n.value,
                  "idx64_general - iterator to existing object in secondary index");

            --itr;
            check(itr != secondary_index.end() && itr->id == 540 && itr->sec == "bob"_n.value,
                  "idx64_general - decrement secondary iterator");

            --itr;
            check(itr != secondary_index.end() && itr->id == 650 && itr->sec == "allyson"_n.value,
                  "idx64_general - decrement secondary iterator again");

            --itr;
            check(itr == secondary_index.begin() && itr->id == 265 && itr->sec == "alice"_n.value,
    ...
    void idx64_require_find_sk_fail(name receiver) {
        ...
        // Load table using multi_index
        multi_index < name{TableName}, record, indexed_by < "bysecondary"_n, const_mem_fun < record, uint64_t,
                &record::get_secondary >> > table(receiver, receiver.value);
...
void test_api_multi_index::idx128_autoincrement_test() {
    ...
    check(expected_key == static_cast<uint64_t>(-1),
          "idx128_autoincrement_test - did not iterate through secondary index properly");
    ...
    */
    ...
    check(table.available_primary_key() == 101,
...
void test_api_multi_index::idx128_autoincrement_test_part2() {
...

    {
        ...
        check(table.available_primary_key() == 3,
    ...
    check(expected_key == static_cast<uint64_t>(-1),
          "idx128_autoincrement_test_part2 - did not iterate through secondary index properly");
...
void test_api_multi_index::idx64_pass_pk_ref_to_other_table() {
    ...
    check(table1_pk_itr != table1.end() && table1_pk_itr->sec == "bob"_n.value,
          "idx64_pass_pk_ref_to_other_table - table.find() of existing primary key");
...
void test_api_multi_index::idx64_pass_sk_ref_to_other_table() {
    ...
    auto table1_pk_itr = table1.find(781);
    check(table1_pk_itr != table1.end() && table1_pk_itr->sec == "bob"_n.value,
          "idx64_pass_sk_ref_to_other_table - table.find() of existing primary key");
...
void test_api_multi_index::idx64_modify_primary_key() {
    ...
    auto pk_itr = table.find(781);
    check(pk_itr != table.end() && pk_itr->sec == "bob"_n.value,
          "idx64_modify_primary_key - table.find() of existing primary key");
...
void test_api_multi_index::idx64_run_out_of_avl_pk() {
    ...
    check(table.available_primary_key() == static_cast<uint64_t>(-3),
          "idx64_run_out_of_avl_pk - incorrect available primary key");
...
void test_api_multi_index::idx64_sk_cache_pk_lookup() {
    ...
    check(sk_itr != sec_index.end() && sk_itr->id == 540,
          "idx64_sk_cache_pk_lookup - sec_index.find() of existing secondary key");
...
void test_api_multi_index::idx64_pk_cache_sk_lookup() {
    ...
    check(pk_itr != table.end() && pk_itr->sec == "bob"_n.value,
          "idx64_pk_cache_sk_lookup - table.find() of existing primary key");
