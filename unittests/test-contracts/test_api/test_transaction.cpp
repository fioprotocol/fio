...
void test_transaction::send_action_recurse() {
    ...
    std::vector <permission_level> permissions = {{"testapi"_n, "active"_n}};
    action act(permissions, name{"testapi"}, name{WASM_TEST_ACTION("test_transaction", "send_action_recurse")},
               test_action);
...
void test_transaction::send_transaction(uint64_t receiver, uint64_t, uint64_t) {
    ...
    trx.actions.emplace_back(permissions, name{"testapi"}, name{WASM_TEST_ACTION("test_action", "read_action_normal")},
                             test_action);
...
void test_transaction::send_action_sender(uint64_t receiver, uint64_t, uint64_t) {
    ...
    trx.actions.emplace_back(permissions, name{"testapi"}, name{WASM_TEST_ACTION("test_action", "test_current_sender")},
                             &cur_send);
    trx.send(0, name{receiver});
}
...
void test_transaction::send_transaction_trigger_error_handler(uint64_t receiver, uint64_t, uint64_t) {
    ...
    trx.actions.emplace_back(permissions, name{"testapi"}, name{WASM_TEST_ACTION("test_action", "assert_false")},
                             test_action);
    trx.send(0, name{receiver});
}
...
void test_transaction::assert_false_error_handler(const eosio::transaction &dtrx) {
    ...
    eosio_assert(dtrx.actions[0].name.value == WASM_TEST_ACTION("test_action", "assert_false"),
                 "transaction has wrong name");
    ...
    eosio_assert(dtrx.actions[0].authorization[0].permission == "active"_n,
                 "action's authorization has wrong permission");
}
...
void test_transaction::send_transaction_large(uint64_t receiver, uint64_t, uint64_t) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        trx.actions.emplace_back(permissions, name{"testapi"},
                                 name{WASM_TEST_ACTION("test_action", "read_action_normal")}, test_action);
    }
...
void test_transaction::send_deferred_transaction(uint64_t receiver, uint64_t, uint64_t) {
    ...
    trx.actions.emplace_back(permissions, name{"testapi"}, name{WASM_TEST_ACTION("test_transaction", "deferred_print")},
                             test_action);
    trx.delay_sec = 2;
...
void test_transaction::send_cf_action_fail() {
    using namespace eosio;
    action act(std::vector < permission_level > {{"dummy"_n, "active"_n}}, "dummy"_n, "event1"_n,
               std::vector < char > {});
    act.send_context_free();
