...
BOOST_REQUIRE_THROW( push_reqauth(N(alice), {permission_level{N(alice), config::active_name}},
...
BOOST_REQUIRE_THROW( push_reqauth(N(alice), {permission_level{N(bob), config::active_name}},
                                  {get_private_key(N(bob), "active")}), missing_auth_exception
);
...
auto original_auth = static_cast<authority>(control->get_authorization_manager().get_permission(
        {N(alice), config::active_name}).auth);
wdump((original_auth));

set_authority( N(alice), config::active_name, delegated_auth
);

auto new_auth = static_cast<authority>(control->get_authorization_manager().get_permission(
        {N(alice), config::active_name}).auth);
wdump((new_auth));
BOOST_CHECK_EQUAL((new_auth
== delegated_auth), true);
...
auto auth = static_cast<authority>(control->get_authorization_manager().get_permission(
        {N(alice), config::active_name}).auth);
wdump((auth));
BOOST_CHECK_EQUAL((new_auth
== auth), true);
...
auto trace = push_reqauth(N(alice), {permission_level{N(alice), config::active_name}},
                          {get_private_key(N(bob), "active")});
...
BOOST_AUTO_TEST_CASE(update_auths) {
        try {
            ...
            BOOST_CHECK_THROW(chain.set_authority("alice", "spending", authority(spending_pub_key), "active",
                                                  {permission_level{"bob", "active"}},
                                                  {chain.get_private_key("bob", "active")}),
            ...
            BOOST_CHECK_THROW(chain.set_authority("alice", "spending", spending_pub_key, "spending",
                                                  {permission_level{"alice", "spending"}}, {spending_priv_key}),
...
BOOST_AUTO_TEST_CASE(link_auths) {try {
    ...
    BOOST_CHECK_THROW(chain.push_reqauth("alice", {permission_level{N(alice), "spending"}}, {spending_priv_key}),
                      irrelevant_auth_exception);
    ...
    BOOST_CHECK_THROW(chain.push_reqauth("alice", {permission_level{N(alice), "scud"}}, {scud_priv_key}),
                      irrelevant_auth_exception);
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) {try {
    ...
    BOOST_CHECK_THROW(chain.push_reqauth("alice", {permission_level{N(alice), "first"}}, {first_priv_key}),
                      unsatisfied_authorization);
...
BOOST_AUTO_TEST_CASE(create_account) {
        try {
            ...
            const auto &joe_active_authority = chain.get<permission_object, by_owner>(
            ...
            BOOST_CHECK_EXCEPTION(chain.create_account("joe"), action_validate_exception,
                                  fc_exception_message_is(
                                          "Cannot create account named joe, as that name is already taken"));
...
BOOST_AUTO_TEST_CASE(stricter_auth) {
        try {
            ...
            auto create_acc = [&](account_name a, account_name creator, int threshold) {
                ...
                authority invalid_auth = authority(threshold, {key_weight{chain.get_public_key(a, "owner"), 1}},
