...
BOOST_REQUIRE_MESSAGE(table_allocation_bytes
> increment_contract_bytes, "increment_contract_bytes must be less than table_allocation_bytes for this test setup to work");
buyrambytes(config::system_account_name, config::system_account_name,
70000);
produce_blocks(10);
...
BOOST_REQUIRE_EQUAL( success(), stake("eosio.stake", "testram11111", core_from_string("10.0000"),
                                      core_from_string("5.0000"))
...
i < 10; ++i) {
try {
set_abi( N(testram11111), contracts::test_ram_limit_abi()
...
set_abi( N(testram22222), contracts::test_ram_limit_abi()
...
BOOST_REQUIRE_MESSAGE(more_ram
>= 0, "Underlying understanding changed, need to reduce size of init_request_bytes");
wdump((init_bytes)(initial_ram_usage)
(init_request_bytes)(more_ram));
...
push_action( N(testram11111), N(setentry), N(testram11111), mvo()
...
wdump((ram_bytes)(ram_usage)
(initial_ram_usage)(init_bytes)(ram_usage - initial_ram_usage)(init_bytes - ram_usage));
...
wlog("ram_tests 2    %%%%%%");
produce_blocks(1);
BOOST_REQUIRE_EQUAL(ram_usage, rlm
.

get_account_ram_usage (N(testram11111))
...
push_action( N(testram11111), N(rmentry), N(testram11111), mvo()
...
push_action( N(testram11111), N(setentry),

{
N(testram11111), N(testram22222)
},
