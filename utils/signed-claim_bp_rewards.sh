...
echo ------------------------------------------
dataJson="{
  \"fio_address\": \"producera:cryptowallet\",
  \"actor\": \"${fioactor}\"
}"

expectedPackedData=056461706978104208414933a95b
cmd="programs/cleos/cleos --no-auto-keosd --url http://$hostname:$nPort --wallet-url http://$hostname:$wPort  convert pack_action_data fio.treasury bpclaim '${dataJson}'"
echo CMD: $cmd
