...
#before running this one needs to run this command on the server to make an expired domain..
#../build/bin/cleos -u http://localhost:8889 push action -j fio.system expdomain '{"actor":"r41zuwovtn44","domain":"expired"}' --permission r41zuwovtn44@active
...
dataJson="{\"fio_domain\":\"expired\",\"max_fee\":\"100000000000\",\"tpid\":\"1\",\"actor\":\"r41zuwovtn44\",\"tpid\":\"adam.dapix\"}"
expectedPackedData=056461706978104208414933a95b
cmd="programs/cleos/cleos --no-auto-keosd --url http://$hostname:$nPort --wallet-url http://$hostname:$wPort  convert pack_action_data fio.system renewdomain '$dataJson'"
echo CMD: $cmd
