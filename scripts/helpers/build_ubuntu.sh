...
echo
[[ $VERSION_ID == "20.04" ]] && [[ $PIN_COMPILER==false ]] && echo "Ubuntu 20.04 requires a PINNED (-P) Build; updating to use pinned build workflow" && PIN_COMPILER=true
echo

# system clang and build essential for Ubuntu 18 (16 too old)
#( [[ $PIN_COMPILER == false ]] && [[ $VERSION_ID == "18.04" ]] ) && EXTRA_DEPS=(clang,dpkg\ -s)
( [[ $PIN_COMPILER == false ]] && ( [[ $VERSION_ID == "18.04" ]] || [[ $VERSION_ID == "20.04" ]] ) ) && EXTRA_DEPS=(clang,dpkg\ -s)
